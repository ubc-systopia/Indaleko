...
class IndalekoRelationship:
    ...
    def validate_vertex(self, vertex: dict) -> bool:
        ...
        assert Indaleko.validate_uuid_string(
            vertex["object"]
        ), f"object ({vertex['object']}) must be a valid UUID."
    ...
    def serialize(self) -> dict:
        ...
        serialized_data = IndalekoRelationshipDataModel.serialize(
            self.indaleko_relationship
        )
        serialized_data["_from"] = (
            f'{self.vertex1["collection"]}/{self.vertex1["object"]}'
        )
        serialized_data["_to"] = (
            f'{self.vertex2["collection"]}/{self.vertex2["object"]}'
        )
...
def main():
    ...
    r = IndalekoRelationship(
        source={"Identifier": args.source, "Version": "1.0"},
