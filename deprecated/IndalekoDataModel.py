...
class IndalekoUUID:
    ...
    Identifier: Annotated[
        UUID, schema(description="A Universally Unique Identifier", format="uuid")
    ]

    Label: Annotated[
        Optional[str], schema(description="A human-readable label for the UUID.")
    ] = Undefined

    @staticmethod
    def get_indaleko_uuid(identifier: UUID, label: str = None) -> "IndalekoUUID":
        """Lookup a UUID."""
        return IndalekoUUID(Identifier=identifier, Label=label)
...
class IndalekoDataModel:
    ...
    class SourceIdentifier:
        ...
        Identifier: Annotated[
            UUID, schema(description="The identifier of the source of the data.")
        ]
        Version: Annotated[
            str, schema(description="The version of the source of the data.")
        ]
        ...
        def serialize(data) -> Dict[str, Any]:
            """Serialize the object to a dictionary."""
            return serialize(
                IndalekoDataModel.SourceIdentifier, data, additional_properties=True
            )
    ...
    class Timestamp:
        ...
        Value: Annotated[
            datetime,
            schema(
                description="Timestamp in ISO date and time format.", format="date-time"
            ),
            schema(
                description="Timestamp in ISO date and time format.", format="date-time"
            ),
        ...
        Description: Annotated[
            Optional[str], schema(description="Description of the timestamp.")
        ] = None
    ...
    def get_timestamp(
        ...
        description: str = "Prototype description",
    ) -> "IndalekoDataModel.Timestamp":
        """Lookup a timestamp."""
        return IndalekoDataModel.Timestamp(
            Label=uuid, Value=value, Description=description
        )
...
def main():
    ...
    ic(
        print_schema(
            graphql_schema(
                query=IndalekoDataModel.get_queries(),
    ...
    ic(
        IndalekoDataModel.SourceIdentifier.serialize(
            IndalekoDataModel.get_source_identifier(source_id["Identifier"])
        )
    )
    ...
    ic(
        IndalekoDataModel.get_timestamp(
            UUID(timestamp["Label"]),
            datetime.fromisoformat(timestamp["Value"]),
            timestamp["Description"],
        )
    )

    indaleko_uuid = {"Identifier": str(uuid4()), "Label": "This is a test UUID"}
    ic(IndalekoUUID.deserialize(indaleko_uuid))
    ic(
        IndalekoUUID.get_indaleko_uuid(
            UUID(indaleko_uuid["Identifier"]), indaleko_uuid["Label"]
        )
    )
    ic(
        IndalekoUUID.serialize(
            IndalekoUUID.get_indaleko_uuid(
                UUID(indaleko_uuid["Identifier"]), indaleko_uuid["Label"]
            )
        )
