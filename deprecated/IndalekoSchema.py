...
class IndalekoSchema:
    ...
    def build_from_definitions() -> "IndalekoSchema":
        ...
        return IndalekoSchema(offline=True, schema=schema)

    @staticmethod
    def build_from_db() -> "IndalekoSchema":
        """Build the schema from the database."""
        db_config = importlib.import_module(
            ".db.db_config", package="Indaleko"
        ).IndalekoDBConfig()
        db_config.start()
        schema = {}
        ...
        return IndalekoSchema(offline=False, schema=schema)
        ...
        for interface in schema_interfaces:
            ...
            if collection_name not in Indaleko.Collections:
                if "Object" == collection_name:
                    schema[Indaleko.Indaleko_Object_Collection] = getattr(
                        module, interface
                    )
                if "Relationship" == collection_name:
                    schema[Indaleko.Indaleko_Relationship_Collection] = getattr(
                        module, interface
                    )
...
class IndalekoSchemaInterface:
    ...
    def __init__(self) -> None:
        ...
        command_parser = self.parser.add_subparsers(
            dest="command", help="Command to run"
        )
        show_command = command_parser.add_parser("show", help="Show Indaleko schema")
        show_command_parser = show_command.add_subparsers(
            dest="subcommand", help="Subcommand to run"
        )
        show_db_schema = show_command_parser.show_db = show_command_parser.add_parser(
            "db", help="Show the database schema"
        )
        show_db_schema.set_defaults(func=IndalekoSchemaInterface.show_db_schema)
        show_library_schema = show_command_parser.show_library = (
            show_command_parser.add_parser("library", help="Show the library schema")
        )
        show_library_schema.set_defaults(
            func=IndalekoSchemaInterface.show_library_schema
        )
        show_definitions = show_command_parser.add_parser(
            "definitions", help="Show the schema from the definitions"
        )
        show_definitions.set_defaults(
            func=IndalekoSchemaInterface.show_definitions_schema
        )
