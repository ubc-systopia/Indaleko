...
class IndalekoSchemaMapping:
    ...
    def __init__(self, db_config: IndalekoDBConfig = None):
        ...
        for collection in self.collections.collections.values():
            ...
            self.mapped_schema[collection.name] = self.map_schema(
                collection.name, self.schema[collection.name]
            )
    ...
    def object_handler(value):
        ...
        assert isinstance(
            value, dict
        ), f"Expected dictionary, got {type(value)} ({value})"
    ...
    def map_schema(self, collection_name: str, schema: dict) -> dict:
        ...
        for label, value in schema.items():
            ...
            if "type" not in value and "properties" not in value:
                ic(
                    "Skipping value without type or properties",
                    collection_name,
                    label,
                    value,
                )
    ...
    def generate_graphql_field(name: str, details: dict, required_fields: list) -> str:
        ...
        if isinstance(details, dict):
            graphql_type = IndalekoSchemaMapping.arango_to_graphql_type_mapping(
            ...
            for item in details:
                if isinstance(item, dict):
                    fields.append(
                        IndalekoSchemaMapping.generate_graphql_field(
                            name, item, required_fields
                        )
                    )
                    ...
                    if name != "required":
                        fields.append(
                            IndalekoSchemaMapping.generate_graphql_field(
                                f"{name}.{item}", details[item], required_fields
                            )
                        )
                else:
                    raise AssertionError(
                        f"List items must be dictionaries or strings, not {type(item)} ({item}, {details})"
                    )
            return "\n".join(fields)
        else:
            raise AssertionError(
                f"Details must be a dictionary or list, not {type(details)} ({details})"
            )
    ...
    def process_properties(properties: dict, required_fields: list) -> list:
        ...
        for prop, details in properties.items():
            if "properties" in details:
                ...
                nested_fields = IndalekoSchemaMapping.process_properties(
                    details["properties"], details.get("required", [])
                )
                ...
                nested_fields = IndalekoSchemaMapping.process_properties(
                    item_details["properties"], item_details.get("required", [])
                )
                fields.append(
                    f"  {prop}: [{details['type']} {{\n"
                    + "\n".join(nested_fields)
    ...
    def generate_graphql_type(name, schema):
        ...
        if "properties" in schema["rule"]:
            fields = IndalekoSchemaMapping.process_properties(
                schema["rule"]["properties"], required_fields
            )
        else:
            for key, value in schema["rule"].items():
                fields.append(
                    IndalekoSchemaMapping.generate_graphql_field(
                        key, value, required_fields
                    )
                )
    ...
    def generate_graphql_schema(self):
        ...
        for collection, schema in self.schema.items():
            graphql_types.append(
                IndalekoSchemaMapping.generate_graphql_type(collection, schema)
            )
        return "\n\n".join(graphql_types)
