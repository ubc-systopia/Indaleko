...
class RelationshipGeneratorAgent(DomainAgent):
    ...
    def _transform_to_db_format(self, record: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Args:
            record: Generated record from RelationshipGeneratorTool

        Returns:
            Transformed record in database format for ArangoDB edge collection
        """
        # Check if this is already in IndalekoRelationshipDataModel format
        if "Record" in record and "Objects" in record and "Relationships" in record:
            ...
            elif isinstance(object_ids, list) and len(object_ids) >= 2:
                source_id = object_ids[0]
                target_id = object_ids[1]
            else:
            ...
            db_record = {
                ...
                "_from": source_id,
                "_to": target_id,
                "objects": list(object_ids) if isinstance(object_ids, tuple) else object_ids,
