...
class IndalekoMachineConfig(IndalekoRecord):
    ...
    def set_platform(self, platform : dict) -> None:
        ...
        '''
        print('\n', platform, '\n')
        assert isinstance(platform, dict), f'platform must be a dict (not {type(platform)})'
        assert 'software' in platform, 'platform must contain a software field'
        assert isinstance(platform['software'], dict), f'platform["software"] must be a dictionary, not {type(platform["software"])}'
        assert isinstance(platform['software']['OS'], str), f'platform must contain a string OS field, not {type(platform["software"]["OS"])}'
        assert isinstance(platform['software']['Version'], str), 'platform must contain a string version field'
        assert isinstance(platform['software']['Architecture'], str), 'platform must contain a string architecture field'
        assert 'hardware' in platform, 'platform must contain a hardware field'
        assert isinstance(platform['hardware'], dict), 'platform["hardware"] must be a dictionary'
        assert isinstance(platform['hardware']['CPU'], str), 'platform must contain a string CPU field'
        assert isinstance(platform['hardware']['Version'], str), 'platform must contain a string version field'
        assert isinstance(platform['hardware']['Cores'], int), 'platform must contain an integer cores field'
    ...
    def set_captured(self, timestamp : datetime) -> None:
        ...
        if isinstance(timestamp, dict):
            ...
            assert isinstance(timestamp['Value'], str), 'timestamp must contain a string Value field'
    ...
    def load_config_from_db(machine_id : str) -> 'IndalekoMachineConfig':
        ...
        if isinstance(entry['source'], str) and 'version' in entry:
            ...
            assert isinstance(entry['source'], dict), f'entry["source"] must be a dict, not {type(entry["source"])}'
    ...
    def build_config(**kwargs) -> 'IndalekoMachineConfig':
        ...
        assert isinstance(kwargs['source_version'], str), 'Source version must be a string'
