    ...
    def fetch_directory(self, url, timeout=10):
        ...
        while retries > 0:
            ...
            try:
                ...
                retries -= 1
                if response is not None and response.status_code == 401:  # seems to indicate a stale token
    ...
    def get_onedrive_metadata(
        ...
        while len(queue) > 0:
            ...
            try:
                ...
                logging.exception(ic(f"Request timed out: {e}. Retrying {url}"))
                queue.insert(0, url)
                continue
            except requests.exceptions.RequestException as e:
                logging.exception("Error: %s - %s", response.status_code, response.text)
                ic(e)
