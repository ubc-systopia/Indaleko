...
class IndalekoOneDriveCloudStorageCollector(BaseCloudStorageCollector):
    ...
    indaleko_onedrive_collector_service_description = (
        "Indexes the OneDrive contents for Indaleko."
    )
    indaleko_onedrive_collector_service_version = "1.0"
    indaleko_onedrive_collector_service_type = (
    ...
    class MicrosoftGraphCredentials:
        ...
        def reset_chosen_account(
            self,
        ) -> "IndalekoOneDriveCloudStorageCollector.MicrosoftGraphCredentials":
            self.__chosen_account__ = -1
            return self
        ...
        def get_account_name(self):
            if self.__output_file_name__ is None:
                ...
                accounts = self.app.get_accounts()
                if accounts:
                    self.__output_file_name__ = accounts[
                        self.__get_chosen_account__()
                    ].get("username")
    ...
    def __init__(self, **kwargs):
        ...
        self.onedrive_config_file = os.path.join(
            self.config_dir, IndalekoOneDriveCloudStorageCollector.onedrive_config_file
        )
        self.onedrive_token_file = os.path.join(
            self.config_dir, IndalekoOneDriveCloudStorageCollector.onedrive_token_file
        )
        self.graphcreds = (
            IndalekoOneDriveCloudStorageCollector.MicrosoftGraphCredentials(
                config=self.onedrive_config_file, cache_file=self.onedrive_token_file
            )
        )
        if "platform" not in kwargs:
        ...
        if "collector_data" not in kwargs:
            kwargs["collector_data"] = (
                IndalekoOneDriveCloudStorageCollector.onedrive_collector_data
            )
    ...
    def get_drives(self) -> list:
        ...
        if response.status_code == 200:
            ...
            logging.error(
                "Error retrieving drives: %s - %s", response.status_code, response.text
            )
            raise ValueError(
        ...
        while retries > 0:
            ...
            try:
                ...
                response = requests.get(
                    url, headers=headers, params=params, timeout=(10, 30)
                )
                sys.settrace(None)
                ...
                retries -= 1
                logging.error(
                    f"{tid} Request timed out: {e}. Retrying {retries} more times."
                )
                ...
                retries -= 1
                if (
                    response is not None and 401 == response.status_code
                ):  # seems to indicate a stale token
                    logging.info(f"{tid} : Request failed (401).  Refresh token.")
                    self.graphcreds.clear_token()
                    headers = self.get_headers()
                else:
                    logging.error(
                        f"{tid} Request failed: {e}. Retrying {retries} more times."
                    )
                ...
                retries -= 1
                logging.error(
                    f"{tid} Request hard timeout): {e}. Retrying {retries} more times."
                )
    ...
    def get_url_for_folder(
        drive_id: str = None, folder_id: str = None, return_children: bool = True
    ) -> None:
    ...
    def fetch_onedrive_metadata(
        self, drive_id: str = "me", item_id: str = "root"
    ) -> dict:
    ...
    def get_onedrive_metadata(
        self, drive_id: str = None, folder_id: str = None
    ) -> list:
        """This method retrieves the metadata of the OneDrive."""
        queue = []
        refresh_count = 0
        ...
        url = self.get_url_for_folder(
            drive_id=drive_id, folder_id=folder_id, return_children=False
        )
        queue.append(url)
        results = []
        passes = 0
        while len(queue) > 0:
            ...
            try:
                ...
                if 401 == response.status_code:  # seems to indicate a stale token
                    ...
                    logging.warning(
                        ic(
                            f"Error: for URL {url} - {response.status_code} - {response.text}"
                        )
                    )
            ...
            if "value" not in response.json():
                ...
                url = self.get_url_for_folder(
                    drive_id=drive_id, folder_id=item["id"], return_children=True
                )
                queue.append(url)
                continue
            for item in response.json()["value"]:
                if "folder" in item and self.recurse:
                    queue.append(
                        self.get_url_for_folder(drive_id=None, folder_id=item["id"])
                    )
                    self.dir_count += 1
                else:
                    self.file_count += 1
    ...
    def find_collector_files(
        ...
        return [
            f
            for f in prospects
            if IndalekoOneDriveCloudStorageCollector.onedrive_platform in f
        ]
