...
class EntityResolutionService:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized EntityResolutionService for machine {self.machine_id} "
        ...
        for request in batch:
            ...
            try:
                ...
                if request["attempts"] > self.max_retries:
                    logger.warning(
                        f"Request {request_id} exceeded maximum retries "
    ...
    def _resolve_entity(self, request: Dict) -> bool:
        ...
        logger.info(
            f"Resolving {entity_type} entity: "
            f"{entity_info['volume_guid']}:{entity_info['frn']}"
        )
