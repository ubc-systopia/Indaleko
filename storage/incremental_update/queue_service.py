...
class EntityResolutionQueue:
    ...
    def _find_existing_request(self, machine_id: str, entity_info: EntityInfo) -> str | None:
        """
        ...
        bind_vars = {"machine_id": machine_id, "volume_guid": entity_info.volume_guid, "frn": entity_info.frn}
        ...
        cursor = db.aql.execute(query, bind_vars=bind_vars, bind_vars_names={"@collection": self.COLLECTION_NAME})

        result = list(cursor)
        return result[0] if result else None

    def dequeue(self, machine_id: str, batch_size: int = 10, prefer_directories: bool = True) -> list[dict]:
        """
        ...
        if prefer_directories:
            directory_batch = self._dequeue_by_type(machine_id, EntityType.DIRECTORY, batch_size)
            ...
            file_batch = self._dequeue_by_type(machine_id, EntityType.FILE, remaining)
    ...
    def _dequeue_by_type(self, machine_id: str, entity_type: EntityType, batch_size: int) -> list[dict]:
        """
    ...
    def get_queue_stats(self, machine_id: str | None = None) -> dict:
        ...
        return (
            result[0]
            if result
            else {"total": 0, "pending": 0, "processing": 0, "completed": 0, "failed": 0, "details": []}
        )
