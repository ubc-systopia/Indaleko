...
def test_create_and_retrieve_patterns(kb_manager, event_id):
    ...
    if query_patterns:
        ...
        print(
            f"Pattern data: {json.dumps(query_patterns[0].pattern_data, default=str)}",
        )
...
def test_entity_relationships(kb_manager):
    ...
    for entity in related_entities:
        print(
            f"- {entity['entity_name']} ({entity['entity_type']}): {entity['relationship_type']} "
...
def run_all_tests():
    ...
    try:
        ...
        print(f"WARNING: {e!s}")
        print(
            "To complete setup, add the Knowledge Base collections to db_collections.py",
        )
    ...
    if args.setup_check:
        try:
            ...
            if collections_defined:
                print(
                    "✓ Knowledge Base collections are defined in IndalekoDBCollections",
                )
            else:
                print(
                    "✗ Knowledge Base collections are NOT defined in IndalekoDBCollections",
                )
            ...
            try:
                events_collection = IndalekoCollections.get_collection(
                    Indaleko_Learning_Event_Collection,
                )
                patterns_collection = IndalekoCollections.get_collection(
                    Indaleko_Knowledge_Pattern_Collection,
                )
                feedback_collection = IndalekoCollections.get_collection(
                    Indaleko_Feedback_Record_Collection,
                )
                ...
                print(f"✗ Failed to retrieve Knowledge Base collections: {e!s}")
                print(
                    "Complete the setup by adding the collections to db_collections.py",
                )
                return
        except Exception as e:
            print(f"Error during setup check: {e!s}")
            return
