...
from archivist.knowledge_base.data_models.knowledge_pattern import KnowledgePatternType
...
class TestContinuousLearning(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Patch imports first
        self.db_config_patcher = patch('archivist.knowledge_base.continuous_learning.IndalekoDBConfig')
        self.collections_patcher = patch('archivist.knowledge_base.continuous_learning.IndalekoCollections')
        self.db_collections_patcher = patch('archivist.knowledge_base.continuous_learning.IndalekoDBCollections')
        ...
        from archivist.knowledge_base.continuous_learning import ContinuousLearningSystem
        ...
        self.learning_system._get_sample_document = MagicMock(return_value={"name": "test"})
        
        # Sample test data
        self.sample_query = "Find PDF documents created last week"
        self.sample_results = [
            {"_id": "doc1", "name": "report.pdf", "created": datetime.now(timezone.utc).isoformat()},
            {"_id": "doc2", "name": "presentation.pdf", "created": datetime.now(timezone.utc).isoformat()}
        ]
    ...
    def test_discover_collectors_and_recorders(self, mock_inspect, mock_pkgutil, mock_importlib):
        ...
        discovery_results = self.learning_system.discover_collectors_and_recorders(force=True)
        ...
        self.assertEqual(call_args[1]['event_type'], LearningEventType.pattern_discovery)
    ...
    def test_analyze_collection_schemas(self):
        ...
        with patch('archivist.knowledge_base.continuous_learning.IndalekoDBCollections.Collections', 
            ...
            self.learning_system._analyze_field_usage = lambda collection_name, fields: {
                "name": {"count": 100, "percentage": 100, "field_type": "string", "required": True},
                "count": {"count": 80, "percentage": 80, "field_type": "number", "required": False}
            }
            self.learning_system._calculate_type_distributions = lambda collection_name, fields: {
                "name": {"counts": {"string": 100}, "percentages": {"string": 100}, "expected_type": "string"},
                "count": {"counts": {"number": 80}, "percentages": {"number": 80}, "expected_type": "number"}
            }
            
            # Perform the analysis
            schema_analysis = self.learning_system.analyze_collection_schemas(force=True)
    ...
    def test_detect_collector_changes(self):
        ...
        self.learning_system._collector_cache = {
            'activity': [
                {'class': 'ActivityCollector', 'module': 'activity.collectors.activity', 'path': 'activity.collectors.activity.ActivityCollector'}
            ],
            'storage': [
                {'class': 'StorageCollector', 'module': 'activity.collectors.storage', 'path': 'activity.collectors.storage.StorageCollector'}
            ]
        }
        self.learning_system._recorder_cache = {
            'activity': [
                {'class': 'ActivityRecorder', 'module': 'activity.recorders.activity', 'path': 'activity.recorders.activity.ActivityRecorder'}
            ]
        }
        self.learning_system._last_collector_discovery = datetime.now(timezone.utc)
        ...
        mock_pattern.updated_at = datetime.now(timezone.utc)
        ...
        def mock_discover(*args, **kwargs):
            return {
                'collectors': {
                    ...
                    'semantic': [
                        {'class': 'SemanticCollector', 'module': 'activity.collectors.semantic', 'path': 'activity.collectors.semantic.SemanticCollector'}
                    ]
                },
                'recorders': {
                    'activity': [
...
def run_tests(args):
    ...
    print("Note: The unit tests require the continuous_learning.py module to be present in the archivist/knowledge_base directory.")
    print("These tests are designed to verify the behavior of that module once it's implemented.")
    print("Currently running the demo instead, which shows the expected functionality with mocked objects.\n")
...
def run_demo(args):
    ...
    class MockContinuousLearningSystem:
        ...
        def discover_collectors_and_recorders(self, force=False):
            return {
                'collectors': {
                    ...
                    'storage': [
                        {'class': 'StorageCollector', 'module': 'activity.collectors.storage', 'path': 'activity.collectors.storage.StorageCollector'},
                        {'class': 'NtfsCollector', 'module': 'activity.collectors.storage.ntfs', 'path': 'activity.collectors.storage.ntfs.NtfsCollector'}
                    ],
                    'semantic': [
                        {'class': 'SemanticCollector', 'module': 'activity.collectors.semantic', 'path': 'activity.collectors.semantic.SemanticCollector'}
                    ]
                ...
                'recorders': {
                    ...
                    'storage': [
                        {'class': 'StorageRecorder', 'module': 'activity.recorders.storage', 'path': 'activity.recorders.storage.StorageRecorder'}
                    ]
                },
                'total_collectors': 4,
                'total_recorders': 2,
        ...
        def learn_from_query_results(self, query_text, query_results, execution_time, user_id=None):
        ...
        def process_user_feedback(self, feedback_type, feedback_data, query_id=None, pattern_id=None, user_id=None):
            # Record feedback
    ...
    print(f"Discovered {discovery_results['total_collectors']} collectors and "
    ...
    print(f"Analyzed {schema_analysis['collections_analyzed']} collections in {elapsed:.3f} seconds")
    ...
    if schema_analysis.get('field_usage_patterns'):
        ...
        for field, usage in list(schema_analysis['field_usage_patterns'].items())[:3]:
            print(f"- {field}: {usage['percentage']:.1f}% usage, type: {usage['field_type']}")
    ...
    sample_results = [
        {"_id": "Objects/123", "Label": "indaleko_report.pdf", "created": datetime.now(timezone.utc).isoformat(), 
         "content_type": "application/pdf", "size": 1234567},
        {"_id": "Objects/456", "Label": "indaleko_architecture.pdf", "created": datetime.now(timezone.utc).isoformat(), 
         "content_type": "application/pdf", "size": 2345678},
        {"_id": "Objects/789", "Label": "indaleko_roadmap.pdf", "created": datetime.now(timezone.utc).isoformat(), 
         "content_type": "application/pdf", "size": 3456789}
    ]
    ...
    print(f"- Total collectors: {change_results['total_collectors']} (previous: {change_results['previous_collector_count']})")
    
    if change_results.get('new_collector_types'):
        print(f"- New collector types: {', '.join(change_results['new_collector_types'])}")
    
    if change_results.get('removed_collector_types'):
        print(f"- Removed collector types: {', '.join(change_results['removed_collector_types'])}")
...
def main():
    ...
    parser.add_argument('--demo', action='store_true', help='Run a demonstration of the system with mocked objects')
    parser.add_argument('--run-tests', action='store_true', help='Run unit tests (currently also runs the demo due to missing module)')
    ...
    if args.run_tests:
        ...
        print("\nExample usage:")
        print("  python test_continuous_learning.py --demo       # Run the demonstration")
        print("  python test_continuous_learning.py --run-tests  # Run the unit tests")
        print("  python test_continuous_learning.py --verbose --demo  # Run with verbose logging")
