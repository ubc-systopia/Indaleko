...
from kb_integration import ArchivistKnowledgeIntegration
...
class KnowledgeBaseCliIntegration:
    ...
    def __init__(self, cli_instance: IndalekoBaseCLI, 
                kb_integration: Optional[ArchivistKnowledgeIntegration] = None):
        """
        ...
        if not args:
            ...
            if query_patterns:
                ...
                for pattern in sorted(query_patterns, key=lambda p: p.confidence, reverse=True):
                    ...
                    query = pattern.pattern_data.get("query_text", "")[:40] + "..." if len(pattern.pattern_data.get("query_text", "")) > 40 else pattern.pattern_data.get("query_text", "")
            ...
            if entity_patterns:
                ...
                for pattern in sorted(entity_patterns, key=lambda p: p.confidence, reverse=True):
            ...
            if schema_patterns:
                ...
                for pattern in sorted(schema_patterns, key=lambda p: p.confidence, reverse=True):
            ...
            if preference_patterns:
                ...
                for pattern in sorted(preference_patterns, key=lambda p: p.confidence, reverse=True):
            ...
            try:
                ...
                pattern = self.kb_integration.kb_manager.get_knowledge_pattern(pattern_id)
    ...
    def handle_entities_command(self, args: str) -> str:
        ...
        if not args:
            ...
            # Get all nodes
            all_nodes = list(self.kb_integration.entity_equivalence._nodes_cache.values())
            ...
            for node in matching_nodes:
                ...
                canonical = self.kb_integration.entity_equivalence.get_canonical_reference(node.entity_id)
                ...
                # Get group members
                all_refs = self.kb_integration.entity_equivalence.get_all_references(node.entity_id)
                if len(all_refs) > 1:
...
def register_kb_integration(cli_instance: IndalekoBaseCLI) -> KnowledgeBaseCliIntegration:
    """
