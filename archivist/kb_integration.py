...
class ArchivistKnowledgeIntegration:
    ...
    def _process_entities(self, entities: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for entity in entities:
            ...
            canonical = self.entity_equivalence.get_canonical_reference(
                entity_node.entity_id,
            )

            # Get all equivalent references
            all_references = self.entity_equivalence.get_all_references(
                entity_node.entity_id,
            )
    ...
    def _record_query_results(
        ...
        if result_info.get("refinements"):
        ...
        if collections and "first_result" in result_info and result_info["first_result"]:
            try:
                ...
                self.logger.warning(f"Error checking for schema changes: {e!s}")
