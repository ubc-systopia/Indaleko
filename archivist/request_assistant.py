...
from query.tools.base import ToolInput, ToolOutput, BaseTool
...
class RequestAssistant:
    ...
    def __init__(
        ...
        archivist_memory: Optional[ArchivistMemory] = None,
    ...
    def _initialize_assistant(self) -> None:
        ...
        config_dir = os.path.join(os.environ.get("INDALEKO_ROOT"), "config")
        assistant_config_file = os.path.join(config_dir, "request-assistant-config.json")
    ...
    def _build_assistant_instructions(self) -> str:
        ...
        try:
            ...
            collection_names = self.db_config.db.collections()
            ...
            primary_collections = [
                "Objects", "Relationships", "ActivityContext", 
                "MachineConfig", "Users", "NamedEntities"
            ]
            ...
            if self._calculate_token_estimate(instructions) < 10000:  # Safe limit for instructions
                ...
                for name in collection_names:
                    if name not in primary_collections:
                        ...
                        if self._calculate_token_estimate(instructions + collection_summary) < 15000:
    ...
    def _get_collection_schema_summary(self, collection_name: str) -> str:
        ...
        try:
            ...
            try:
                ...
                summary += f"- Schema: Unable to retrieve schema\n"
            ...
            summary += f"Error retrieving collection information: {str(e)}\n"
    ...
    def _summarize_schema(self, schema: Dict[str, Any], max_tokens: int = 1000) -> Dict[str, Any]:
        """
        ...
        if "properties" in schema:
            ...
            important_fields = ["id", "name", "type", "key", "label", "title", "description"]
            sorted_props = sorted(
            ...
            for prop_name, prop_value in sorted_props:
                ...
                if isinstance(prop_value, dict):
                    simplified_prop = {
                        ...
                        "description": prop_value.get("description", f"Property {prop_name}")
                    }
                    ...
                    prop_tokens = self._calculate_token_estimate(json.dumps(simplified_prop))
                    
                    if current_estimate + prop_tokens > max_tokens:
                        summary["properties"][prop_name] = {"type": "omitted", "description": "Property details omitted to reduce token usage"}
                    else:
    ...
    def execute_tool(self, tool_call: Dict[str, Any], conversation_id: str) -> Dict[str, Any]:
        ...
        if function_name == "query_executor" and self.enable_query_history:
            ...
            if query and not explain_only and not force_execute:
                ...
                if cached_result:
                    ...
                    setattr(result, "execution_time", 0.0)
                    setattr(result, "from_cache", True)
                    
                    if self.progress_callback:
                        self.progress_callback(f"Retrieved from cache: {function_name}", 1.0)
        ...
        if result.success:
            ...
            output = {
                "output": json.dumps(output_result, default=str)
            }
        else:
            output = {
                "error": result.error,
        ...
        if "results" in result:
            ...
            if isinstance(results, list):
                ...
                if original_count > 50:
                    ...
                    processed_result["message"] = f"Response truncated to 50 of {original_count} results to manage token usage"
                else:
    ...
    def _process_nl_parser_results(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        essential_fields = [
            "intent", "entities", "collections", "query", "is_successful"
        ]
    ...
    def _process_dict_results(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        important_fields = [
            "id", "name", "key", "title", "description", "type", "query", 
        ...
        current_estimate = self._calculate_token_estimate(json.dumps(processed_result, default=str))
    ...
    def update_archivist_memory(self, conversation: ConversationState) -> None:
        ...
        self.archivist_memory.memory.continuation_context = (
            f"Recent conversation topics: {', '.join([m.content[:50] + '...' if len(m.content) > 50 else m.content for m in user_messages[-2:]])}"
        )
    ...
    def _extract_and_store_named_entities(self, text: str) -> None:
        ...
        try:
            ...
            response = client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are a precise entity extraction system. Extract entities exactly as they appear."},
    ...
    def _store_named_entities(self, entities: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            if not self.db_config.db.has_collection(collection_name):
                ic(f"Collection {collection_name} does not exist, skipping entity storage")
                return
            ...
            for entity_data in entities:
                ...
                if entity_type not in ["person", "location", "organization", "device", "event", "document"]:
                    ...
                    if entity_type in ["company", "business", "school", "university"]:
                        ...
                        entity_type = "device"
                    elif entity_type in ["file", "spreadsheet", "presentation", "paper"]:
                ...
                entity_model = IndalekoNamedEntityDataModel(
                    Record=IndalekoRecordDataModel(
                        SourceIdentifier=IndalekoSourceIdentifierDataModel(
                            ...
                            Version="1.0",
                            Description=f"Named Entity from Archivist Conversation"
                        ),
                        Timestamp=datetime.now(timezone.utc)
                    ),
                    name=entity_text,
                    entity_type=entity_type,
                    description=f"Entity extracted from user conversation",
    ...
    def _wait_for_run(self, thread_id: str, run_id: str, conversation_id: str, auto_refresh: bool = True) -> Dict[str, Any]:
        """
        ...
        while True:
            ...
            if run.status == "completed":
                ...
                is_token_limit_error = (
                    error_code in ["context_length_exceeded", "max_tokens_exceeded"] or 
    ...
    def _record_database_interaction(self, tool_input: Any, result: Any) -> None:
        ...
        try:
            ...
            try:
                ...
                from query.query_processing.query_history import IndalekoQueryHistoryDataModel
                from query.query_processing.data_models.query_output import QueryOutputModel
                from data_models.record import IndalekoRecordDataModel
                from data_models.source_identifier import IndalekoSourceIdentifierDataModel
                    ...
                    if execution_plan and len(json.dumps(execution_plan)) > 100000:  # ~100KB limit
                        if isinstance(execution_plan, dict):
                            ...
                            simplified_plan = {
                                ...
                                "estimatedNrItems": execution_plan.get("estimatedNrItems"),
                ...
                if self.db_config.db.has_collection(collection_name):
                    ...
                    if truncated_results:
                        ic(f"Recorded truncated query to history: {query[:50]} (original size: {result_size/1024/1024:.2f}MB)")
                    else:
    ...
    def _check_query_cache(self, query: str, bind_vars: Dict[str, Any]) -> Optional[Any]:
        """
    ...
    def _create_conversation_summary(self, conversation: ConversationState) -> str:
        ...
        if user_msgs:
            ...
            common_words = ["the", "a", "an", "and", "or", "but", "in", "on", "at", "to", "for", "with", 
                           ...
                           "may", "might", "must", "that", "this", "these", "those", "it", "they", 
                           "them", "their", "his", "her", "he", "she", "I", "me", "my", "we", "us", "our"]
