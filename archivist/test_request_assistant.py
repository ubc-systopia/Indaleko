...
def run_basic_test(model: str = "gpt-4o", debug: bool = False) -> None:
    ...
    print(f"{Fore.GREEN}Assistant initialized in {time.time() - start_time:.2f}s{Style.RESET_ALL}")
...
def run_conversation_test(model: str = "gpt-4o", interactive: bool = False) -> None:
    ...
    filepath = os.path.join("conversations", f"test_conversation_{int(time.time())}.json")
    ...
    for content_type, preference in sorted(preferences.items(), key=lambda x: x[1], reverse=True):
...
def run_tool_usage_test(model: str = "gpt-4o") -> None:
    ...
    print(f"\n{Fore.BLUE}User: {query}{Style.RESET_ALL}")
    print(f"{Fore.YELLOW}Processing (this may take a while due to tool execution)...{Style.RESET_ALL}")
...
def run_context_management_test(model: str = "gpt-4o") -> None:
    ...
    print(f"Reduction: {(1 - len(json.dumps(summarized)) / len(json.dumps(complex_schema))) * 100:.2f}%")
...
def main():
    ...
    parser.add_argument("--interactive", action="store_true", help="Run in interactive mode for conversation test")
    parser.add_argument("--test", choices=["basic", "conversation", "tools", "context", "all"], default="basic",
