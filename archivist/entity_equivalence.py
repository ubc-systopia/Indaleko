...
class EntityEquivalenceManager:
    ...
    def _setup_collections(self):
        ...
        self.nodes_collection = nodes_collection.collection
        self.relations_collection = relations_collection.collection
        self.groups_collection = groups_collection.collection
    ...
    def add_entity_reference(self, 
                           name: str, 
                           entity_type: IndalekoNamedEntityType,
                           canonical: bool = False,
                           source: Optional[str] = None,
                           context: Optional[str] = None) -> EntityEquivalenceNode:
        """
    ...
    def _find_potential_matches(self, node: EntityEquivalenceNode, 
                              similarity_threshold: float = 0.85) -> List[Tuple[UUID, float]]:
        """
    ...
    def _suggest_relation(self, source_id: UUID, target_id: UUID, confidence: float) -> None:
        """
    ...
    def add_relation(self, 
                    ...
                    evidence: Optional[str] = None) -> EntityEquivalenceRelation:
        """
    ...
    def get_canonical_reference(self, entity_id: UUID) -> Optional[EntityEquivalenceNode]:
        """
...
def main():
    ...
    manager.merge_entities(node3.entity_id, node1.entity_id, relation_type="professional")
    ...
    print(f"Canonical reference for '{node2.name}': {canonical.name if canonical else 'None'}")
