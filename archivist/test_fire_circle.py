...
def test_create_entity(ui: TestInterface) -> None:
    ...
    try:
        ...
        ui.display_error(f"Failed to create entities: {str(e)}")
...
def test_process_message(ui: TestInterface) -> None:
    ...
    try:
        ...
        ui.display_success("Successfully processed messages with different perspectives")
    except Exception as e:
        ui.display_error(f"Failed to process messages: {str(e)}")
...
def test_orchestrator(ui: TestInterface) -> None:
    ...
    try:
        ...
        for role, perspective in result["perspectives"].items():
            ui.display_info(f"{role.capitalize()} ({len(perspective)} chars): {perspective[:100]}...")
        
        if result.get("synthesis"):
            ui.display_info(f"Synthesis ({len(result['synthesis'])} chars): {result['synthesis'][:100]}...")
        
        ui.display_success("Successfully orchestrated multi-perspective analysis")
    except Exception as e:
        ui.display_error(f"Failed to test orchestrator: {str(e)}")
...
def test_archivist_integration(ui: TestInterface) -> None:
    ...
    try:
        ...
        ui.display_error(f"Failed to test Archivist integration: {str(e)}")
...
def test_pattern_suggestions(ui: TestInterface) -> None:
    ...
    try:
        ...
        ui.display_error(f"Failed to test pattern suggestions: {str(e)}")
...
def main() -> None:
    ...
    parser.add_argument("--messages", action="store_true", help="Test message processing")
    parser.add_argument("--orchestrator", action="store_true", help="Test orchestrator")
    parser.add_argument("--integration", action="store_true", help="Test Archivist integration")
    parser.add_argument("--suggestions", action="store_true", help="Test pattern suggestions")
    ...
    if not HAS_FIRE_CIRCLE:
        ui.display_error("Fire Circle is not available. Please check your installation.")
        return
    ...
    run_all = args.all or not any([
        args.entities, args.messages, args.orchestrator,
        args.integration, args.suggestions
    ])
