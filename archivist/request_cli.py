...
from archivist.request_assistant import RequestAssistant, ConversationState
...
class RequestArchivistCLI(IndalekoBaseCLI):
    ...
    def __init__(self, model: str = "gpt-4o", debug: bool = False, batch_mode: bool = False):
        """
...
"""
    ...
    def initialize(self) -> None:
        ...
        print(f"{Fore.GREEN}Archivist initialized. Type /help for available commands.{Style.RESET_ALL}")
        print(f"{Fore.GREEN}Starting new conversation: {self.conversation_id}{Style.RESET_ALL}")
        print()
    ...
    def run_batch(self, input_file: str) -> None:
        ...
        try:
            ...
            print(f"{Fore.GREEN}Processing {len(queries)} queries from {input_file}{Style.RESET_ALL}")
    ...
    def clear_conversation(self, args: str) -> None:
        ...
        print(f"{Fore.GREEN}Started new conversation: {self.conversation_id}{Style.RESET_ALL}")
    ...
    def load_conversation(self, args: str) -> None:
        ...
        try:
            ...
            print(f"{Fore.GREEN}Using conversation: {self.conversation_id}{Style.RESET_ALL}")
            ...
            if conversation and conversation.messages:
                ...
                for i, msg in enumerate(conversation.messages[-3:]):
                    ...
                    print(f"{role_color}{msg.role.capitalize()}:{Style.RESET_ALL} {msg.content[:100]}...")
    ...
    def dump_conversation(self, args: str) -> None:
        ...
        if args == "full":
            ...
            for i, msg in enumerate(conversation.messages):
                role_color = Fore.BLUE if msg.role == "user" else (
                    Fore.GREEN if msg.role == "assistant" else Fore.YELLOW
                )
...
"""
        ...
        if subcommand == "list":
            ...
            if len(parts) < 2:
                print(f"{Fore.RED}Error: Invalid format. Use TYPE:NAME[:DESCRIPTION]{Style.RESET_ALL}")
                return
            ...
            valid_types = ["person", "location", "organization", "device", "event", "document"]
            if entity_type not in valid_types:
                print(f"{Fore.RED}Error: Invalid entity type. Valid types: {', '.join(valid_types)}{Style.RESET_ALL}")
                return
    ...
    def _list_entities(self, entity_type: Optional[str] = None) -> None:
        """
        ...
        try:
            ...
            # Execute the query
            cursor = self.assistant.db_config.db.aql.execute(aql_query, bind_vars=bind_vars)
            ...
            if entity_type:
                print(f"{Fore.CYAN}{len(entities)} {entity_type.title()} Entities:{Style.RESET_ALL}")
                for entity in entities:
                    # Extract key if present
                    entity_id = entity.get("_key") or entity.get("id", "unknown")
                    confidence = entity.get("confidence", 1.0)
                    print(f"  [{entity_id}] {entity['name']} (confidence: {confidence:.2f})")
    ...
    def _add_entity(self, entity_type: str, name: str, description: str) -> None:
        ...
        try:
            ...
            entity_model = IndalekoNamedEntityDataModel(
                Record=IndalekoRecordDataModel(
                    SourceIdentifier=IndalekoSourceIdentifierDataModel(
                        ...
                        Version="1.0",
                        Description=f"User-added Named Entity"
                    ),
            ...
            cursor = self.assistant.db_config.db.aql.execute(
                aql_query,
                bind_vars={
                    "name": name,
                    "type": entity_type
            ...
            if existing:
                print(f"{Fore.YELLOW}Entity already exists: {name} ({entity_type}){Style.RESET_ALL}")
                return
                
            # Save to database
            collection = self.assistant.db_config.db.collection("NamedEntities")
            doc = json.loads(entity_model.model_dump_json())
            result = collection.insert(doc)
            
            print(f"{Fore.GREEN}Added new entity: {name} ({entity_type}){Style.RESET_ALL}")
    ...
    def _delete_entity(self, entity_id: str) -> None:
        ...
        try:
            ...
            try:
                ...
                if not entity:
                    print(f"{Fore.RED}Entity not found with ID: {entity_id}{Style.RESET_ALL}")
                    return
                    
                # Confirm deletion
                entity_name = entity.get("name", "Unknown")
                entity_type = entity.get("entity_type", "unknown")
                print(f"{Fore.YELLOW}Deleting: {entity_name} ({entity_type}){Style.RESET_ALL}")
    ...
    def _search_entities(self, search_term: str) -> None:
        ...
        try:
            ...
            if not entities:
                print(f"{Fore.YELLOW}No entities found matching: '{search_term}'{Style.RESET_ALL}")
                return
                
            # Show results
            print(f"{Fore.CYAN}Found {len(entities)} entities matching '{search_term}':{Style.RESET_ALL}")
        ...
        if args:
            ...
            if subcommand == "save":
                ...
                if not subargs:
                    ...
                    print(f"Usage: /memory add-goal NAME:DESCRIPTION")
                    return
                    
                if ":" not in subargs:
                    print(f"{Fore.RED}Error: Invalid format. Use NAME:DESCRIPTION{Style.RESET_ALL}")
                    return
                    
                name, description = subargs.split(":", 1)
                self.assistant.archivist_memory.add_long_term_goal(name.strip(), description.strip())
                self.assistant.archivist_memory.save_memory()
                ...
                if not insights:
                    ...
                    for i, insight in enumerate(insights):
                        print(f"  {i+1}. {insight.insight} (confidence: {insight.confidence:.2f})")
                ...
                if not strategies:
                    ...
                    for i, strategy in enumerate(strategies):
                        print(f"  {i+1}. {strategy.strategy_name} (success rate: {strategy.success_rate:.2f})")
                ...
                print(f"{Fore.RED}Unknown memory subcommand: {subcommand}{Style.RESET_ALL}")
...
def main():
    ...
    parser.add_argument("--batch", help="Run in batch mode with the specified input file")
