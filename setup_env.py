...
def install_uv() -> None:
    ...
    if system.startswith('win'):
        return install_uv_windows()
    elif system.startswith('linux'):
        return install_uv_linux()
    elif system.startswith('darwin'):
        return install_uv_darwin()
    else:
...
def compute_venv_name(platform : str = None, python_version : tuple = None) -> str:
    '''
    ...
    return f".venv-{platform}-python{python_version[0]}.{python_version[1]}"


def check_or_create_virtualenv(platform : str = None,
    ...
    if sys.platform.startswith('win'):
        venv_path = os.path.join(os.getcwd(), 'Scripts', 'activate')
        print(f"Virtual environment created. Activate it with '{venv_path}'.")
    else:
        venv_path = os.path.join(os.getcwd(), 'bin', 'activate')
        print(f"Virtual environment created. Activate it with 'source {venv_path}'.")
...
def find_best_lockfile() -> Union[str, None]:
    '''
...
def main():
    ...
    default_lock_file = find_best_lockfile()
