...
class GoogleCalendarCollector(CalendarCollectorBase):
        ...
        if not creds or not creds.valid:
            ...
            if not creds:
                if not os.path.exists(self.config_path):
                    self.logger.error(f"Google Calendar API config file not found: {self.config_path}")
                    self.logger.error("Please download OAuth 2.0 Client ID credentials from Google Cloud Console")
                    self.logger.error("and save them as 'gcalendar_config.json' in the config directory")
                    return None
                
                try:
                    # Run the OAuth flow
                    flow = InstalledAppFlow.from_client_secrets_file(self.config_path, self.SCOPES)
    ...
    def convert_to_calendar_event(self, event_data: Dict[str, Any], calendar_id: str) -> CalendarEvent:
        ...
        try:
            ...
            status = status_map.get(event_data.get('status', 'confirmed'), EventStatus.UNKNOWN)
            ...
            if is_recurring:
                ...
                for rule in recurrence_rules:
                    if rule.startswith('RRULE:'):
                        ...
                        recurrence_type = recurrence_type_map.get(freq, EventRecurrence.CUSTOM)
            ...
            for attendee_data in event_data.get('attendees', []):
                ...
                response = response_map.get(response_status, EventResponse.NOT_RESPONDED)
            ...
            if not is_organizer and self.user_email:
                for attendee_data in event_data.get('attendees', []):
                    if attendee_data.get('email') == self.user_email:
                        ...
                        user_response = response_map.get(response_status, EventResponse.NOT_RESPONDED)
                        break
