...
from activity.collectors.collaboration.data_models.calendar_event import (
    CalendarEvent, EventAttendee, EventLocation, RecurrencePattern, 
    EventAttachment, EventStatus, EventResponse, EventRecurrence
)
...
class OutlookCalendarCollector(CalendarCollectorBase):
    ...
    def __init__(self, **kwargs):
        ...
        # Configuration paths
        self.config_path = kwargs.get("config_path", os.path.join(
            os.environ.get("INDALEKO_ROOT"), "config", "outlook_calendar_config.json"
        ))
        self.token_path = kwargs.get("token_path", os.path.join(
        ...
        self.client_secret = kwargs.get("client_secret", self.config.get("client_secret"))
        self.tenant_id = kwargs.get("tenant_id", self.config.get("tenant_id", "common"))
        self.redirect_uri = kwargs.get("redirect_uri", self.config.get("redirect_uri", "http://localhost:8000"))
    ...
    def authenticate(self) -> bool:
        ...
        try:
            ...
            if not self.client_id or not self.client_secret:
                ...
                self.logger.error("Please provide client_id and client_secret or configure outlook_calendar_config.json")
                return False
    ...
    def get_calendars(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code != 200:
                self.logger.error(f"Error retrieving calendars: {response.status_code} - {response.text}")
                return []
    ...
    def get_events(
        ...
        try:
            ...
            # Add select fields
            params['$select'] = 'id,subject,bodyPreview,body,importance,sensitivity,start,end,location,' \
            ...
            if response.status_code != 200:
                self.logger.error(f"Error retrieving events: {response.status_code} - {response.text}")
                return []
            ...
            for event in events_data:
                if event.get('hasAttachments', False):
                    ...
                    if attachments_response.status_code == 200:
                        event['attachments'] = attachments_response.json().get('value', [])
                    else:
                        self.logger.warning(f"Failed to get attachments for event {event_id}")
                        event['attachments'] = []
                else:
                    event['attachments'] = []
    ...
    def get_event_details(self, calendar_id: str, event_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code != 200:
                self.logger.error(f"Error retrieving event details: {response.status_code} - {response.text}")
                return {}
            ...
            if event_data.get('hasAttachments', False):
                ...
                if attachments_response.status_code == 200:
                    event_data['attachments'] = attachments_response.json().get('value', [])
                else:
                    self.logger.warning(f"Failed to get attachments for event {event_id}")
    ...
    def convert_to_calendar_event(self, event_data: Dict[str, Any], calendar_id: str) -> CalendarEvent:
        ...
        try:
            ...
            recurrence_data = event_data.get('recurrence', None)
            ...
            if is_recurring:
                ...
                recurrence_type = recurrence_type_map.get(type_str, EventRecurrence.CUSTOM)
            ...
            for attachment_data in event_data.get('attachments', []):
                ...
                if attachment_data.get('@odata.type', '') == '#microsoft.graph.referenceAttachment':
                    uri = attachment_data.get('referenceAttachmentLastModifiedDateTime', '')
            ...
            is_organizer = False
            if self.user_info:
                user_email = self.user_info.get('mail') or self.user_info.get('userPrincipalName')
            ...
            if not is_organizer and self.user_info:
                ...
                for attendee_data in event_data.get('attendees', []):
                    ...
                    if email_data.get('address') == user_email:
                        ...
                        user_response = response_map.get(status_str, EventResponse.NOT_RESPONDED)
                        break
