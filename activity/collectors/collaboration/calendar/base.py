...
class CalendarCollectorBase(CollaborationCollector):
    ...
    def collect_data(self, **kwargs) -> None:
        ...
        for calendar in calendars:
            ...
            self.logger.info(f"Collecting events from calendar: {calendar_name} ({calendar_id})")
        ...
        self.logger.info(f"Collected {total_events} events from {len(calendars)} calendars")
    ...
    def set_cursor(self, cursor: str) -> None:
        ...
        if cursor:
            try:
                ...
                if last_sync_time:
                    self.last_sync_time = datetime.datetime.fromisoformat(last_sync_time)
            except Exception as e:
