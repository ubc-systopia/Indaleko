...
from activity.collectors.collaboration.discord.data_models.file_share_data_model import DiscordDataModel
...
class DiscordFileShareCollector(CollaborationCollector):
    ...
    def __init__(self, token_file: Optional[str] = None, token: Optional[str] = None, **kwargs):
        """
    ...
    def get_channel_messages(self, channel_id: str, limit: int = 100) -> List[Dict]:
        ...
        try:
            ...
            self.logger.error(f"Failed to retrieve messages for channel {channel_id}: {e}")
            return []
    
    def extract_file_attachments(self, messages: List[Dict], channel_data: Dict) -> List[Dict]:
        """
    ...
    def scan_dm_channels(self) -> List[Dict]:
        ...
        for channel in self.dm_channels:
            ...
            self.logger.info(f"Found {len(attachments)} attachments in DM with {recipient}")
    ...
    def scan_guild_channels(self) -> List[Dict]:
        ...
        for guild in guilds:
            ...
            for channel in channels:
                ...
                self.logger.info(f"Found {len(attachments)} attachments in #{channel_name}")
    ...
    def collect_file_attachments(self) -> List[Dict]:
        ...
        self.logger.info(f"Collected a total of {len(self.file_attachments)} file attachments")
    ...
    def get_all_processed_data(self) -> List[Dict]:
        ...
        """
        return [self.convert_attachment_to_model(attachment).model_dump() 
