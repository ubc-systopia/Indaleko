...
class EventLocation(IndalekoBaseModel):
    ...
    coordinates: Optional[Dict[str, float]] = None  # {"latitude": 42.3, "longitude": -71.1}
    is_virtual: bool = False
    join_url: Optional[str] = None  # For virtual meetings (Teams, Zoom, etc.)
...
class RecurrencePattern(IndalekoBaseModel):
    ...
    day_of_week: Optional[List[int]] = None  # 0=Sunday, 1=Monday, etc.
...
class EventAttachment(IndalekoBaseModel):
    ...
    file_id: Optional[str] = None  # Provider-specific identifier
...
class CalendarEvent(BaseCollaborationDataModel):
    ...
    sensitivity: Optional[str] = None  # "normal", "personal", "private", "confidential"
    importance: Optional[str] = None  # "low", "normal", "high"
    
    # Recurrence
    is_recurring: bool = False
    recurrence: Optional[RecurrencePattern] = None
    series_master_id: Optional[str] = None  # If this is a recurring instance, the ID of the master
    instance_index: Optional[int] = None  # For recurring events, which occurrence this is
    ...
    online_meeting_provider: Optional[str] = None  # "teams", "zoom", "webex", etc.
    join_url: Optional[str] = None  # URL to join the meeting
    ...
    def __init__(self, **data):
        ...
        if self.last_modified_time.tzinfo is None:
            self.last_modified_time = self.last_modified_time.replace(tzinfo=timezone.utc)
