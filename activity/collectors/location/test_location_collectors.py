...
class TestIPLocation(unittest.TestCase):
    ...
    def test_initialization(self):
        loc = IPLocation()
        assert loc.ip_address == "123.123.123.123"
        assert isinstance(loc.location_data, IPLocationDataModel)
        # Coordinates should be nested under Location attribute
        assert loc.location_data.Location.latitude == 10.0
        assert loc.location_data.Location.longitude == 20.0
    ...
    def test_collect_data(self):
        ...
        if hasattr(loc, "data"):
            loc.data.clear()
        loc.collect_data()
        assert hasattr(loc, "data")
        assert len(loc.data) > 0
        record = loc.data[-1]
        assert isinstance(record, dict)
        # Check nested Location in the record
        loc_data = record.get("Location", {})
        assert loc_data.get("latitude") == 10.0
        assert loc_data.get("longitude") == 20.0
        assert loc_data.get("source") == "IP"
...
class TestWiFiLocation(unittest.TestCase):
    ...
    def test_collect_data(self):
        ...
        assert loc_data.get("latitude") == 30.0
        assert loc_data.get("longitude") == 40.0
        assert loc_data.get("accuracy") == 30.0
        assert loc_data.get("source") == "WiFi"
    ...
    def test_get_coordinates(self):
        ...
        assert coords.get("latitude") == 30.0
        assert coords.get("longitude") == 40.0
