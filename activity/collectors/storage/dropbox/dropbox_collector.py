...
class DropboxStorageActivityCollector(StorageActivityCollector):
    ...
    def __init__(self, **kwargs):
        ...
        self._dropbox_config_file = str(Path(self._config_dir) / self.DROPBOX_CONFIG_FILE)
    ...
    def _process_dropbox_entry(self, entry):
        ...
        try:
            ...
            is_directory = False
            if hasattr(entry, "FolderMetadata") or isinstance(entry, dropbox.files.FolderMetadata):
                is_directory = True
            ...
            activity_data = DropboxStorageActivityData(
                ...
                created_time=self._parse_datetime(getattr(entry, "client_modified", None)) if hasattr(entry, "client_modified") else None,
                modified_time=self._parse_datetime(getattr(entry, "server_modified", None)) if hasattr(entry, "server_modified") else None
            )
    ...
    def _determine_activity_type(self, entry) -> Optional[StorageActivityType]:
        """
        ...
        if hasattr(entry, "DeletedMetadata") or isinstance(entry, dropbox.files.DeletedMetadata):
    ...
    def _load_dropbox_config(self) -> None:
        ...
        if not os.path.exists(self._dropbox_config_file):
            self._logger.warning(f"Config file {self._dropbox_config_file} does not exist")
            return
        
        try:
            with open(self._dropbox_config_file, "rt", encoding="utf-8") as f:
                self._dropbox_config = json.load(f)
            
            # Validate config
            if "app_key" not in self._dropbox_config or "app_secret" not in self._dropbox_config:
                self._logger.warning("Invalid Dropbox config file: missing app_key or app_secret")
                self._dropbox_config = None
        except Exception as e:
    ...
    def _load_dropbox_credentials(self) -> None:
        ...
        try:
            if not os.path.exists(self._dropbox_token_file):
                self._logger.warning(f"Token file {self._dropbox_token_file} does not exist")
                self._dropbox_credentials = None
                return
                
            with open(self._dropbox_token_file, "rt", encoding="utf-8-sig") as f:
    ...
    def _store_dropbox_credentials(self) -> None:
        ...
        try:
            ...
            with open(self._dropbox_token_file, "wt", encoding="utf-8-sig") as f:
    ...
    def _refresh_access_token(self) -> None:
        ...
        try:
            ...
            if "access_token" in response_data and "expires_in" in response_data:
                ...
                self._logger.error(f"Invalid response during token refresh: {response_data}")
                
        except Exception as e:
            self._logger.error(f"Error refreshing Dropbox access token: {e}")
    
    def _parse_datetime(self, dt_str) -> Optional[datetime.datetime]:
        """
        ...
        try:
            if isinstance(dt_str, datetime.datetime):
                ...
                if dt_str.tzinfo is None:
                    return dt_str.replace(tzinfo=datetime.timezone.utc)
                return dt_str
...
if __name__ == "__main__":
    ...
    try:
        import time
        print(f"Monitoring Dropbox activities for {collector._monitor_interval * 5} seconds...")
