...
class TestGoogleDriveCollector(unittest.TestCase):
    ...
    @patch('activity.collectors.storage.cloud.gdrive_activity_collector.GoogleDriveActivityCollector._authenticate')
    @patch('activity.collectors.storage.cloud.gdrive_activity_collector.GoogleDriveActivityCollector._init_apis')
    @patch('activity.collectors.storage.cloud.gdrive_activity_collector.GoogleDriveActivityCollector._load_config')
    ...
    def test_get_collector_name(self):
        ...
        self.assertEqual(self.collector.get_collector_name(), "Google Drive Activity Collector")
    
    def test_get_provider_id(self):
        """Test get_provider_id method."""
        self.assertEqual(self.collector.get_provider_id(), uuid.UUID("3e7d8f29-7c73-41c5-b3d4-1a9b42567890"))
...
@unittest.skipIf("GoogleDriveActivityRecorder" not in globals(), "Recorder class not available")
class TestGoogleDriveRecorder(unittest.TestCase):
    ...
    @patch('activity.collectors.storage.cloud.gdrive_activity_collector.GoogleDriveActivityCollector._init_apis')
    @patch('activity.collectors.storage.cloud.gdrive_activity_collector.GoogleDriveActivityCollector._load_config')
    @patch('activity.recorders.storage.cloud.gdrive.recorder.GoogleDriveActivityRecorder._connect_to_db')
    @patch('activity.recorders.storage.cloud.gdrive.recorder.GoogleDriveActivityRecorder._register_with_service_manager')
    def setUp(self, mock_register, mock_connect, mock_load_config, mock_init_apis, mock_authenticate):
        """Set up test environment."""
        # Mock authentication and API initialization
        mock_authenticate.return_value = None
        mock_init_apis.return_value = None
    ...
    @patch('activity.recorders.storage.cloud.gdrive.recorder.get_semantic_attributes_for_activity')
    ...
    @patch('activity.recorders.storage.cloud.gdrive.recorder.GoogleDriveActivityRecorder._build_gdrive_activity_document')
