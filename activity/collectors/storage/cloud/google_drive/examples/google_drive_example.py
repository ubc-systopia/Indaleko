...
try:
    ...
    from activity.recorders.storage.cloud.google_drive.recorder import GoogleDriveActivityRecorder
...
def display_file_details(file_info):
    ...
    if file_info.created_time:
        try:
            created_dt = datetime.fromisoformat(file_info.created_time.replace('Z', '+00:00'))
    ...
    if file_info.modified_time:
        try:
            modified_dt = datetime.fromisoformat(file_info.modified_time.replace('Z', '+00:00'))
    ...
    if detailed:
        ...
        if activity_type == "RENAME":
            ...
            if activity.shared_with:
                shared_with = ", ".join([f"{u.display_name or u.email or u.user_id}" for u in activity.shared_with])
...
def main():
    ...
    parser.add_argument("--days", type=int, default=7, help="Number of days of history to collect")
    parser.add_argument("--limit", type=int, default=100, help="Maximum number of activities to display")
    parser.add_argument("--detailed", action="store_true", help="Show detailed activity information")
    parser.add_argument("--debug", action="store_true", help="Enable debug logging")
    parser.add_argument("--test-oauth", action="store_true", help="Test only the OAuth flow")
    parser.add_argument("--reauth", action="store_true", help="Force re-authentication by removing the token file")
    ...
    if args.debug:
        ...
        logging.getLogger("activity.collectors.storage.cloud.oauth_utils").setLevel(logging.DEBUG)
    ...
    if args.test_oauth:
        ...
        try:
            ...
            credentials_file = args.credentials or os.path.join(default_config_dir, "gdrive_client_secrets.json")
            token_file = args.token or os.path.join(default_config_dir, "gdrive_token.json")
            ...
            if args.reauth and os.path.exists(token_file):
                print(f"Removing existing token file {token_file} to force re-authentication")
                try:
            ...
            try:
                ...
                print("  pip install --upgrade google-api-python-client google-auth-httplib2 google-auth-oauthlib requests")
                return
            ...
            if os.path.exists(token_file):
                print(f"Existing token file found. Will attempt to refresh if needed.")
            ...
            try:
                ...
                try:
                    ...
                    if detect_environment() == "WSL":
                        ...
                        print("Successfully obtained and saved credentials with manual approach")
                    else:
                        ...
                        print("Successfully obtained and saved credentials with patched validation")
            ...
            if credentials:
                ...
                try:
                    ...
                    if user_info:
                        ...
                        print("Could not retrieve user information, but authentication succeeded.")
                except Exception as e:
                ...
                try:
                    ...
                    if service:
                        print("✅ Successfully built Drive API service. Token is working correctly.")
                        
                        # Try a simple API call
                        try:
                            about = service.about().get(fields="user").execute()
                            print(f"API call successful! Connected to Google Drive as: {about['user'].get('emailAddress')}")
                        except Exception as e:
                            print(f"API call failed: {e}")
                    else:
                        print("⚠️ Could not build Drive API service. Token may have issues.")
                except Exception as e:
    ...
    try:
        ...
        print("1. Try running with the --test-oauth flag to debug authentication issues")
    ...
    try:
        ...
        sorted_activities = sorted(
            processed_activities,
            key=lambda a: a.timestamp if isinstance(a.timestamp, datetime) else datetime.fromisoformat(a.timestamp.replace('Z', '+00:00')),
            reverse=True
        )
        ...
        if args.output:
            ...
            try:
                ...
                print(f"Successfully saved {len(processed_activities)} activities to {args.output}")
            except Exception as e:
