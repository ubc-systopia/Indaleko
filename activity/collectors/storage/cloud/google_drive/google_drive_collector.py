...
class GoogleDriveActivityCollector(CollectorBase):
    ...
    def __init__(self, **kwargs):
        ...
        self.config_path = kwargs.get("config_path", os.path.join(self.config_dir, "gdrive_collector_config.json"))
        self.credentials_file = kwargs.get("credentials_file", os.path.join(self.config_dir, "gdrive_client_secrets.json"))
        self.token_file = kwargs.get("token_file", os.path.join(self.config_dir, "gdrive_token.json"))
        ...
        self.state_file = kwargs.get("state_file", os.path.join(data_dir, "gdrive_collector_state.json"))
        self.output_file = kwargs.get("output_file", os.path.join(data_dir, "gdrive_activities.jsonl"))
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        default_config = {
            ...
            "logging": {
                "log_file": os.path.join(os.environ.get("INDALEKO_ROOT", "."), "logs", "gdrive_collector.log"),
                "log_level": "INFO"
            }
        }
    ...
    def _get_file_info(self, target: Dict[str, Any]) -> Optional[GDriveFileInfo]:
        ...
        try:
            file_details = self.drive_service.files().get(
                fileId=file_id,
            ...
            if 'parents' in file_details and file_details['parents']:
    ...
    def _determine_activity_type(self, action_detail: Dict[str, Any]) -> GDriveActivityType:
    ...
    def _extract_activity_details(self, activity: Dict[str, Any]) -> Optional[GDriveActivityData]:
        ...
        try:
            ...
            for detail in action_details:
                ...
                if 'move' in detail and activity_type == GDriveActivityType.MOVE:
                    ...
                    if 'addedParents' in move_details and move_details['addedParents']:
                        ...
                        if 'driveItem' in parent:
                            ...
                            try:
                                ...
                                logger.debug(f"Error getting destination folder name: {e}")
                ...
                if 'permissionChange' in detail and activity_type == GDriveActivityType.SHARE:
                    ...
                    if 'addedPermissions' in perm_details:
                        ...
                        for perm in added_perms:
                            if 'user' in perm:
                                shared_user = self._get_user_info({'user': perm['user']})
    ...
    def _get_activities(self, start_time: Optional[str] = None, page_token: Optional[str] = None) -> Tuple[List[Dict[str, Any]], Optional[str]]:
        ...
        try:
            ...
            logger.debug(f"Received {len(activities)} activities from Drive Activity API")
    ...
    def store_data(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Stored {len(self.activities)} activities to {self.output_file}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Google Drive Activity Collector for Indaleko")
    ...
    parser.add_argument("--full", action="store_true", help="Perform full collection instead of incremental")
    parser.add_argument("--direct-to-db", action="store_true", help="Write directly to database")
