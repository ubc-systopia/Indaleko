...
def main():
    ...
    parser.add_argument("--poll-interval", type=int, default=60, help="How often to poll for changes (seconds)")
    parser.add_argument("--filter-types", type=str, default="", help="Comma-separated list of activity types to include")
    parser.add_argument("--shared-only", action="store_true", help="Only show activities for shared files")
    ...
    try:
        ...
        while time.time() < end_time:
            ...
            logger.info(f"Collected {activity_count} activities in {elapsed:.1f} seconds. {remaining:.1f} seconds remaining.")
        ...
        shared_count = len([a for a in activities if getattr(a, "shared_folder_id", None) is not None])
        logger.info(f"Shared file activities: {shared_count} ({shared_count/len(activities)*100:.1f}% if len(activities) > 0 else 0)%)")
