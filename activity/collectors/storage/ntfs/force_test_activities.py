...
def generate_test_files(volume: str, num_files: int = 3, verbose: bool = False) -> List[str]:
...
def map_usn_record_to_activity(usn_record: Dict[str, Any]) -> NtfsStorageActivityData:
    ...
    # Get timestamp
    timestamp = usn_record.get("timestamp_dt", datetime.now(timezone.utc))
    ...
    return NtfsStorageActivityData(
        ...
        parent_file_reference_number=usn_record.get("parent_file_reference_number", "0"),
...
def generate_mock_activities(num_activities: int = 5, verbose: bool = False) -> List[NtfsStorageActivityData]:
    ...
    file_types = [
        "document.docx", "spreadsheet.xlsx", "image.jpg",
        "presentation.pptx", "text.txt", "archive.zip"
    ]
...
def save_activities_to_file(activities: List[NtfsStorageActivityData], output_path: str) -> None:
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Force test activities for NTFS Activity Collector")
    parser.add_argument("--volume", type=str, default="C:", help="Volume to monitor (default: C:)")
    parser.add_argument("--num-files", type=int, default=3, help="Number of test files to create (default: 3)")
    parser.add_argument("--output", type=str, default="ntfs_activities.jsonl", help="Output file (default: ntfs_activities.jsonl)")
    parser.add_argument("--mock", action="store_true", help="Use mock activities instead of USN journal")
    parser.add_argument("--num-activities", type=int, default=5, help="Number of mock activities (default: 5)")
    ...
    try:
        ...
        if args.mock:
            ...
            # Create test files
            generated_files = generate_test_files(args.volume, args.num_files, args.verbose)
            ...
            journal_info, records = get_usn_journal_records(args.volume, None, args.verbose)
