...
def determine_activity_type(reason_flags: List[str]) -> str:
    ...
    try:
        ...
        elif ("EA_CHANGE" in reason_flags or 
              "BASIC_INFO_CHANGE" in reason_flags or
              "COMPRESSION_CHANGE" in reason_flags or
              "ENCRYPTION_CHANGE" in reason_flags):
            return StorageActivityType.ATTRIBUTE_CHANGE
        elif "CLOSE" in reason_flags:
            return StorageActivityType.CLOSE
        elif ("DATA_OVERWRITE" in reason_flags or
              "DATA_EXTEND" in reason_flags or
              "DATA_TRUNCATION" in reason_flags):
...
def convert_record_to_model(record: Dict[str, Any], volume: str, provider_id: str) -> NtfsStorageActivityData:
    """
...
def test_collector():
    ...
    try:
        model = convert_record_to_model(standard_record, "C:", NtfsUsnJournalCollector.DEFAULT_PROVIDER_ID)
    ...
    try:
        model = convert_record_to_model(rename_old_record, "C:", NtfsUsnJournalCollector.DEFAULT_PROVIDER_ID)
        ...
        assert "RENAME_OLD_NAME" in model.attributes.get("usn_reason_flags", []), "Missing reason flag"
    ...
    try:
        model = convert_record_to_model(rename_new_record, "C:", NtfsUsnJournalCollector.DEFAULT_PROVIDER_ID)
        ...
        assert "RENAME_NEW_NAME" in model.attributes.get("usn_reason_flags", []), "Missing reason flag"
...
def main():
    ...
    parser.add_argument("--volumes", type=str, default="C:",
    ...
    parser.add_argument("--output", type=str, default="ntfs_activities.jsonl",
