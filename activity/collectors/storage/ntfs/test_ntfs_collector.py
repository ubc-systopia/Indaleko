...
class TestNtfsStorageActivityCollector(unittest.TestCase):
    ...
    def test_init_with_mock_mode(self):
        ...
        with patch('activity.collectors.storage.ntfs.ntfs_collector.WINDOWS_AVAILABLE', True):
        ...
        self.assertTrue(collector._use_volume_guids)  # Volume GUIDs should be the default
    ...
    def test_volume_guid_mapping(self):
        ...
        self.mock_machine_config.map_drive_letter_to_volume_guid.assert_called_once_with("C")
    ...
    def test_mock_data_generation(self):
        ...
        with patch('activity.collectors.storage.ntfs.ntfs_collector.WINDOWS_AVAILABLE', False):
            collector = NtfsStorageActivityCollector(
                volumes=["C:"],
                auto_start=True
            )
    ...
    def test_error_handling_volume_opening(self):
        ...
        with patch('activity.collectors.storage.ntfs.ntfs_collector.win32file.CreateFile', 
                  side_effect=Exception("Failed to open volume")):
            
            collector = NtfsStorageActivityCollector(
                mock=False,  # Not in mock mode
                volumes=["C:"],
                auto_start=True
            )
