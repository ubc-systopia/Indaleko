...
def main():
    ...
    try:
        ...
        try:
            ...
            struct.pack_into("<QQQHH", buffer_in, 0, 0, first_usn, 0xFFFFFFFFFFFFFFFF, 2, 2)
            ...
            try:
                ...
                if win_err.winerror == 38:  # ERROR_HANDLE_EOF
                    print("No new USN records available (reached end of file) - this is normal")
            ...
            if len(result) > 8:
                ...
                while offset < len(result):
                    ...
                    if offset + 4 <= len(result):
                        record_length = struct.unpack("<L", result[offset:offset+4])[0]
                        if record_length == 0:
                            break
                        ...
                        if offset + 58 <= len(result):
                            ...
                            file_name_length = struct.unpack("<H", result[offset+58:offset+60])[0]
                            file_name_offset = struct.unpack("<H", result[offset+60:offset+62])[0]
                            
                            # Extract filename if possible
                            if offset + file_name_offset + file_name_length <= len(result):
                                try:
                                    file_name_bytes = result[offset+file_name_offset:offset+file_name_offset+file_name_length]
