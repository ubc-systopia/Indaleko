...
def main():
    ...
    # Add test options
    parser.add_argument("--force-activity", action="store_true",
                        help="Force file activity generation for testing")
    parser.add_argument("--activity-interval", type=float, default=5.0,
    ...
    logger.info(f"Duration: {args.duration if args.duration > 0 else 'Until Ctrl+C'} seconds")
    ...
    try:
        ...
        if args.force_activity:
            logger.info(f"Starting file activity generator with interval {args.activity_interval}s")
        ...
        try:
            if args.duration > 0:
                ...
                while time.time() < end_time:
                    ...
                    if activities or args.verbose:
                        ...
                        if args.verbose and activities:
                            logger.info(f"Recent activities (showing up to 5):")
                            for i, activity in enumerate(activities[-5:]):
                                logger.info(f"  {i+1}. [{activity.activity_type}] {activity.file_name}")
                ...
                activity_interval = max(1, int(args.interval * 2))  # Display update interval
                last_count = 0
                        ...
                        if new_count > 0 or args.verbose:
                            ...
                            if activities:
                                ...
                                for i, activity in enumerate(activities[-min(5, len(activities)):]):
                                    timestamp = activity.timestamp.strftime("%H:%M:%S") if hasattr(activity, 'timestamp') else "Unknown"
                                    filename = activity.file_name if hasattr(activity, 'file_name') else "Unknown"
                                    activity_type = activity.activity_type if hasattr(activity, 'activity_type') else "Unknown"
                                    
                                    print(f"  {i+1}. [{timestamp}] [{activity_type}] {filename}")
