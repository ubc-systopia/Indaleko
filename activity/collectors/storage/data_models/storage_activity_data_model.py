...
class BaseStorageActivityData(IndalekoBaseModel):
    ...
    previous_file_name: Optional[str] = Field(
        None,
        description="Previous file name for rename/move operations"
    )
    previous_file_path: Optional[str] = Field(
        None,
        description="Previous file path for move operations"
    )

    # Provenance
    provenance: Optional[BaseProvenanceDataModel] = Field(
        None,
...
class NtfsStorageActivityData(BaseStorageActivityData):
    ...
    parent_file_reference_number: Optional[str] = Field(
        None,
        description="Parent directory file reference number"
    )
    reason_flags: Optional[int] = Field(
        None,
        description="USN Journal reason flags"
    )
    volume_name: str = Field(
        ...,
        description="Volume name (e.g., C:)"
    )
    is_directory: bool = Field(
        default=False,
        description="Whether the item is a directory"
    )
    security_id: Optional[int] = Field(
        None,
        description="Security ID for the file"
    )
    usn: Optional[int] = Field(
        None,
        description="USN Journal sequence number"
    )
...
class CloudStorageActivityData(BaseStorageActivityData):
    ...
    cloud_parent_id: Optional[str] = Field(
        None,
        description="Parent directory/folder ID"
    )
    shared: Optional[bool] = Field(None, description="Whether the item is shared")
    web_url: Optional[str] = Field(None, description="URL to access the item in web browser")
    mime_type: Optional[str] = Field(None, description="MIME type of the file")
    size: Optional[int] = Field(None, description="Size of the file in bytes")
    is_directory: bool = Field(default=False, description="Whether the item is a directory/folder")
    created_time: Optional[AwareDatetime] = Field(None, description="Time when the item was created")
    modified_time: Optional[AwareDatetime] = Field(None, description="Time when the item was last modified")
            ...
            if isinstance(self.modified_time, str):
                try:
                    ...
                    if not self.modified_time.tzinfo:
                        self.modified_time = self.modified_time.replace(tzinfo=timezone.utc)
                except ValueError:
...
class DropboxStorageActivityData(CloudStorageActivityData):
    ...
    revision: Optional[str] = Field(None, description="Dropbox file revision ID")
    shared_folder_id: Optional[str] = Field(None, description="ID of the shared folder if applicable")
...
class OneDriveStorageActivityData(CloudStorageActivityData):
    ...
    c_tag: Optional[str] = Field(None, description="Change tag for detecting changes")
    e_tag: Optional[str] = Field(None, description="ETag for the item")


class GoogleDriveStorageActivityData(CloudStorageActivityData):
    """Google Drive-specific storage activity data."""
    file_id: str = Field(..., description="Google Drive file ID")
    drive_id: Optional[str] = Field(None, description="Google Drive ID if not the default")
    parents: Optional[List[str]] = Field(None, description="IDs of parent folders")
    spaces: Optional[List[str]] = Field(None, description="Spaces containing the file (drive, photos, etc.)")
    version: Optional[str] = Field(None, description="Version number of the file")


class StorageActivityMetadata(IndalekoBaseModel):
    """Metadata for storage activity collection."""
    collection_start_time: AwareDatetime = Field(default_factory=lambda: datetime.now(timezone.utc))
    provider_type: StorageProviderType = Field(..., description="Type of storage provider")
    provider_name: str = Field(..., description="Name of the activity provider")
    source_machine: Optional[str] = Field(None, description="Source machine name if applicable")
    activity_count: int = Field(default=0, description="Number of activities collected")
    storage_location: Optional[str] = Field(None, description="Storage location (volume, URL, etc.)")
    provenance: Optional[Any] = Field(None, description="Provenance information (optional)")
    ...
    def ensure_timezone(self) -> 'StorageActivityMetadata':
        ...
        if not hasattr(self, 'collection_start_time'):
            ...
            if isinstance(self.collection_start_time, str):
                try:
                    # Try to parse as ISO format with timezone
                    self.collection_start_time = datetime.fromisoformat(self.collection_start_time)
                    # If no timezone info, add UTC
                    if not self.collection_start_time.tzinfo:
                        self.collection_start_time = self.collection_start_time.replace(tzinfo=timezone.utc)
                except ValueError:
                    # If parsing fails, use current time
                    self.collection_start_time = datetime.now(timezone.utc)
            # Then check if it's a datetime without timezone
...
class StorageActivityData(IndalekoBaseModel):
    ...
    Timestamp: IndalekoTimestampDataModel = Field(default_factory=IndalekoTimestampDataModel)
