...
class YouTubeVideoActivity(IndalekoMultiClassifiedActivityDataModel):
    ...
    class YouTubeMetadata(IndalekoMultiClassifiedActivityDataModel.Config):
        ...
        like_status: Optional[str] = None  # 'liked', 'disliked', or None
    ...
    def from_youtube_api(cls, video_data: Dict[str, Any], watch_data: Dict[str, Any]):
        ...
        if "contentDetails" in video_data and "duration" in video_data["contentDetails"]:
            duration_sec = parse_youtube_duration(video_data["contentDetails"]["duration"])
...
def calculate_watch_percentage(watch_data):
    ...
    if "percent_watched" in watch_data:
        ...
        if watch_data["duration"] > 0:
            return min(1.0, float(watch_data["current_time"]) / float(watch_data["duration"]))
    return 0.0
...
def calculate_ambient_score(video_data, watch_data):
    ...
    duration = parse_youtube_duration(video_data.get("contentDetails", {}).get("duration", ""))
    ...
    ambient_keywords = ["ambient", "background", "music", "asmr", "noise", "sleep", "study", "relaxing"]
...
def calculate_consumption_score(video_data, watch_data):
    ...
    entertainment_categories = ["1", "17", "24", "23", "20"]  # Film, Sports, Entertainment, Comedy, Gaming
...
def calculate_research_score(video_data, watch_data):
    ...
    educational_categories = ["27", "28", "22", "15"]  # Education, Science, Howto, Pets&Animals
    if video_data.get("snippet", {}).get("categoryId") in educational_categories:
        score += 0.6
    
    # Educational keywords
    edu_keywords = ["tutorial", "how to", "guide", "learn", "course", "lecture", "explained", "lesson"]
    title = video_data.get("snippet", {}).get("title", "").lower()
    ...
    edu_channels = ["academy", "university", "school", "learn", "education", "course", "tutorial"]
...
def calculate_productivity_score(video_data, watch_data):
    ...
    prod_keywords = ["productivity", "workflow", "organization", "time management", 
                    "efficiency", "gtd", "getting things done", "work", "business"]
    title = video_data.get("snippet", {}).get("title", "").lower()
    if any(kw in title for kw in prod_keywords):
        score += 0.6
    
    # Work-related categories
    work_categories = ["28", "27", "22"]  # Science & Technology, Education, Howto & Style
    ...
    tool_keywords = ["software", "tutorial", "tool", "app", "application", "program", "coding"]
