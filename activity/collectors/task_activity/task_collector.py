...
class TaskActivityCollector(CollectorBase):
    ...
    def _create_test_data(self):
        ...
        task_templates = [
            ...
            },
            {
                ...
                "due_time": datetime.now(timezone.utc) - timedelta(days=1),
                "completed_time": datetime.now(timezone.utc) - timedelta(hours=12),
            },
            {
    ...
    def get_tasks(self, filters: Optional[Dict] = None) -> List[TaskData]:
        """
    ...
    def create_task(self, task_data: Dict) -> TaskData:
        ...
        if "created_time" not in task_data:
            task_data["created_time"] = datetime.now(timezone.utc)
    ...
    def update_task(self, task_id: str, updates: Dict) -> Optional[TaskData]:
        """
        ...
        if "status" in updates and updates["status"] == TaskStatus.COMPLETED:
            if "completed_time" not in updates:
                update_data["completed_time"] = datetime.now(timezone.utc)
    ...
    def create_random_task(self) -> TaskData:
        ...
        task_data = {
            ...
            "created_time": datetime.now(timezone.utc) - timedelta(days=random.randint(0, 10)),
        }
        
        # Add due date (50% chance)
        if random.random() > 0.5:
            task_data["due_time"] = datetime.now(timezone.utc) + timedelta(days=random.randint(1, 14))
            
        # If task is completed, add completion time
        if task_data["status"] == TaskStatus.COMPLETED:
            task_data["completed_time"] = datetime.now(timezone.utc) - timedelta(hours=random.randint(1, 48))
...
def main():
    ...
    if tasks:
        ...
        updated_task = collector.update_task(
            ...
            {"status": TaskStatus.COMPLETED if task_to_update.status != TaskStatus.COMPLETED else TaskStatus.IN_PROGRESS}
        )
        if updated_task:
    ...
    for activity in activities:
        ic(f"- {activity['timestamp']}: {activity['action']} task {activity['task_id']}")
