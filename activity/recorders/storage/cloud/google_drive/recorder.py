...
class GoogleDriveActivityRecorder(StorageActivityRecorder):
    ...
    def __init__(self, **kwargs):
        ...
        kwargs["collection_name"] = kwargs.get("collection_name", "GoogleDriveStorageActivity")
        ...
        if collector is None:
            ...
            raise ValueError("collector must be an instance of GoogleDriveActivityCollector")
        ...
        self._metadata = StorageActivityMetadata(
            ...
            storage_location=f"Google Drive"
        )
    ...
    def _build_gdrive_activity_document(
        self,
        activity_data: Union[GoogleDriveStorageActivityData, GDriveActivityData],
        ...
        if semantic_attributes is None:
            ...
            for attr in semantic_attributes:
                if attr.Identifier == str(StorageActivityAttributes.STORAGE_GOOGLE_DRIVE.value):
            ...
            if hasattr(activity_data, "shared") and activity_data.shared:
                ...
                for attr in semantic_attributes:
                    if attr.Identifier == str(StorageActivityAttributes.STORAGE_SHARED.value):
    ...
    def store_activity(
        self, 
        activity_data: Union[GoogleDriveStorageActivityData, GDriveActivityData, Dict]
    ) -> uuid.UUID:
        """
    ...
    def get_google_drive_specific_statistics(self) -> Dict[str, Any]:
        ...
        try:
            file_type_cursor = self._db.db.aql.execute(
                file_type_query,
                bind_vars={"@collection": self._collection_name}
            )
            sharing_cursor = self._db.db.aql.execute(
                sharing_query,
                bind_vars={"@collection": self._collection_name}
            )
            app_cursor = self._db.db.aql.execute(
                app_query,
                bind_vars={"@collection": self._collection_name}
            )
    ...
    def _register_with_service_manager(self) -> None:
        ...
        if not hasattr(self, '_db') or self._db is None:
            self._logger.info("Skipping registration with service manager since no database connection is active")
            return
