...
import data_models.timestamp as timestamp

def parse_arguments():
    """Parse command line arguments."""
    parser = argparse.ArgumentParser(description='Verify NtfsHotTierRecorder implementation')
    
    # Database connection options
    parser.add_argument('--database-url', type=str, default='http://localhost:8529',
                      help='URL for ArangoDB server')
    parser.add_argument('--database', type=str, default='indaleko',
                      help='Database name')
    ...
    parser.add_argument('--verify-entities', action='store_true',
                      help='Verify entity mapping functionality')
    parser.add_argument('--test-ttl', action='store_true',
                      help='Test TTL expiration')
...
def find_jsonl_files(path: Optional[str] = None) -> List[str]:
    """
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    activity_type = activity.get('Activity', {}).get('activity_type', 'unknown')
...
def load_data_to_database(recorder: NtfsHotTierRecorder, files: List[str], args) -> Dict[str, Any]:
    """
    ...
    for file_path in files:
        ...
        if args.dry_run or getattr(recorder, "_no_db", False):
            print(f"Dry run: Would have processed {activity_counts['total_lines']} activities")
        ...
        print(f"Processed {loaded}/{activity_counts['total_lines']} activities in {processing_time:.2f} seconds")
...
def test_ttl_expiration(recorder: NtfsHotTierRecorder, args) -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"Inserted test activity with expiration in 30 seconds. Activity key: {test_activity['_key']}")
        print("Waiting for TTL index to process expiration...")
        print("Note: ArangoDB typically processes TTL every 30 seconds, so this may take up to 60 seconds.")
        ...
        for i in range(max_wait):
            ...
            try:
                still_exists = recorder._activities_collection.get(test_activity["_key"])
...
def run_performance_benchmark(recorder: NtfsHotTierRecorder, args) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            print(f"Benchmark: get_recent_activities - {query_time:.4f} seconds, {len(activities) if activities else 0} results")
        except Exception as e:
            benchmarks["recent_activities"] = {
                "success": False,
                "error": str(e),
        ...
        try:
            ...
            bind_vars = {
                "@collection": recorder._activities_collection.name
            }
            ...
            if entity_ids:
                ...
                print(f"Benchmark: get_activities_by_entity - {query_time:.4f} seconds, {len(activities) if activities else 0} results")
            else:
                benchmarks["entity_activities"] = {
                    "success": False,
        ...
        try:
            ...
            print(f"Benchmark: get_activities_by_time_range - {query_time:.4f} seconds, {len(activities) if activities else 0} results")
        except Exception as e:
            benchmarks["time_range_activities"] = {
                "success": False,
                "error": str(e),
...
def test_query_capabilities(recorder: NtfsHotTierRecorder, args) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            print(f"Query test: get_recent_activities - {len(activities) if activities else 0} results")
        except Exception as e:
            query_tests["recent_activities"] = {
                "success": False,
                "error": str(e)
        ...
        try:
            ...
            if activity_types:
                ...
                print(f"Query test: get_activities_by_type - {len(activities) if activities else 0} results for type '{test_type}'")
            else:
                query_tests["activity_type"] = {
                    "success": False,
        ...
        try:
            ...
            if paths:
                ...
                print(f"Query test: get_activities_by_path - {len(activities) if activities else 0} results for path '{dir_path}'")
            else:
                query_tests["path"] = {
                    "success": False,
...
def run_full_verification(recorder: NtfsHotTierRecorder, files: List[str], args) -> Dict[str, Any]:
    """
    ...
    """
    results = {
        "verification_started": datetime.datetime.now(timezone.utc).isoformat(),
        "steps": {}
    }
    ...
    if not connection_result and not args.dry_run:
        print("ERROR: Database connection verification failed. Cannot continue with verification.")
    ...
    results["verification_completed"] = datetime.datetime.now(timezone.utc).isoformat()
