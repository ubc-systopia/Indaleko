...
def create_hot_tier_recorder(
    db_config: IndalekoDBConfig | None,
    ttl_days: int = 4,
    debug: bool = False,
...
def generate_summary_report(
    ...
    report.append(
        f"- Activities loaded to database: {loaded_activities:,} ({loaded_activities/total_activities*100:.1f}% success)",
    )
    report.append(f"- Total processing time: {processing_time:.2f} seconds")
    if processing_time > 0:
        report.append(
            f"- Average processing rate: {loaded_activities/processing_time:.1f} activities/second",
        )
...
def save_report_to_file(report: str, output_path: str | None = None) -> str:
    """
...
def main():
    ...
    try:
        ...
        if not args.no_queries and not args.dry_run:
            queries = run_test_queries(recorder, verbose=args.verbose)
