...
def test_ttl_processing(activities: List[Dict[str, Any]], ttl_days: int = 4) -> Dict[str, Any]:
    """Test TTL processing by simulating expiration dates."""
    for activity in activities:
        # Add TTL timestamp
        activity_time = datetime.fromisoformat(activity["timestamp"])
        ttl_timestamp = activity_time.timestamp() + (ttl_days * 24 * 60 * 60)
        ttl_date = datetime.fromtimestamp(ttl_timestamp, tz=timezone.utc)
...
def print_results(results: Dict[str, Any]) -> None:
    ...
    print(f"  Activities with entities: {results['entity_results']['activities_with_entities']}")
...
def main():
    ...
    file_path = os.path.join(
        ...
        "activity", "collectors", "storage", "ntfs", "ntfs_activities.jsonl"
    )
