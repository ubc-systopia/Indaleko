...
IMPORTANT_DOCUMENT_EXTENSIONS = {
    ...
    # Source code
    '.py', '.js', '.ts', '.html', '.css', '.c', '.cpp', '.h', '.java', '.cs',
    '.go', '.rs', '.php', '.rb', '.swift',
    # Data
...
IMPORTANT_PATH_SEGMENTS = {
    'Documents', 'Projects', 'src', 'source', 'repos', 'work', 'research',
    'thesis', 'paper', 'manuscript', 'report'
}

# Paths with lower importance
TEMPORARY_PATH_SEGMENTS = {
    'temp', 'tmp', 'cache', 'downloaded', '.git', 'node_modules', '__pycache__',
    '.venv', 'bin', 'obj', 'build', 'dist'
}
...
class ImportanceScorer:
    ...
    def __init__(self, **kwargs):
        ...
        if self._debug:
            self._logger.debug(f"Initialized ImportanceScorer with weights: {self._weights}")
            
        # Initialize path pattern cache for faster matching
        self._path_pattern_cache = {
            "important": re.compile("|".join([rf"\\{p}\\|/{p}/" for p in IMPORTANT_PATH_SEGMENTS]), re.IGNORECASE),
            "temporary": re.compile("|".join([rf"\\{p}\\|/{p}/" for p in TEMPORARY_PATH_SEGMENTS]), re.IGNORECASE)
        }
    
    def calculate_importance(self, activity_data: Dict[str, Any], 
                           entity_metadata: Optional[Dict[str, Any]] = None,
                           search_hits: int = 0) -> float:
        """
        ...
        frequency_score = self._calculate_frequency_score(activity_data, entity_metadata, search_hits)
        ...
        if self._debug:
            self._logger.debug(f"Importance calculation: recency={recency_score:.2f}, "
    ...
    def _calculate_content_score(self, activity_data: Dict[str, Any]) -> float:
        ...
        if os.path.basename(file_path).lower() in {
            'readme.md', 'license', 'package.json', 'cargo.toml', 'pyproject.toml',
            'makefile', 'dockerfile', 'manifest', 'config'
        }:
            base_score += 0.15
    ...
    def _calculate_novelty_score(self, 
                               activity_data: Dict[str, Any],
                               entity_metadata: Optional[Dict[str, Any]] = None) -> float:
        """
    ...
    def set_weights(self, weights: Dict[str, float]) -> None:
        ...
        if not all(key in weights for key in required_keys):
            raise ValueError(f"Missing required keys in weights. Required: {required_keys}")
...
if __name__ == "__main__":
    ...
    older_activity["timestamp"] = (datetime.now(timezone.utc) - timedelta(days=10)).isoformat()
