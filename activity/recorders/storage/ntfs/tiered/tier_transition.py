...
class TierTransitionManager:
    ...
    def __init__(self, **kwargs):
        ...
        if self._hot_tier is None:
            ...
            try:
                self._hot_tier = NtfsHotTierRecorder(
                    debug=self._debug
                )
    ...
    def get_transition_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            threshold_time = datetime.now(timezone.utc) - timedelta(hours=self._age_threshold_hours)
            ...
            cursor = self._hot_tier._db.db.aql.execute(
                ready_query,
                bind_vars={
            ...
            stats["warm_tier"] = {
                ...
                "aggregated_activities": warm_stats.get("by_aggregation", {}).get("aggregated", 0),
                "individual_activities": warm_stats.get("by_aggregation", {}).get("individual", 0)
            }
    ...
    def transition_batch(self, 
        ...
        try:
            ...
            self._logger.info(f"Processing {len(hot_activities)} activities for warm tier")
    ...
    def run_transition(self, 
                     max_batches: int = 10, 
                     pause_seconds: int = 5) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for batch in range(max_batches):
                ...
                if found == 0:
                    self._logger.info("No more activities found for transition, stopping")
                    break
                    
                # Pause between batches
                if batch < max_batches - 1 and found > 0:
                    self._logger.info(f"Pausing for {pause_seconds} seconds before next batch")
