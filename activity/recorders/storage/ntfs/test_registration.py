    ...
    try:
        ...
        try:
            ...
            recorder_id = uuid.UUID(
                "9b3a7e8c-6d2f-4e91-8b5a-f3c7d2e1a0b9",
            )  # Fixed ID from the class
            ...
            logger.info(
                f"NTFS Storage Activity recorder initialized with ID: {recorder.get_recorder_id()}",
            )
            ...
            if collector:
                logger.info(
                    f"Using collector with provider ID: {collector._provider_id}",
                )
                logger.info(
                    f"Volume GUID support: {'Enabled' if hasattr(collector, '_use_volume_guids') and collector._use_volume_guids else 'Disabled'}",
                )
                ...
                logger.warning(
                    "No NTFS collector available - skipping activity generation",
                )
            ...
            logger.warning(
                "This module may require Windows or specific Python packages.",
            )
        ...
        logger.info(
            f"  - Collection name attribute: {'_collection_name' in dir(StorageActivityRecorder)}",
        )
        logger.info(
            f"  - Registration method: {'_register_with_activity_service' in dir(StorageActivityRecorder)}",
        )
        ...
        try:
            from activity.recorders.storage.cloud.dropbox.dropbox_recorder import (
                DropboxStorageActivityRecorder,
            )

            logger.info("  - DropboxStorageActivityRecorder")
        except ImportError:
            logger.info("  - DropboxStorageActivityRecorder (unavailable)")

        try:
            from activity.recorders.storage.cloud.onedrive.onedrive_recorder import (
                OneDriveStorageActivityRecorder,
            )

            logger.info("  - OneDriveStorageActivityRecorder")
        except ImportError:
            logger.info("  - OneDriveStorageActivityRecorder (unavailable)")

        try:
            from activity.recorders.storage.cloud.gdrive.gdrive_recorder import (
                GDriveStorageActivityRecorder,
            )

            logger.info("  - GDriveStorageActivityRecorder")
        except ImportError:
            logger.info("  - GDriveStorageActivityRecorder (unavailable)")
