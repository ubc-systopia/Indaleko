...
def test_integration_directly():
    ...
    for i, activity in enumerate(activities):
        ...
        logging.info(f"Enhanced activity {i+1}: {enhanced.get('activity_context_handle') if isinstance(enhanced, dict) else 'No handle'}")
...
def test_integration_with_recorder(use_hot_tier: bool = False):
    ...
    if context_data:
        ...
        cursors = context_data.get('Cursors', [])
        logging.info(f"Context has {len(cursors)} cursors")
        ntfs_cursors = [cursor for cursor in cursors 
                       if cursor.get('Provider') == str(NtfsActivityContextIntegration.NTFS_CONTEXT_PROVIDER_ID)]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test NTFS Activity Context Integration")
    parser.add_argument("--direct", action="store_true", help="Test integration directly")
    parser.add_argument("--recorder", action="store_true", help="Test with standard recorder")
    parser.add_argument("--hot-tier", action="store_true", help="Test with hot tier recorder")
