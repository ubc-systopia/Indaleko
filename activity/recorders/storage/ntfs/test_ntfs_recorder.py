...
class TestNtfsStorageActivityRecorder(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_collector._provider_id = uuid.UUID("7d8f5a92-35c7-41e6-b13d-6c4e89e7f2a5")
        ...
        self.mock_collection.add_document.return_value = {"_id": "mock_id", "_key": "mock_key"}
    ...
    @patch('activity.recorders.storage.base.StorageActivityRecorder._register_with_activity_service')
    @patch('hashlib.md5')
    ...
    def test_build_ntfs_activity_document(self, mock_register, mock_connect):
        ...
        # Build document
        with patch('activity.recorders.storage.ntfs.ntfs_recorder.get_semantic_attributes_for_activity') as mock_get_attr:
    ...
    def test_timezone_aware_datetime_handling(self, mock_register, mock_connect):
        ...
        # Build document
        with patch('activity.recorders.storage.ntfs.ntfs_recorder.get_semantic_attributes_for_activity'):
            document = recorder._build_ntfs_activity_document(test_activity)
    ...
    def test_get_recorder_characteristics(self, mock_register, mock_connect):
        ...
        self.assertTrue(any("ACTIVITY_DATA_SYSTEM_ACTIVITY" in str(char) for char in characteristics))
        self.assertTrue(any("ACTIVITY_DATA_FILE_ACTIVITY" in str(char) for char in characteristics))
        self.assertTrue(any("ACTIVITY_DATA_WINDOWS_SPECIFIC" in str(char) for char in characteristics))

    @patch('activity.recorders.storage.base.StorageActivityRecorder._connect_to_db')
    @patch('activity.recorders.storage.base.StorageActivityRecorder._register_with_activity_service')
    @patch('activity.recorders.storage.ntfs.ntfs_recorder.NtfsStorageActivityCollector')
    def test_fallback_collector_creation(self, mock_collector_class, mock_register, mock_connect):
