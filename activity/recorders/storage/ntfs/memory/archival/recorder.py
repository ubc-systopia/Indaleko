...
class NtfsArchivalMemoryRecorder(StorageActivityRecorder):
    ...
    def __init__(self, **kwargs):
        ...
        self._entity_collection_name = kwargs.get("entity_collection_name", self.ENTITY_COLLECTION_NAME)
        self._long_term_collection_name = kwargs.get("long_term_collection_name", self.LONG_TERM_COLLECTION_NAME)
        self._importance_threshold = kwargs.get("importance_threshold", self.IMPORTANCE_THRESHOLD)
        self._knowledge_graph_collection_name = kwargs.get("knowledge_graph_collection_name", self.KNOWLEDGE_GRAPH_COLLECTION_NAME)
        ...
        kwargs["description"] = kwargs.get(
            "description", "Records highly significant NTFS file system activities in archival memory"
        )
    ...
    def _register_with_service_manager(self) -> None:
        ...
        try:
            ...
            from activity.collectors.storage.semantic_attributes import get_storage_activity_semantic_attributes
            ...
            registration_kwargs = {
                ...
                "Tags": ["storage", "activity", "ntfs", archival_memory_tag, "cognitive_memory"]
            }

            # Register with service manager
            try:
                self._logger.info(f"Registering with activity registration service: {self._recorder_id}")
                service = IndalekoActivityDataRegistrationService()
                service.register_provider(**registration_kwargs)
                self._logger.info(f"Successfully registered with service manager: {self._recorder_id}")
            except Exception as e:
    ...
    def _setup_collections(self):
        ...
        try:
            ...
            if not self._db.collection_exists(self._entity_collection_name):
                self._logger.info(f"Creating entity collection: {self._entity_collection_name}")
                entity_collection = self._db.create_collection(self._entity_collection_name)
                self._logger.info(f"Created entity collection: {entity_collection.name}")
            else:
            ...
            if not self._db.collection_exists(self._knowledge_graph_collection_name):
                self._logger.info(f"Creating knowledge graph collection: {self._knowledge_graph_collection_name}")
                knowledge_graph_collection = self._db.create_collection(self._knowledge_graph_collection_name, edge=True)
                self._logger.info(f"Created knowledge graph collection: {knowledge_graph_collection.name}")
            else:
                self._logger.info(f"Knowledge graph collection already exists: {self._knowledge_graph_collection_name}")
    ...
    def _setup_indices(self):
        ...
        try:
            ...
            self._collection.add_hash_index(fields=["Record.Data.activity_types"], unique=False)
            
            # Index on timestamp for time-based queries
            self._collection.add_hash_index(fields=["Record.Data.first_seen"], unique=False)
            self._collection.add_hash_index(fields=["Record.Data.last_activity"], unique=False)
            
            # Index on semantic concepts for concept-based queries
            self._collection.add_hash_index(fields=["Record.Data.semantic_concepts"], unique=False)
            
            # Index on ontology concepts for advanced semantic queries
            self._collection.add_hash_index(fields=["Record.Data.ontology.concepts"], unique=False)
            ...
            if self._db.collection_exists(self._knowledge_graph_collection_name):
                knowledge_graph_collection = self._db.get_collection(self._knowledge_graph_collection_name)
                
                # Index on relationship type
                knowledge_graph_collection.add_hash_index(fields=["type"], unique=False)
                
                # Index on creation date
                knowledge_graph_collection.add_hash_index(fields=["created_at"], unique=False)
                
                # Index on strength
                knowledge_graph_collection.add_hash_index(fields=["strength"], unique=False)
                
                # Index on semantic relationship
                knowledge_graph_collection.add_hash_index(fields=["semantic_type"], unique=False)
    ...
    def _enhance_ontology(self, w5h_concepts: Dict[str, List[str]]) -> Dict:
        ...
        document_concepts = [c for c in w5h_concepts.get("what", []) if c in [
            "document", "report", "memo", "letter", "resume"
        ]]
        if document_concepts:
            categories["document_types"] = document_concepts
            
        # Code types category
        code_concepts = [c for c in w5h_concepts.get("what", []) if c in [
            "source_code", "python", "javascript", "java", "c_cpp"
        ]]
        if code_concepts:
            categories["code_types"] = code_concepts
            
        # Media types category
        media_concepts = [c for c in w5h_concepts.get("what", []) if c in [
            "image", "video", "audio"
        ]]
        if media_concepts:
        ...
        location_concepts = [c for c in w5h_concepts.get("where", []) if c in [
            "documents_folder", "desktop", "downloads_folder", "pictures_folder",
            "videos_folder", "music_folder"
        ]]
        if location_concepts:
            categories["location_types"] = location_concepts
            
        # Usage patterns category
        usage_concepts = [c for c in w5h_concepts.get("how", []) if c in [
            "frequently_modified", "frequently_accessed", "reference_material",
            "active_reference", "balanced_usage", "work_in_progress"
        ]]
        if usage_concepts:
            categories["usage_patterns"] = usage_concepts
            
        # Purpose category
        purpose_concepts = [c for c in w5h_concepts.get("why", []) if c in [
            "project_work", "work_related", "personal_use", "temporary_use",
            "backup_purpose", "high_importance", "very_high_importance"
        ]]
        if purpose_concepts:
        ...
        if "work_related" in w5h_concepts.get("why", []) and "document" in w5h_concepts.get("what", []):
            inferences.append("work_document")
            
        # Infer project code from combinations
        if "project_work" in w5h_concepts.get("why", []) and "source_code" in w5h_concepts.get("what", []):
            inferences.append("project_code")
            
        # Infer active project from combinations
        if "project_work" in w5h_concepts.get("why", []) and any(c in w5h_concepts.get("how", []) 
                                                               for c in ["frequently_modified", "work_in_progress"]):
            inferences.append("active_project")
            
        # Infer reference material from combinations
        if "reference_material" in w5h_concepts.get("how", []) and any(c in w5h_concepts.get("what", []) 
                                                                     for c in ["document", "pdf"]):
    ...
    def _build_knowledge_graph_relationships(self, entity_id: uuid.UUID, entity_data: Dict) -> List[Dict]:
        """
        ...
        try:
            ...
            if project_indicators:
                ...
                project_cursor = self._db.db.aql.execute(
                    project_query,
                    bind_vars={
                        "@collection": self._collection_name,
                        "entity_id": str(entity_id),
                ...
                for related_entity in project_cursor:
                    ...
                    relationship = {
                        ...
                        "strength": 0.9,
                        "created_at": datetime.now(timezone.utc).isoformat(),
                        "description": f"Entities belong to the same project"
                    }
    ...
    def _build_archival_memory_document(
        ...
        document = {
            ...
            "Record": {
                ...
                "Attributes": [
                    {
                        "Identifier": str(StorageActivityAttributes.STORAGE_ACTIVITY.value),
                    ...
                    {
                        "Identifier": str(uuid.uuid5(uuid.NAMESPACE_URL, "indaleko:attribute:archival_memory")),
                ...
                "Data": {
                    ...
                    "first_seen": entity_data.get("created_at", datetime.now(timezone.utc).isoformat()),
                    "last_modified": entity_data.get("last_modified", datetime.now(timezone.utc).isoformat()),
                    "last_accessed": entity_data.get("last_accessed", datetime.now(timezone.utc).isoformat()),
                    "file_reference_number": entity_data.get("file_reference_number", ""),
                    "timestamp": datetime.now(timezone.utc).isoformat(),
                    "last_activity": long_term_data_dict.get("last_activity", datetime.now(timezone.utc).isoformat()),
                    ...
                    "memory_lineage": {
                        ...
                        "transitions": [
                            ...
                            *long_term_data_dict.get("memory_lineage", {}).get("transitions", []),
                            # Add new transition to archival memory
                            {
                                ...
                                "importance_at_transition": long_term_data_dict.get("importance_score", 0.0)
                            }
                        ],
                        "long_term_id": long_term_data.get("_key", ""),
                        "source_activities": long_term_data_dict.get("source_activities", []),
        ...
        document["Record"]["Data"]["semantic_concepts"] = long_term_data_dict.get("semantic_concepts", [])
        ...
        document["Record"]["Data"]["source_activities"] = long_term_data_dict.get("source_activities", [])
        
        # Add importance score (slightly enhanced for archival memory)
        importance_score = min(1.0, long_term_data_dict.get("importance_score", 0.8) * 1.05)
        ...
        document["Record"]["Data"]["historical_context"] = {
            ...
            "contextual_note": self._generate_contextual_note(long_term_data_dict, w5h_concepts)
        }
    ...
    def consolidate_from_long_term_memory(
        ...
        try:
            ...
            self._logger.info(f"Consolidating entities from long-term memory (min age: {min_age_days} days, min importance: {min_importance})")
            ...
            try:
                ...
                self._logger.error(f"Error getting eligible entities from long-term memory: {e}")
                # Fall back to direct query
                min_date = (datetime.now(timezone.utc) - timedelta(days=min_age_days)).isoformat()
            ...
            for long_term_entity in eligible_entities:
                ...
                try:
                    ...
                    entity_data = self._get_entity_from_entity_collection(uuid.UUID(entity_id))
                    if not entity_data:
                        self._logger.warning(f"Entity {entity_id} not found in entity collection")
                        continue
                    ...
                    relationships = self._build_knowledge_graph_relationships(uuid.UUID(entity_id), long_term_entity)
                    ...
                    if relationships:
                        ...
                        self._update_knowledge_graph_metadata(uuid.UUID(entity_id), len(relationships), relationships)
            ...
            self._logger.info(f"Consolidation complete: {stats['entities_consolidated']} entities consolidated")
    ...
    def _mark_consolidated_in_long_term(self, entity_id: uuid.UUID) -> bool:
        ...
        try:
            ...
            self._logger.error(f"Error marking entity as consolidated in long-term memory: {e}")
            return False
    
    def _update_knowledge_graph_metadata(self, entity_id: uuid.UUID, count: int, relationships: List[Dict]) -> bool:
        """
    ...
    def get_archival_memory_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            importance_cursor = self._db.db.aql.execute(
                importance_query,
                bind_vars={"@collection": self._collection_name}
            )
            ...
            concept_cursor = self._db.db.aql.execute(
                concept_query,
                bind_vars={"@collection": self._collection_name}
            )
            ...
            knowledge_graph_cursor = self._db.db.aql.execute(
                knowledge_graph_query,
                bind_vars={"@collection": self._collection_name}
            )
            ...
            age_cursor = self._db.db.aql.execute(
                age_query,
                bind_vars={"@collection": self._collection_name}
            )
            ...
            if self._db.collection_exists(self._knowledge_graph_collection_name):
                ...
                rel_count_cursor = self._db.db.aql.execute(
                    rel_count_query,
                ...
                rel_type_cursor = self._db.db.aql.execute(
                    rel_type_query,
                    bind_vars={"@collection": self._knowledge_graph_collection_name}
                )
    ...
    def search_archival_memory(
        ...
        w5h_filter: Optional[Dict[str, List[str]]] = None,
    ...
    def get_knowledge_graph_relationships(
        ...
        try:
            # Build query based on parameters
            filter_conditions = ["rel._from == CONCAT(@collection, '/', @entity_id) OR rel._to == CONCAT(@collection, '/', @entity_id)"]
...
if __name__ == "__main__":
    ...
    mode_group.add_argument("--consolidate-from-long-term", action="store_true",
    ...
    mode_group.add_argument("--search", type=str,
    ...
    parser.add_argument("--min-importance", type=float, default=0.8,
    ...
    # Add search options
    parser.add_argument("--concept-filter", type=str,
        ...
        if args.consolidate_from_long_term:
            ...
            if "error" in stats:
                ...
                if "avg_relationships_per_entity" in stats:
                    print(f"  Avg relationships per entity: {stats.get('avg_relationships_per_entity', 0):.1f}")
                    
                if "avg_entity_age_days" in stats:
                    print(f"  Avg entity age (days): {stats.get('avg_entity_age_days', 0):.1f}")
                ...
                print(f"  Importance threshold: {stats.get('importance_threshold', 'unknown')}")
            ...
            for i, entity in enumerate(results):
                ...
                if historical:
                    ...
                    print(f"    First seen: {historical.get('first_seen_date', 'Unknown')}")
                    print(f"    Activity span: {historical.get('activity_span_days', 0)} days")
                    print(f"    Note: {historical.get('contextual_note', '')}")
                
                # Show knowledge graph relationships if available
                if "knowledge_graph_relationships" in entity and entity["knowledge_graph_relationships"]:
                    ...
                    for j, rel in enumerate(entity["knowledge_graph_relationships"]):
                        ...
                        print(f"    {j+1}. {os.path.basename(related_path)} ({relationship.get('type', 'related')})")
