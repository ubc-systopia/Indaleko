...
class ImportanceScorer:
    ...
    EXTENSION_WEIGHTS = {
        ...
        # Media files
        "jpg": 0.6, "jpeg": 0.6, "png": 0.6, "gif": 0.6, "mp3": 0.6, "mp4": 0.6, "avi": 0.6,
        "mov": 0.6, "wav": 0.6, "svg": 0.6, "bmp": 0.5, "tiff": 0.5, "flac": 0.5,
    ...
    def __init__(self, **kwargs):
        ...
        self._compiled_patterns = [(re.compile(pattern), weight) 
    ...
    def calculate_importance(self, activity_data: Dict[str, Any]) -> float:
        ...
        if importance_boost > 0:
            ...
            if self._debug:
                self._logger.debug(f"Applied importance boost of {importance_boost}: {importance:.2f}")
    ...
    def _calculate_activity_type_importance(self, activity_data: Dict[str, Any]) -> float:
        """
    ...
    def _calculate_recency_importance(self, activity_data: Dict[str, Any]) -> float:
        ...
        try:
            ...
            age_days = (datetime.now(timezone.utc) - activity_time).total_seconds() / (24 * 60 * 60)
    ...
    def _calculate_metadata_importance(self, activity_data: Dict[str, Any]) -> float:
        ...
        if "attributes" in activity_data:
            ...
            if attrs.get("usn_reason_simplified") in ["security_change", "named_data_extend"]:
                score += 0.05
        
        # Cap at 1.0
        return min(1.0, score)
    
    def calculate_importance_decay(self, 
    ...
    def estimate_retention_days(self, importance: float, memory_type: str = "short_term") -> int:
        """
...
if __name__ == "__main__":
    ...
    if args.input:
            ...
            for line_num, line in enumerate(f, 1):
                try:
                    ...
                    results.append({
                        ...
                        "retention_days": {
                            "sensory": scorer.estimate_retention_days(score, "sensory"),
                            "short_term": scorer.estimate_retention_days(score, "short_term"),
                            "long_term": scorer.estimate_retention_days(score, "long_term")
                        }
                    })
                except Exception as e:
        ...
        for activity in test_activities:
            ...
            print(f"  Retention (Sensory): {scorer.estimate_retention_days(score, 'sensory')} days")
            print(f"  Retention (Short-Term): {scorer.estimate_retention_days(score, 'short_term')} days")
        ...
        for activity, age_hours in activities_with_ages:
            score = scorer.calculate_importance(activity)
            decision = scorer.should_consolidate(score, age_hours, "sensory", "short_term")
            path = activity["file_path"]
