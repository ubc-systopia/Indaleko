...
class TemplateRecorder(RecorderBase):
    ...
    def __init__(self, **kwargs):
        ...
        self._collection_name = kwargs.get("collection_name", f"{self.__class__.__name__}Data")
        self._db_config_path = kwargs.get("db_config_path", None)
        
        # Service configuration
        self._provider_type = kwargs.get("provider_type", "Activity")
        self._provider_subtype = kwargs.get("provider_subtype", activity_type.capitalize())
        self._tags = kwargs.get("tags", [activity_type.lower()])
        
        # Activity characteristics
        self._characteristics = kwargs.get("characteristics", [
        ...
        try:
            ...
            if self._collection_name in self._db.get_collections():
                ...
                self._collection = self._db.create_collection(
                    ...
                    description=f"{self._name} data collection"
                )
                
            self._logger.info(f"Connected to database, collection: {self._collection_name}")
    ...
    def _register_with_service_manager(self) -> None:
        ...
        try:
            ...
            try:
                ...
                self._logger.info(f"Registered recorder with service manager: {self._recorder_id}")
            except Exception as e:
    ...
    def get_semantic_attributes(self, activity_data: Any) -> List[IndalekoSemanticAttributeDataModel]:
        """
