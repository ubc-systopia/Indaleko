...
class CalendarRecorder(RecorderBase):
    ...
    def __init__(self, **kwargs):
        ...
        self._recorder_id = kwargs.get("recorder_id", uuid.UUID("f7d8e9c0-a1b2-c3d4-e5f6-a7b8c9d0e1f2"))
        ...
        # Processing state
        self.processing_time = datetime.datetime.now(datetime.timezone.utc)
    ...
    def set_cursor(self, cursor: str) -> None:
        ...
        if cursor:
            try:
                ...
                if processing_time:
                    self.processing_time = datetime.datetime.fromisoformat(processing_time)
            except Exception as e:
                self.logger.error(f"Error parsing cursor: {e}")
                
    def _create_semantic_attributes(self, event: CalendarEvent) -> List[IndalekoSemanticAttributeDataModel]:
    ...
    def process_data(self, data: Any) -> List[CalendarEvent]:
        ...
        if isinstance(data, list) and all(isinstance(event, CalendarEvent) for event in data):
            ...
            self.logger.error("Invalid data format. Expected list of CalendarEvent objects.")
            return []
    ...
    def _register_with_service_manager(self) -> None:
        ...
        try:
            ...
            self.logger.info(f"Registered calendar recorder with service manager: {self._recorder_id}")
