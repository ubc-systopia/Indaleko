...
from activity.collectors.collaboration.data_models.email_file_share import EmailFileShareCollaborationDataModel as EmailFileShareData
...
class OutlookFileShareRecorder(RecorderBase):
    ...
    def __init__(self, collector: Optional[OutlookFileShareCollector] = None, data_dir: str = "./outlook_data", **kwargs):
        """
        ...
        collector_data = self.provider_registrar.lookup_provider_by_identifier(str(self.source_data["Identifier"]))
    ...
    def create_semantic_attributes(self, file_share: Dict) -> List[IndalekoSemanticAttributeDataModel]:
        """
    ...
    def build_email_file_share_document(
        ...
        timestamp = file_share.get("timestamp", datetime.now(timezone.utc).isoformat())
    ...
    def store_file_share(self, file_share: Dict) -> Optional[Dict]:
        """
        ...
        try:
            ...
            if file_share["id"] in self.processed_ids:
                self.logger.info(f"Skipping already processed file share: {file_share.get('filename')}")
                return None
            ...
            self.logger.info(f"Stored file share: {file_share.get('filename')} (key: {result})")
            ...
            self.logger.error(f"Error storing file share {file_share.get('filename')}: {e}")
            return None
    ...
    def lookup_file_by_url(self, url: str) -> Optional[Dict]:
        """
...
def main():
    ...
    try:
        ...
        sample_data = {
            ...
            "recipientEmailAddresses": ["recipient1@example.com", "recipient2@example.com"],
