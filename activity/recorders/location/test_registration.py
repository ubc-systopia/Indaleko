...
def main():
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"Windows GPS Location recorder initialized with ID: {recorder.get_recorder_id()}",
            )
        ...
        from activity.recorders.location.location_data_recorder import (
            BaseLocationDataRecorder,
        )

        # Show available location recorders
        logger.info("Available location recorder classes:")
        logger.info(" - BaseLocationDataRecorder")

        # Check if IP location recorder is available
        try:
            from activity.recorders.location.ip_location_recorder import (
                IPLocationRecorder,
            )

            logger.info(" - IPLocationRecorder")
        except ImportError:
            logger.info(" - IPLocationRecorder (unavailable)")

        # Check if WiFi location recorder is available
        try:
            from activity.recorders.location.wifi_location_recorder import (
                WiFiLocationRecorder,
            )

            logger.info(" - WiFiLocationRecorder")
        except ImportError:
            logger.info(" - WiFiLocationRecorder (unavailable)")

        # Check if Tile location recorder is available
        try:
            from activity.recorders.location.tile_location_recorder import (
                TileLocationRecorder,
            )

            logger.info(" - TileLocationRecorder")
        except ImportError:
            logger.info(" - TileLocationRecorder (unavailable)")
