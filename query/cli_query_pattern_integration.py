...
class QueryPatternCLIIntegration:
    ...
    def __init__(self, cli_instance: IndalekoBaseCLI, db_config: Optional[IndalekoDBConfig] = None):
        """
    ...
    def _register_commands(self):
        ...
        self.cli.append_help_text("  /patterns          - Query pattern analysis commands")
...
"""
    ...
    def _handle_analyze(self, args: List[str]) -> CommandResult:
        ...
        try:
            ...
            parser.add_argument("--max-queries", type=int, default=500, help="Maximum number of queries to analyze")
            ...
            # Run the analysis
            self.cli.print_message("Running query pattern analysis...", message_type="info")
    ...
    def _handle_metrics(self, args: List[str]) -> CommandResult:
        ...
        try:
            ...
            if not hasattr(self.analyzer.data, "user_metrics") or not self.analyzer.data.user_metrics:
                # Calculate metrics
                self.cli.print_message("Calculating query metrics...", message_type="info")
...
"""
    ...
    def _handle_chains(self, args: List[str]) -> CommandResult:
        ...
        try:
            ...
            return CommandResult(
                success=False,
                message=f"Error retrieving query chains: {str(e)}"
            )
    ...
    def _handle_entities(self, args: List[str]) -> CommandResult:
        ...
        try:
            ...
            if not hasattr(self.analyzer.data, "entity_usage") or not self.analyzer.data.entity_usage:
                ...
                self.cli.print_message("Loading query history to analyze entities...", message_type="info")
            ...
            if entity_name:
                ...
                if entity_usage.intents:
                    message += "\nIntents:\n"
                    for intent, count in sorted(entity_usage.intents.items(), key=lambda x: x[1], reverse=True):
                        message += f"- {intent}: {count}\n"
                
                # Show co-occurring entities
                if entity_usage.co_occurring_entities:
                    message += "\nCo-occurring Entities:\n"
                    co_entities = sorted(entity_usage.co_occurring_entities.items(), key=lambda x: x[1], reverse=True)
                ...
                top_entities = sorted(entities.values(), key=lambda e: e.mention_count, reverse=True)[:10]
                ...
                success_entities = sorted(success_entities, key=lambda e: e.success_rate, reverse=True)[:5]
                    ...
                    for co_entity, count in entity.co_occurring_entities.items():
                        if count >= 2:  # Minimum co-occurrence count
                            ...
                            if strength >= 0.5:  # Strong co-occurrence threshold
                                strong_cooccurrences.append((entity_name, co_entity, count, strength))
                
                # Sort by strength and show top 5
                strong_cooccurrences.sort(key=lambda x: x[3], reverse=True)
                
                for i, (entity1, entity2, count, strength) in enumerate(strong_cooccurrences[:5], 1):
            ...
            return CommandResult(
                success=False,
                message=f"Error retrieving entity information: {str(e)}"
            )
    ...
    def _handle_suggestions(self, args: List[str]) -> CommandResult:
        ...
        try:
            ...
            if parsed_args.context:
                ...
                if hasattr(self.analyzer.data, "entity_usage") and parsed_args.context not in self.analyzer.data.entity_usage:
                    return CommandResult(
                        success=False,
...
"""
    ...
    def _handle_save(self, args: List[str]) -> CommandResult:
        ...
        try:
            ...
            parser.add_argument("--file", type=str, default="query_patterns.json", help="Output file name")
            ...
            results = {
                ...
                "patterns": [p.model_dump() for p in self.analyzer.data.query_patterns] if hasattr(self.analyzer.data, "query_patterns") else [],
                "chains": [c.model_dump() for c in self.analyzer.data.query_chains] if hasattr(self.analyzer.data, "query_chains") else [],
                "entity_usage": {k: v.model_dump() for k, v in self.analyzer.data.entity_usage.items()} if hasattr(self.analyzer.data, "entity_usage") else {},
                "metrics": next(iter(self.analyzer.data.user_metrics.values())).model_dump() if hasattr(self.analyzer.data, "user_metrics") and self.analyzer.data.user_metrics else None
            }
...
def register_query_pattern_commands(cli_instance: IndalekoBaseCLI, db_config: Optional[IndalekoDBConfig] = None) -> QueryPatternCLIIntegration:
    """
...
def main():
    ...
    cli = IndalekoBaseCLI(
        ...
        default_handler=lambda cmd: print(f"Unknown command: {cmd}. Type /help for help.")
    )
