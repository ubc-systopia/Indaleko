...
class CognitiveMemoryQueryTool(BaseTool):
            ...
            examples=[
                {
                    ...
                    "returns": {
                        "results": [
                            {"memory_tier": "long_term", "file_path": "Documents/ProjectReport.pdf", "importance": 0.85},
                            {"memory_tier": "archival", "file_path": "Archive/2023/Q2Report.pdf", "importance": 0.92}
                        ],
                        "tier_stats": {
    ...
    def _initialize_memory_recorders(self) -> None:
        ...
        try:
            ...
            import importlib
            sensory_module = importlib.import_module("activity.recorders.storage.ntfs.memory.sensory.recorder")
            NtfsSensoryMemoryRecorder = getattr(sensory_module, "NtfsSensoryMemoryRecorder")
        ...
        try:
            # Importing here to avoid dependency issues
            import importlib
            short_term_module = importlib.import_module("activity.recorders.storage.ntfs.memory.short_term.recorder")
            NtfsShortTermMemoryRecorder = getattr(short_term_module, "NtfsShortTermMemoryRecorder")
        ...
        try:
            # Importing here to avoid dependency issues
            import importlib
            long_term_module = importlib.import_module("activity.recorders.storage.ntfs.memory.long_term.recorder")
            NtfsLongTermMemoryRecorder = getattr(long_term_module, "NtfsLongTermMemoryRecorder")
        ...
        try:
            # Importing here to avoid dependency issues
            import importlib
            archival_module = importlib.import_module("activity.recorders.storage.ntfs.memory.archival.recorder")
            NtfsArchivalMemoryRecorder = getattr(archival_module, "NtfsArchivalMemoryRecorder")
        ...
        if errors:
            error_summary = "\n".join(errors)
            ic(f"Memory recorder initialization completed with some errors:\n{error_summary}")
        else:
            ic("All memory recorders initialized successfully")
    
    def _query_sensory_memory(self, query: str, importance_min: float = 0.0, limit: int = 10) -> List[Dict[str, Any]]:
        """
    ...
    def _query_short_term_memory(self, query: str, importance_min: float = 0.0, limit: int = 10) -> List[Dict[str, Any]]:
        """
    ...
    def _calculate_tier_statistics(self, results_by_tier: Dict[str, List[Dict[str, Any]]]) -> Dict[str, Dict[str, Any]]:
        """
    ...
    def _extract_importance_score(self, result: Dict[str, Any]) -> Optional[float]:
        """
    ...
    def _rank_results(self, results: List[Dict[str, Any]], tier_weights: Dict[str, float]) -> List[Dict[str, Any]]:
        """
    ...
    def execute(self, input_data: ToolInput) -> ToolOutput:
        ...
        include_relationships = input_data.parameters.get("include_relationships", False)
        ...
        if memory_tiers_str.lower() == "all":
            ...
            if invalid_tiers:
                ...
                print(f"Warning: Invalid memory tier names ignored: {', '.join(invalid_tiers)}. "
