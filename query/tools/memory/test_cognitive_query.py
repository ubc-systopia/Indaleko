...
def run_test(query: str, memory_tiers: str = "all", importance_min: float = 0.0, 
             w5h_filter: Optional[Dict[str, List[str]]] = None, 
             concept_filter: Optional[List[str]] = None,
             include_relationships: bool = False, limit: int = 10,
             db_config_path: Optional[str] = None, debug: bool = False) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        def progress_callback(progress_data):
            logger.info(f"Progress: {progress_data.stage} - {progress_data.message} ({progress_data.progress:.0%})")
        ...
        if result.success:
            ...
            for tier, stats in tier_stats.items():
                logger.info(f"  {tier}: {stats['count']} results, avg importance: {stats['avg_importance']:.2f}")
...
def main():
    ...
    parser.add_argument("--memory-tiers", type=str, default="all",
                       help="Memory tiers to include in the search (comma-separated list)")
    parser.add_argument("--importance-min", type=float, default=0.0,
                       help="Minimum importance score for results")
    parser.add_argument("--w5h-filter", type=str, default=None,
                       help="JSON string for W5H filter (e.g. '{\"what\": [\"document\"]}')")
    parser.add_argument("--concept-filter", type=str, default=None,
    ...
    parser.add_argument("--test-tier", type=str, default=None,
                      help="Test a specific memory tier only (sensory, short_term, long_term, archival)")
    parser.add_argument("--test-w5h", action="store_true",
                      help="Test W5H filtering with a sample filter")
    parser.add_argument("--test-concepts", action="store_true",
    ...
    if "error" in results:
        ...
        if result_list:
            ...
            for i, result in enumerate(result_list[:5]):  # Show top 5
                ...
                print(f"  {i+1}. [{memory_tier}] {file_path} (importance: {importance:.2f})")
