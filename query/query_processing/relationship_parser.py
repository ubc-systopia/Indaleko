...
class RelationshipParser:
    ...
    def __init__(
        ...
        enhanced_parser: Optional[EnhancedNLParser] = None,
    ):
        """
    ...
    def parse_relationship_query(self, query: str) -> RelationshipQuery:
        ...
        if relationship_type != RelationshipType.UNKNOWN:
            logging.info(f"Detected relationship type {relationship_type} using pattern matching")
            return self._build_relationship_query(relationship_type, entities, query, enhanced_understanding)
            
        # Otherwise, use the LLM for more complex analysis
        return self._extract_relationship_query_llm(query, enhanced_understanding)
    
    def _check_relationship_patterns(self, query: str) -> Tuple[RelationshipType, Optional[Dict[str, Any]]]:
        """
    ...
    def _extract_entities_from_match(self, match: re.Match, rel_type: RelationshipType) -> Dict[str, Any]:
        """
        ...
        if rel_type in (RelationshipType.CREATED, RelationshipType.MODIFIED, RelationshipType.VIEWED):
        ...
        elif rel_type in (RelationshipType.SHARED_WITH, RelationshipType.COLLABORATED_WITH):
    ...
    def _build_relationship_query(
        ...
        if relationship_type in (RelationshipType.CREATED, RelationshipType.MODIFIED, RelationshipType.VIEWED):
            # User-File relationship
        ...
        elif relationship_type in (RelationshipType.SHARED_WITH, RelationshipType.COLLABORATED_WITH):
            # User-User relationship
        ...
        if hasattr(enhanced_understanding, "constraints"):
            for constraint in enhanced_understanding.constraints:
                if constraint.field in ("timestamp", "date", "time", "created", "modified"):
    ...
    def _extract_relationship_query_llm(self, query: str, enhanced_understanding: Any) -> RelationshipQuery:
        """
        ...
        context_data = {
            ...
            "enhanced_understanding": enhanced_understanding.model_dump() if hasattr(enhanced_understanding, "model_dump") else str(enhanced_understanding)
        }
