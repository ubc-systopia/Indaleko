...
class FileStatistics:
    ...
    def __init__(self, db_config: IndalekoDBConfig | None = None):
        ...
        self.db = self.db_config._arangodb
...
def display_report(report: dict[str, Any], display_buffer: list[str] = None) -> None:
    ...
    output(
        f"Files: {report['file_count']:,} ({report['file_count']/max(1, report['total_objects'])*100:.1f}%)",
    )
    output(
        f"Directories: {report['directory_count']:,} ({report['directory_count']/max(1, report['total_objects'])*100:.1f}%)",
    )
    ...
    output("\n=== Top 5 File Types ===")
    file_types = dict(
        sorted(report["file_types"].items(), key=lambda x: x[1], reverse=True)[:5],
    )
    for ext, count in file_types.items():
        output(
            f".{ext}: {count:,} files ({count/max(1, report['file_count'])*100:.1f}%)",
        )
