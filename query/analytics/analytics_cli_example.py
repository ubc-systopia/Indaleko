...
def run_analytics_examples(db_config: Optional[IndalekoDBConfig] = None):
    """
    ...
    if age_distribution:
        ...
        for item in age_distribution:
            print(f"{item['age_range']}: {item['count']:,} files, "
    ...
    if file_count > 0:
        ...
        if recent_ratio > 0.2:
            insights.append("High recent activity: Consider increasing backup frequency")
        elif recent_ratio < 0.05:
            insights.append("Low recent activity: Consider archive/backup storage for older data")
    ...
    if file_types:
        ...
        if file_count > 0 and doc_files / file_count > 0.5:
            insights.append("Document-heavy usage: Consider document management optimizations")
        ...
        if file_count > 0 and media_files / file_count > 0.5:
            insights.append("Media-heavy usage: Consider media management optimizations")
    
    # Storage insights
    if size_stats and size_stats['total_size'] > 500 * 1024 * 1024 * 1024:  # 500 GB
        insights.append("Large storage footprint: Consider storage optimization strategies")
