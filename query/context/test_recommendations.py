...
from query.context.recommendations.entity_relationship import EntityRelationshipRecommender
...
def test_activity_context_recommender():
    ...
    for i, suggestion in enumerate(suggestions):
        print(f"{i+1}. {suggestion.query_text} (confidence: {suggestion.confidence:.2f})")
        print(f"   Rationale: {suggestion.rationale}")
        print(f"   Source context: {suggestion.source_context.get('activity_type', 'unknown')}")
        print(f"   Tags: {suggestion.tags}")
        print()
...
def test_recommendation_engine():
    ...
    for i, recommendation in enumerate(recommendations):
        print(f"{i+1}. {recommendation.query_text} (confidence: {recommendation.confidence:.2f})")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test the recommendation engine components")
    parser.add_argument("--history", action="store_true", help="Test query history recommender")
    parser.add_argument("--activity", action="store_true", help="Test activity context recommender")
    parser.add_argument("--entity", action="store_true", help="Test entity relationship recommender")
    parser.add_argument("--temporal", action="store_true", help="Test temporal pattern recommender")
    parser.add_argument("--engine", action="store_true", help="Test recommendation engine")
