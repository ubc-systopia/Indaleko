...
def create_test_queries(provider: QueryActivityProvider, count: int = 5) -> List[uuid.UUID]:
...
def test_activity_provider(args: argparse.Namespace) -> None:
    ...
    for i in range(len(query_ids) - 1):
        rel_type = provider._detect_relationship(
            f"Query {i}", f"Query {i+1}"
        )
...
def test_integration(args: argparse.Namespace) -> None:
    ...
    # Compare paths
    for i, (query, query_id) in enumerate(zip(path, query_ids)):
...
def main():
    ...
    parser.add_argument("--provider", action="store_true", help="Test activity provider")
    parser.add_argument("--navigation", action="store_true", help="Test navigation")
    parser.add_argument("--relationship", action="store_true", help="Test relationship detection")
    parser.add_argument("--visualization", action="store_true", help="Test visualization")
    parser.add_argument("--integration", action="store_true", help="Test full integration")
    parser.add_argument("--all", action="store_true", help="Run all tests")
    parser.add_argument("--count", type=int, default=5, help="Number of queries to create/use")
    ...
    if not (args.provider or args.navigation or args.relationship or 
            args.visualization or args.integration or args.all):
        args.all = True
