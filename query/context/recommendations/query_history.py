...
class QueryHistoryRecommender(RecommendationProvider):
    ...
    def __init__(
        self,
        query_provider: QueryActivityProvider | None = None,
        ...
        self.relationship_detector = relationship_detector or QueryRelationshipDetector(
            debug=debug,
        )
    ...
    def _initialize_from_history(self) -> None:
        ...
        for activity in recent_activities:
            if activity.relationship_type and activity.previous_query_id:
                ...
                prev_activity = self.query_provider.get_query_activity(
    ...
    def _apply_pattern_to_query(self, query: str, pattern: str) -> str | None:
        """
        ...
        if action == "add_term" and len(parts) == 2:
            ...
            if common_term in self._extract_terms(
                query,
            ) and pivot_term not in self._extract_terms(query):
                # Add the pivot term
