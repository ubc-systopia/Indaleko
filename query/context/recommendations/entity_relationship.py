...
class EntityRelationshipRecommender(RecommendationProvider):
    ...
    def __init__(self, db_config=None, debug: bool = False):
        ...
        self._entity_types = {
            "person": {
                ...
                "confidence": 0.9,
                "relationships": ["shared_with", "created_by", "modified_by", "mentioned_in"]
            },
            "organization": {
                "templates": [
            ...
            "file": {
                ...
                "confidence": 0.9,
                "relationships": ["similar_to", "same_folder", "references", "version_of"]
            },
            "location": {
                "templates": [
    ...
    def _extract_entities_from_query(self, query: str) -> List[Dict[str, Any]]:
        ...
        date_indicators = ["yesterday", "today", "last week", "last month", "this month"]
    ...
    def _generate_suggestions_for_entity(
        ...
        entity_type = entity.get("type", "default")
        entity_config = self._entity_types.get(entity_type, self._entity_types["default"])
        ...
        for template in templates:
            try:
                ...
                suggestion = self.create_suggestion(
                    ...
                    tags=[f"entity_type:{entity_type}", f"entity:{entity.get('name', 'unknown')}"]
                )
        ...
        for related_entity in related_entities:
            ...
            related_config = self._entity_types.get(related_type, self._entity_types["default"])
            ...
            if relationship in relationships:
                for template in related_config["templates"][:2]:  # Limit to 2 templates for diversity
                    try:
                        ...
                        query_text = template.format(name=related_entity.get("name", "unknown"))
                        ...
                        suggestion = self.create_suggestion(
                            ...
                            relevance_factors={
                                "entity_confidence": related_entity.get("confidence", 0.5),
                                "relationship_strength": 0.8,  # Would be based on actual relationship data
                                "template_success": self._get_template_success_ratio(template)
                            },
                            tags=[f"entity_type:{related_type}", f"relationship:{relationship}"]
                        )
    ...
    def _ensure_entity_type_diversity(self, suggestions: List[QuerySuggestion]) -> List[QuerySuggestion]:
        """
    ...
    def update_from_feedback(
        ...
        self._logger.debug(f"Updated feedback for entity type {entity_type}, template: {template}")
        self._logger.debug(f"Entity type success: {self._entity_type_success.get(entity_type, 0)}, failure: {self._entity_type_failure.get(entity_type, 0)}")
        self._logger.debug(f"Template success: {self._template_success.get(template, 0)}, failure: {self._template_failure.get(template, 0)}")
...
def main():
    ...
    for i, suggestion in enumerate(suggestions):
        ...
        print(f"   Entity type: {suggestion.source_context.get('entity_type', 'unknown')}")
        if "relationship" in suggestion.source_context:
            print(f"   Relationship: {suggestion.source_context['relationship']} with {suggestion.source_context.get('source_entity', 'unknown')}")
