...
class ActivityContextRecommender(RecommendationProvider):
    ...
    def _get_recent_activities(self, activity_handle: uuid.UUID) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if self._context_service:
                ...
                try:
                    ...
                    if context_docs:
                        ...
                        if "Cursors" in context_doc:
                            for cursor in context_doc["Cursors"]:
                                provider_type = self._detect_provider_type(cursor.get("Provider", ""))
                    ...
                    self._logger.info(f"Retrieved {len(activities)} activities from context document")
                except Exception as e:
            ...
            try:
                ...
                self._logger.info(f"Retrieved {len(file_docs)} file activities from database")
            except Exception as e:
                self._logger.error(f"Error querying file activities: {e}")
            
            # If no activities found, return fallback simulated activities
            if not activities:
                self._logger.warning("No activities found in database, using fallback data")
    ...
    def _group_activities_by_type(self, activities: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
        """
            ...
            for template in mapping["templates"]:
                try:
                    ...
                    suggestion = self.create_suggestion(
                        ...
                        relevance_factors={
                            "recency": self._calculate_recency_score(attributes.get("timestamp")),
                            "attribute_coverage": len(all_attrs) / (len(mapping["required_attributes"]) + len(mapping["extracted_attributes"])) if mapping["required_attributes"] or mapping["extracted_attributes"] else 0.5,
    ...
    def _derive_attribute(self, attribute: str, available_attributes: Dict[str, Any]) -> Optional[str]:
        """
        ...
        if attribute == "file_type" and ("file_name" in available_attributes or "file_path" in available_attributes):
            file_name = available_attributes.get("file_name", available_attributes.get("file_path", ""))
            if "." in file_name:
    ...
    def _generate_default_suggestions(self, count: int) -> List[QuerySuggestion]:
        ...
        default_queries = [
            ...
            ("Find email attachments from last week", "Based on common information need"),
    ...
    def update_from_feedback(
        ...
        self._logger.debug(f"Updated template feedback: {template}, successes: {self._successful_templates.get(template, 0)}, failures: {self._failed_templates.get(template, 0)}")
