...
from query.memory.proactive_archivist import ProactiveArchivist, ProactiveSuggestion, SuggestionType, SuggestionPriority
...
class RecommendationArchivistIntegration:
    ...
    def __init__(
        ...
        self.recommendation_engine = recommendation_engine or RecommendationEngine(debug=self.debug)
    ...
    def register_commands(self):
        ...
        for cmd, handler in self.commands.items():
            self.cli.register_command(cmd, lambda args, cmd=cmd: self.handle_command(cmd, args))
        
        # Add help text
        self.cli.append_help_text("\nRecommendation Commands:")
        self.cli.append_help_text("  /recommend           - Show query recommendations")
        self.cli.append_help_text("  /rconfig             - Configure recommendation settings")
        self.cli.append_help_text("  /rstats              - Show recommendation statistics")
        self.cli.append_help_text("  /rfeedback           - Provide feedback on recommendations")
        self.cli.append_help_text("  /rtest               - Test recommendation generation")
    ...
    def show_recommendation_help(self, args):
        ...
        print("\nUsage Examples:")
        print("  /recommend                     - Show recommendations based on current context")
        print("  /recommend <query>             - Show recommendations for a specific query")
    ...
    def show_recommendations(self, args):
        ...
        if not self.settings["enabled"]:
            print("Recommendations are currently disabled. Use /rconfig enabled true to enable.")
            return
        ...
        self.last_recommendations_time = datetime.now(timezone.utc)
    ...
    def _prepare_context_data(self) -> Dict[str, Any]:
        ...
        if (self.proactive and hasattr(self.proactive, "cross_source_detector") and 
            self.proactive.cross_source_detector is not None):
    ...
    def _display_recommendations(self, recommendations: List[QuerySuggestion]):
        ...
        for i, recommendation in enumerate(recommendations, 1):
            ...
            print(f"   Source: {recommendation.source.value}, Confidence: {recommendation.confidence:.2f}")
    ...
    def _convert_to_proactive_suggestions(self, recommendations: List[QuerySuggestion]):
        ...
        for recommendation in recommendations:
            suggestion_type = source_to_type.get(recommendation.source, SuggestionType.QUERY)
            ...
            existing_queries = [
                q for s in self.proactive.data.active_suggestions 
    ...
    def show_recommendation_stats(self, args):
        ...
        print(f"Total suggestions: {sum(self.recommendation_engine.suggestion_counts.values())}")
        ...
        for source, count in self.recommendation_engine.suggestion_counts.items():
            if count > 0:
                ...
                print(f"  {source.value}: {count} suggestions, {acceptance} accepted ({rate:.2f})")
    ...
    def provide_recommendation_feedback(self, args):
        ...
        try:
            ...
            self.recommendation_engine.record_feedback(suggestion.suggestion_id, feedback)
            
            # Confirm to user
            if feedback_type == "accept":
                print(f"Thanks for accepting the recommendation! I'll show more like this.")
                ...
                print(f"Thanks for the feedback. I'll show fewer recommendations like this.")
    ...
    def test_recommendations(self, args):
        ...
        if args:
            try:
                ...
                print(f"Valid sources: {', '.join(s.value for s in RecommendationSource)}")
                return
        ...
        if all_recommendations:
            ...
            ranked = sorted(all_recommendations, key=lambda x: x.confidence, reverse=True)
    ...
    def update_context_with_query(self, query_text, results=None):
        ...
        if topics:
            self.context["current_topics"] = list(set(self.context["current_topics"] + topics))
    ...
    def _extract_topics_from_query(self, query_text):
        ...
        common_topics = [
            "work", "personal", "finance", "project", "report", "presentation",
            "email", "document", "photo", "video", "music", "code"
        ]
    ...
    def check_show_recommendations(self, query_text=None):
        ...
        if (self.last_recommendations_time and
            (now - self.last_recommendations_time).total_seconds() < self.recommendation_cooldown_minutes * 60):
            return False
    ...
    def add_recommendation_insights_to_archivist(self):
        ...
        for source_type, count in self.recommendation_engine.suggestion_counts.items():
            if count >= 10:  # Only add insights if we have sufficient data
                acceptance = self.recommendation_engine.acceptance_counts.get(source_type, 0)
