...
class QueryRelationshipDetector:
    ...
    def __init__(self, db_config=None, debug=False, use_llm=False):
        ...
        if use_llm:
            try:
                ...
                self._logger.info("Initialized LLM connector for relationship detection")
    ...
    def detect_relationship(
        self, 
        query1: Union[str, uuid.UUID, Dict[str, Any]],
        query2: Union[str, uuid.UUID, Dict[str, Any]]
    ...
    def _get_query_text(self, query: Union[str, uuid.UUID, Dict[str, Any]]) -> Optional[str]:
        """
        Extract query text from various input types.
    ...
    def _detect_relationship_rules(
        ...
        for pattern in refinement_patterns:
            if not re.search(pattern, q1, re.IGNORECASE) and re.search(pattern, q2, re.IGNORECASE):
    ...
    def _detect_relationship_llm(
        ...
        self._logger.warning("LLM-based detection not implemented, falling back to rules")
        return self._detect_relationship_rules(query1, query2)
    
    def detect_relationship_batch(
        self,
        queries: List[Union[str, uuid.UUID, Dict[str, Any]]]
