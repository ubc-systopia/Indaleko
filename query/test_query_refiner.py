    ...
    file_type_facet = Facet(
        ...
        values=[
            ...
            FacetValue(
                value="xlsx",
                count=5,
                query_refinement="file_type:xlsx"
            )
        ],
        coverage=0.85,
    ...
    date_facet = Facet(
        ...
        values=[
            ...
            FacetValue(
                value="Last year",
                count=8,
                query_refinement="date:\"last year\""
            )
        ],
        coverage=0.90,
    ...
    author_facet = Facet(
        ...
        values=[
            ...
            FacetValue(
                value="Jane Doe",
                count=7,
                query_refinement="author:\"Jane Doe\""
            ),
            FacetValue(
                value="Alice Johnson",
                count=5,
    ...
    location_facet = Facet(
        ...
        values=[
            ...
            FacetValue(
                value="desktop",
                count=12,
                query_refinement="location:desktop"
            )
        ],
        coverage=0.95,
...
def main():
    ...
    test_cases = [
        {"name": "Add file type refinement", "facet": facets.facets[0], "value": facets.facets[0].values[0]},
        {"name": "Add date refinement", "facet": facets.facets[1], "value": facets.facets[1].values[0]},
        {"name": "Add author refinement", "facet": facets.facets[2], "value": facets.facets[2].values[0]},
        {"name": "Remove file type refinement", "remove": (facets.facets[0].name, facets.facets[0].values[0].value)},
        {"name": "Add location refinement", "facet": facets.facets[3], "value": facets.facets[3].values[0]},
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        if "facet" in test_case and "value" in test_case:
            # Apply a refinement
            refined_query, state = refiner.apply_refinement(test_case["facet"], test_case["value"])
            print(f"Refined query: {refined_query}")
            print("Active refinements:")
            for j, refinement in enumerate(state.active_refinements, 1):
                print(f"  {j}. {refinement.facet_name}: {refinement.value} → {refinement.query_fragment}")
