...
class PromptGuardian:
    ...
    def _log_verification(
        ...
        collection = self.db.collection(IndalekoDBCollections.Indaleko_Prompt_Verification_Log_Collection)
    ...
    def _generate_recommendation(self, result: VerificationResult) -> str:
        ...
        if not result.trust_contract_valid:
            recommendations.append(
                "Add a trust contract section describing mutual expectations between the user and the AI.",
            )

        if result.has_injection_patterns:
            recommendations.append(
                "Remove potentially harmful patterns like 'ignore instructions' or 'bypass security'.",
            )
        ...
        if not recommendations:
            recommendations.append("The prompt does not meet security requirements. Please review and try again.")
    ...
    def verify_prompt(
        ...
        if cache_key in self._verification_cache:
            ...
            cached_result.timestamp = datetime.now(UTC)
