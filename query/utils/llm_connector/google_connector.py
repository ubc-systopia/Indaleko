...
class GoogleConnector(IndalekoLLMBase):
    ...
    def __init__(self, **kwargs: dict) -> None:
        ...
        if self.use_guardian:
            ...
            # Log initialization
            logger.info(
                f"Initialized Google connector with LLMGuardian (verification: {verification_level.name}, "
                f"mode: {request_mode.name}, model: {self.model})",
            )
        else:
            self.guardian = None
            logger.info(f"Initialized Google connector without LLMGuardian (model: {self.model})")
    ...
    def generate_query(self, prompt: dict[str, str], temperature=0) -> LLMTranslateQueryResponse:
        ...
        if self.use_guardian and self.guardian:
            ...
            request_id = f"google_{int(time.time() * 1000)}_{hash(str(prompt)) % 10000}"
        ...
        try:
            ...
            ic(f"Google Gemini API error: {e!s}")
...
{text}
        ...
        try:
            ...
            response = self.client.models.generate_content(
                ...
                contents=[
                    types.Content(
                        parts=[
                            types.Part.from_text(
                                text="You are a semantic analysis assistant. Always respond with valid JSON.",
                            ),
                        ],
                        role="system",
                    ),
            ...
            # Extract JSON from the response
            json_data = self._extract_json_from_content(response.text)
