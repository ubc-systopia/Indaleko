...
class DeepseekConnector(IndalekoLLMBase):
    ...
    def __init__(self, **kwargs: dict) -> None:
        ...
        if self.use_guardian:
            ...
            # Log initialization
            logger.info(
                f"Initialized Deepseek connector with LLMGuardian (verification: {verification_level.name}, "
                f"mode: {request_mode.name}, model: {self.model})",
            )
        else:
            self.guardian = None
            logger.info(f"Initialized Deepseek connector without LLMGuardian (model: {self.model})")
    ...
    def generate_query(self, prompt: dict[str, str], temperature=0) -> LLMTranslateQueryResponse:
        ...
        if self.use_guardian and self.guardian:
            ...
            request_id = f"deepseek_{int(time.time() * 1000)}_{hash(str(prompt)) % 10000}"
