...
class ResultGroup(IndalekoBaseModel):
    ...
    primary: Dict[str, Any] = Field(..., description="The primary (representative) result")
    duplicates: List[Dict[str, Any]] = Field(default_factory=list, description="Duplicate results")
    similarity_scores: List[float] = Field(default_factory=list, description="Similarity scores of duplicates to primary")
    last_modified: Optional[datetime] = Field(default=None, description="Last modification time of primary item")
    item_count: int = Field(default=1, description="Total number of items in this group")
...
class FormattedResults(IndalekoBaseModel):
    ...
    result_groups: List[ResultGroup] = Field(default_factory=list, description="Groups of similar results")
    original_count: int = Field(default=0, description="Original number of results before deduplication")
    unique_count: int = Field(default=0, description="Number of unique results after deduplication")
    suppressed_count: int = Field(default=0, description="Number of suppressed duplicate results")
    summary: str = Field(default="", description="Summary of the results")
    query_time: Optional[float] = Field(default=None, description="Query execution time in seconds")
    categories: Dict[str, int] = Field(default_factory=dict, description="Counts of result categories")
...
def extract_timestamp(result: Dict[str, Any]) -> Optional[datetime]:
    """
...
def deduplicate_results(
    ...
    timed_results.sort(key=lambda x: (x[1] is None, x[1] if x[1] is not None else datetime.min), reverse=True)
    ...
    for i in remaining:
        ...
        for j in remaining:
            if j != i and j not in processed_indices:
                ...
                is_same, score = resolve_indaleko_objects(result, other_result, threshold=similarity_threshold)
    ...
    for primary_idx, group_indices in exact_groups.items():
        ...
        for idx in group_indices:
            if idx != primary_idx:
                ...
                _, score = resolve_indaleko_objects(primary_result, dup_result, threshold=0.0)
...
def format_result_for_display(result: Dict[str, Any], include_details: bool = True) -> str:
    """
...
def format_result_group_for_display(group: ResultGroup, include_duplicates: bool = True) -> str:
    """
    ...
    if group.duplicates:
        ...
        if include_duplicates:
            for i, (duplicate, score) in enumerate(zip(group.duplicates, group.similarity_scores), 1):
                formatted.append(f"\n  Duplicate {i} (similarity: {score:.2f}):")
                
                # Format with less detail for duplicates
                dup_formatted = format_result_for_display(duplicate, include_details=False)
    ...
    if include_summary:
        ...
        if formatted_results.categories:
            ...
            if len(sorted_categories) > 5:
                display.append(f"  ... and {len(sorted_categories) - 5} more categories")
