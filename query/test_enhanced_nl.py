...
from query.query_processing.data_models.query_language_enhancer import EnhancedQueryUnderstanding
...
def process_query(
    ...
    translated_query = aql_translator.translate_enhanced(query_understanding, query_data)
    ...
    if query_understanding.intent.secondary_intents:
        print(f"Secondary Intents: {', '.join(query_understanding.intent.secondary_intents)}")
    ...
    if query_understanding.entities:
        ...
        for entity in query_understanding.entities:
            print_color(f"- {entity.original_text} → {entity.normalized_value} ({entity.entity_type})", "green")
    
    # Display constraints
    if query_understanding.constraints:
        print_section("Query Constraints")
        for constraint in query_understanding.constraints:
            print_color(f"- {constraint.field} {constraint.operation} {constraint.value}", "yellow")
    ...
    if hasattr(aql_translator, 'performance_hints') and aql_translator.performance_hints:
        ...
        for hint in aql_translator.performance_hints:
            if hint.severity == "warning":
                print_color(f"- {hint.description} ({hint.affected_component})", "yellow")
    ...
    if query_understanding.context:
        ...
        if query_understanding.context.temporal_context:
            ...
            print(f"Temporal Context: {temp.time_field} {temp.start_time or ''} - {temp.end_time or ''}")
    ...
    if execute and "execution_results" in result:
        ...
        for i, res in enumerate(result["execution_results"]):
            ...
            if isinstance(res, dict) and "Label" in res:
                ...
                # Just print the first few keys and values for readability
