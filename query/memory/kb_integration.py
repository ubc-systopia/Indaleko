...
def initialize_kb_for_cli(
    cli_instance: IndalekoBaseCLI,
) -> KnowledgeBaseCliIntegration | None:
    """
    ...
    try:
        ...
        logging.exception(f"Failed to initialize Knowledge Base integration: {e!s}")
        return None


def enhance_query_with_kb(
    cli_instance: IndalekoBaseCLI,
    query_text: str,
    intent: str = "",
...
def record_query_results(
    ...
    try:
        ...
        logging.exception(f"Error recording query results with Knowledge Base: {e!s}")
