...
def run_demo():
    ...
    results = {
        ...
        "patterns": [p.model_dump() for p in analyzer.data.query_patterns] if hasattr(analyzer.data, "query_patterns") else [],
        "chains": [c.model_dump() for c in analyzer.data.query_chains] if hasattr(analyzer.data, "query_chains") else [],
        "entity_usage": {k: v.model_dump() for k, v in analyzer.data.entity_usage.items()} if hasattr(analyzer.data, "entity_usage") else {},
