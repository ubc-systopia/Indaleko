...
def generate_synthetic_events(count=1000, time_span_days=30, correlation_ratio=0.3):
    ...
    common_entities = [
        "project_indaleko", "database_schema", "query_system",
        "documentation", "file_analyzer", "performance_tests",
        "user_interface", "api_gateway", "config_manager"
    ]
    ...
    for i in range(uncorrelated_count):
        ...
        entities = random.sample(common_entities, min(entity_count, len(common_entities)))
    ...
    for group in range(correlation_groups):
        ...
        random_seconds = random.randint(0, (time_span_days-1) * 86400)  # Leave room for correlated events
        ...
        if pattern["shared_entities"]:
            group_entities = random.sample(common_entities, min(3, len(common_entities)))
        else:
            group_entities = []
        ...
        for i, source_type in enumerate(pattern["source_types"]):
            ...
            event = CrossSourceEvent(
                ...
                event_type=event_type,
                entities=entities,
                importance=random.uniform(0.3, 0.9)  # Correlated events tend to be more important
            )
    ...
    try:
        ...
        for i, source_type in enumerate(source_types):
            ...
            if events:
                ...
                plt.scatter(timestamps, y_values, color=colors[i], alpha=0.7, label=source_type.value)
...
def benchmark_performance(args):
    ...
    for count in event_counts:
        ...
        print(f"Correlation detection: {correlation_time:.2f}s, {len(correlations)} correlations found")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test the enhanced cross-source pattern detection system")
    parser.add_argument("--synthetic", action="store_true", help="Test with synthetic data")
    parser.add_argument("--benchmark", action="store_true", help="Benchmark performance with different event counts")
    parser.add_argument("--visualize", action="store_true", help="Visualize detection results")
    parser.add_argument("--event-count", type=int, default=1000, help="Number of synthetic events to generate")
    parser.add_argument("--time-span", type=int, default=30, help="Time span for synthetic events (days)")
    parser.add_argument("--correlation-ratio", type=float, default=0.3, help="Ratio of correlated events")
    ...
    parser.add_argument("--min-entity-overlap", type=float, default=0.0, help="Minimum entity overlap for correlations")
    parser.add_argument("--adaptive-window", action="store_true", help="Use adaptive time windows for correlations")
