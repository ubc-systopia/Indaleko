...
def test_create_memory():
    ...
    memory.add_insight("retrieval", "Location-based search helps narrow down results", 0.7)
    
    # Update preferences
    memory._add_or_update_preference("search", "Prefers detailed results over summaries", 0.85)
...
def verify_collection():
    ...
    if not db_config.db.has_collection(collection_name):
        print(f"Error: Collection {collection_name} does not exist.")
        return False
    
    # Get collection
    collection = db_config.db.collection(collection_name)
    ...
    if count > 0:
        ...
        if documents:
            ...
            if goals:
                print("\n  Goals:")
                for goal in goals:
                    print(f"    - {goal.get('name')}: {goal.get('progress', 0) * 100:.0f}% complete")
            ...
            if insights:
                ...
                for insight in insights:
                    print(f"    - {insight.get('insight')} (confidence: {insight.get('confidence', 0):.2f})")
...
def test_load_memory():
    ...
    if memory.memory.semantic_topics:
        print("\nTopics:")
        for topic, importance in sorted(memory.memory.semantic_topics.items(), key=lambda x: x[1], reverse=True):
...
def list_collections():
    ...
    # Get collections
    collections = db_config.db.collections()
    ...
    for collection in sorted(user_collections, key=lambda x: x["name"]):
        ...
        count = db_config.db.collection(name).count()
...
def main():
    ...
    parser.add_argument("--create", action="store_true", help="Create and save test memory data")
    parser.add_argument("--verify", action="store_true", help="Verify the Archivist memory collection")
    parser.add_argument("--load", action="store_true", help="Load memory from the database")
    parser.add_argument("--list", action="store_true", help="List all collections in the database")
