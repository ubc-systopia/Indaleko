...
class SuggestionHistory(BaseModel):
    ...
    suggestions: List[ProactiveSuggestion] = Field(default_factory=list, description="All suggestions generated")
    positive_interactions: Dict[SuggestionType, int] = Field(default_factory=dict, description="Count of positive interactions by type")
    negative_interactions: Dict[SuggestionType, int] = Field(default_factory=dict, description="Count of negative interactions by type")
    
    def record_interaction(self, suggestion: ProactiveSuggestion, feedback_value: float) -> None:
        """
...
class TemporalPattern(BaseModel):
    ...
    pattern_type: str = Field(..., description="Type of temporal pattern (e.g., 'daily', 'weekly', 'monthly')")
    description: str = Field(..., description="Description of the pattern")
    confidence: float = Field(default=0.5, description="Confidence in this pattern (0.0-1.0)")
    timeframe: Dict[str, Any] = Field(..., description="Timeframe specification for the pattern")
    associated_actions: List[str] = Field(default_factory=list, description="Actions associated with this pattern")
...
class ProactiveArchivistData(BaseModel):
    ...
    suggestion_history: SuggestionHistory = Field(default_factory=SuggestionHistory, description="History of suggestions and interactions")
    active_suggestions: List[ProactiveSuggestion] = Field(default_factory=list, description="Currently active suggestions")
    temporal_patterns: List[TemporalPattern] = Field(default_factory=list, description="Detected temporal patterns")
    sequential_patterns: Dict[str, List[str]] = Field(default_factory=dict, description="Detected sequential patterns in queries")
    context_triggers: Dict[str, List[str]] = Field(default_factory=dict, description="Context triggers for proactive suggestions")
    suggestion_thresholds: Dict[SuggestionType, float] = Field(default_factory=dict, description="Confidence thresholds for different suggestion types")
    cross_source_enabled: bool = Field(default=True, description="Whether cross-source pattern detection is enabled")
    last_cross_source_analysis: Optional[datetime] = Field(default=None, description="When cross-source patterns were last analyzed")
...
class ProactiveArchivist:
    ...
    def cross_source_detector(self):
        ...
        if self._cross_source_detector is None:
            ...
            self._cross_source_detector = CrossSourcePatternDetector(self.archivist.db_config)
        return self._cross_source_detector
        
    def generate_suggestions(self, context: Optional[Dict[str, Any]] = None) -> List[ProactiveSuggestion]:
        """
        ...
        if self.data.cross_source_enabled and self.data.last_cross_source_analysis:
            ...
            cross_source_suggestions = [
                s for s in self.data.active_suggestions 
        ...
        filtered_suggestions = [
            ...
            if s.confidence >= self.data.suggestion_thresholds.get(s.suggestion_type, 0.7)
        ]
    ...
    def _generate_goal_based_suggestions(self, context: Optional[Dict[str, Any]] = None) -> List[ProactiveSuggestion]:
        """
        ...
        for goal in self.archivist.memory.long_term_goals:
            ...
            if goal.last_updated < datetime.now(timezone.utc) - timedelta(days=14):
                confidence = min(0.5 + (0.1 * (datetime.now(timezone.utc) - goal.last_updated).days / 7), 0.9)
    ...
    def _generate_topic_based_suggestions(self, context: Optional[Dict[str, Any]] = None) -> List[ProactiveSuggestion]:
        """
    ...
    def _generate_queries_for_topic(self, topic: str) -> List[str]:
        ...
        return topic_query_templates.get(topic.lower(), [
            f"recent {topic} files",
            f"important {topic} documents",
            f"{topic} information from last month"
        ])
    
    def _generate_temporal_pattern_suggestions(self, context: Optional[Dict[str, Any]] = None) -> List[ProactiveSuggestion]:
        """
    ...
    def _generate_strategy_suggestions(self, context: Optional[Dict[str, Any]] = None) -> List[ProactiveSuggestion]:
        """
        ...
        effective_strategies = [
            s for s in self.archivist.memory.effective_strategies 
    ...
    def detect_temporal_patterns(self, query_history) -> None:
        ...
        queries = query_history.get_recent_queries(30)  # Get more history for pattern detection
    ...
    def _add_or_update_temporal_pattern(self, pattern_type, description, confidence, timeframe):
        ...
        # Add new pattern
        self.data.temporal_patterns.append(TemporalPattern(
        ...
        for pattern in self.data.temporal_patterns:
            if pattern.confidence > 0.7:
                if pattern.pattern_type == "daily":
                    ...
                    day = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"][pattern.timeframe["day_of_week"]]
    ...
    def record_user_feedback(self, suggestion_id: str, feedback_value: float) -> None:
        ...
        for suggestion in self.data.active_suggestions:
            if suggestion.suggestion_id == suggestion_id:
                ...
                # Update history
                self.data.suggestion_history.record_interaction(suggestion, feedback_value)
                ...
                suggestion_type = suggestion.suggestion_type
                current_threshold = self.data.suggestion_thresholds.get(suggestion_type, 0.7)
                
                if feedback_value > 0:
                    # Slightly lower threshold for types with positive feedback
                    self.data.suggestion_thresholds[suggestion_type] = max(0.5, current_threshold - 0.05)
                elif feedback_value < 0:
                    # Slightly raise threshold for types with negative feedback
                    self.data.suggestion_thresholds[suggestion_type] = min(0.9, current_threshold + 0.05)
                
                break
    
    def get_suggested_query(self, context: Optional[Dict[str, Any]] = None) -> Optional[str]:
        """
    ...
    def analyze_session(self, query_history, context=None) -> None:
        ...
        if self.data.cross_source_enabled:
            ...
            if (self.data.last_cross_source_analysis is None or 
                (now - self.data.last_cross_source_analysis).total_seconds() > 24 * 60 * 60):
                self.analyze_cross_source_patterns()
    ...
    def analyze_cross_source_patterns(self) -> None:
        ...
        try:
            ...
            ic(f"Cross-source analysis: Found {len(patterns)} patterns, {len(correlations)} correlations")
            ...
            for suggestion in suggestions:
                ...
                if proactive_suggestion.confidence >= self.data.suggestion_thresholds.get(proactive_suggestion.suggestion_type, 0.7):
            ...
            for pattern in patterns:
                if pattern.confidence >= 0.65:  # Only add high-confidence patterns as insights
                    ...
                    self.archivist.add_insight(insight_category, insight_text, pattern.confidence)
            ...
            for correlation in correlations:
                if correlation.confidence >= 0.7:  # Only add high-confidence correlations as insights
                    ...
                    self.archivist.add_insight(insight_category, insight_text, correlation.confidence)
            
            # Update last analysis timestamp
            self.data.last_cross_source_analysis = datetime.now(timezone.utc)
...
def main():
    ...
    # Add some test data
    archivist.add_long_term_goal("File Organization", "Organize personal documents by project and year")
    archivist.update_goal_progress("File Organization", 0.35)
    
    archivist.add_insight("organization", "User struggles with finding documents older than 6 months", 0.8)
    archivist.add_insight("retrieval", "Location data is highly valuable for narrowing searches", 0.7)
