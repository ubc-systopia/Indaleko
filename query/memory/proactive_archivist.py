...
class ProactiveSuggestion(BaseModel):
    ...
    suggestion_id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="Unique identifier for this suggestion")
    ...
    created_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), description="When this suggestion was created")
    expires_at: Optional[datetime] = Field(None, description="When this suggestion expires (if applicable)")
    priority: SuggestionPriority = Field(default=SuggestionPriority.MEDIUM, description="Priority level of the suggestion")
    confidence: float = Field(default=0.5, description="Confidence in this suggestion (0.0-1.0)")
    context: Dict[str, Any] = Field(default_factory=dict, description="Contextual information related to this suggestion")
    related_queries: List[str] = Field(default_factory=list, description="Queries related to this suggestion")
    dismissed: bool = Field(default=False, description="Whether this suggestion has been dismissed")
    acted_upon: bool = Field(default=False, description="Whether the user has acted upon this suggestion")
    feedback: Optional[float] = Field(None, description="User feedback on this suggestion (-1.0 to 1.0)")
...
class ProactiveArchivist:
    ...
    def __init__(self, archivist_memory: ArchivistMemory):
        ...
        # Initialize cross-source pattern detector
        self.cross_source_detector = CrossSourcePatternDetector(self.archivist.db_config)
