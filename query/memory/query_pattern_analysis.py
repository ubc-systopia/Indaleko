...
from query.memory.cross_source_patterns import CrossSourcePatternDetector, CrossSourceEvent
from query.history.data_models.query_history import IndalekoQueryHistoryDataModel, QueryHistoryData
...
class QueryEntityUsage(BaseModel):
    ...
    mention_count: int = Field(default=0, description="Number of times this entity appears in queries")
    first_seen: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), description="When this entity was first seen")
    last_seen: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), description="When this entity was last seen")
    co_occurring_entities: Dict[str, int] = Field(default_factory=dict, description="Entities that co-occur with this one")
    intents: Dict[str, int] = Field(default_factory=dict, description="Intents associated with this entity")
    success_rate: float = Field(default=0.0, description="Percentage of successful queries with this entity")
    query_examples: List[str] = Field(default_factory=list, description="Example queries containing this entity")
...
class QuerySequence(BaseModel):
    ...
    sequence_id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="Unique identifier for this sequence")
    sequence_signature: str = Field(..., description="Signature identifying this query sequence")
    queries: List[str] = Field(default_factory=list, description="IDs of queries in this sequence")
    timestamps: List[datetime] = Field(default_factory=list, description="Timestamps of queries in sequence")
    frequency: int = Field(default=1, description="Number of times this sequence has been observed")
    avg_time_between: Optional[float] = Field(None, description="Average time between queries in seconds")
    success_rate: float = Field(default=0.0, description="Percentage of successful queries in this sequence")
    common_entities: List[str] = Field(default_factory=list, description="Entities common across queries")
    refinement_pattern: Optional[str] = Field(None, description="Pattern of query refinement if present")
    is_refinement: bool = Field(default=False, description="Whether this is a refinement sequence")
    attributes: Dict[str, Any] = Field(default_factory=dict, description="Additional attributes")
...
class QueryPattern(BaseModel):
    ...
    pattern_type: str = Field(..., description="Type of pattern (time-based, entity-based, etc.)")
    ...
    first_observed: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), description="When this pattern was first observed")
    ...
    supporting_evidence: List[str] = Field(default_factory=list, description="Query IDs supporting this pattern")
    entities_involved: List[str] = Field(default_factory=list, description="Entities involved in this pattern")
    intents_involved: List[str] = Field(default_factory=list, description="Intents involved in this pattern")
    temporal_factors: Dict[str, Any] = Field(default_factory=dict, description="Temporal factors in this pattern")
    attributes: Dict[str, Any] = Field(default_factory=dict, description="Additional attributes")
...
class QueryChain(BaseModel):
    ...
    chain_id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="Unique identifier for this chain")
    chain_type: QueryChainType = Field(..., description="Type of query chain")
    queries: List[str] = Field(default_factory=list, description="IDs of queries in this chain")
    query_texts: List[str] = Field(default_factory=list, description="Texts of queries in this chain")
    start_time: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), description="When this chain started")
    end_time: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), description="When this chain ended")
    duration: float = Field(default=0.0, description="Duration of the chain in seconds")
    shared_entities: List[str] = Field(default_factory=list, description="Entities shared across the chain")
    transition_patterns: List[Dict[str, Any]] = Field(default_factory=list, description="Patterns in transitions between queries")
    outcome: Optional[str] = Field(None, description="Final outcome of the chain")
    success_rate: float = Field(default=0.0, description="Success rate of queries in the chain")
    attributes: Dict[str, Any] = Field(default_factory=dict, description="Additional attributes")
...
class QueryRefinement(BaseModel):
    ...
    refinement_id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="Unique identifier for this refinement")
    ...
    similarity: float = Field(default=0.0, description="Similarity measure between the queries")
    added_terms: List[str] = Field(default_factory=list, description="Terms added in the refinement")
    removed_terms: List[str] = Field(default_factory=list, description="Terms removed in the refinement")
    added_entities: List[str] = Field(default_factory=list, description="Entities added in the refinement")
    removed_entities: List[str] = Field(default_factory=list, description="Entities removed in the refinement")
    attributes: Dict[str, Any] = Field(default_factory=dict, description="Additional attributes")
...
class UserQueryHistoryMetrics(BaseModel):
    ...
    time_period: str = Field(default="all_time", description="Time period for these metrics")
    start_date: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), description="Start of the time period")
    end_date: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), description="End of the time period")
    total_queries: int = Field(default=0, description="Total number of queries")
    
    # Query success metrics
    successful_queries: int = Field(default=0, description="Number of queries with results")
    empty_result_queries: int = Field(default=0, description="Number of queries with no results")
    success_rate: float = Field(default=0.0, description="Percentage of successful queries")
    
    # Query complexity metrics
    avg_query_length: float = Field(default=0.0, description="Average length of queries in characters")
    avg_entity_count: float = Field(default=0.0, description="Average number of entities per query")
    
    # Temporal metrics
    queries_by_hour: Dict[int, int] = Field(default_factory=dict, description="Distribution of queries by hour")
    queries_by_day: Dict[int, int] = Field(default_factory=dict, description="Distribution of queries by day of week")
    
    # Content metrics
    top_entities: Dict[str, int] = Field(default_factory=dict, description="Most frequently queried entities")
    top_intents: Dict[str, int] = Field(default_factory=dict, description="Most frequent query intents")
    
    # Chain metrics
    avg_chain_length: float = Field(default=0.0, description="Average number of queries in a chain")
    refinement_rate: float = Field(default=0.0, description="Rate of query refinement")
    
    # Performance metrics
    avg_query_time: float = Field(default=0.0, description="Average query execution time in seconds")
    max_query_time: float = Field(default=0.0, description="Maximum query execution time in seconds")


class QueryAssociationRule(BaseModel):
    """An association rule between query elements."""
    
    rule_id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="Unique identifier for this rule")
    antecedent: List[str] = Field(..., description="Antecedent elements")
    consequent: List[str] = Field(..., description="Consequent elements")
    support: float = Field(..., description="Support for this rule (frequency of antecedent and consequent together)")
    confidence: float = Field(..., description="Confidence in this rule (frequency of consequent given antecedent)")
    lift: float = Field(..., description="Lift of this rule (how much above chance)")
    conviction: Optional[float] = Field(None, description="Conviction of this rule")
    rule_type: str = Field(..., description="Type of rule (entity, intent, etc.)")
    observed_count: int = Field(default=0, description="Number of times this rule has been observed")
    examples: List[Dict[str, Any]] = Field(default_factory=list, description="Example occurrences of this rule")
...
class QueryPatternAnalysisData(IndalekoBaseModel):
    ...
    queries: Dict[str, Dict[str, Any]] = Field(default_factory=dict, description="Query history records")
    query_timeline: List[Tuple[str, datetime]] = Field(default_factory=list, description="Chronological timeline of queries")
    
    # Entity analysis
    entity_usage: Dict[str, QueryEntityUsage] = Field(default_factory=dict, description="Analysis of entity usage")
    
    # Pattern detection
    query_patterns: List[QueryPattern] = Field(default_factory=list, description="Detected query patterns")
    query_sequences: List[QuerySequence] = Field(default_factory=list, description="Detected query sequences")
    
    # Query chains and refinements
    query_chains: List[QueryChain] = Field(default_factory=list, description="Detected query chains")
    query_refinements: List[QueryRefinement] = Field(default_factory=list, description="Detected query refinements")
    
    # Association rules
    association_rules: List[QueryAssociationRule] = Field(default_factory=list, description="Association rules between query elements")
    
    # User metrics
    user_metrics: Dict[str, UserQueryHistoryMetrics] = Field(default_factory=dict, description="Metrics on user query behavior")
    
    # Cross-source integration
    cross_source_events: List[str] = Field(default_factory=list, description="IDs of related cross-source events")
    
    # System metadata
    last_update: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), description="When this data was last updated")
...
class QueryPatternAnalyzer:
    ...
    def load_query_history(self, max_queries: int = 1000, days_back: int = 30) -> int:
        ...
        if not self.db_config or not self.db_config.db:
            self.logger.warning("No database connection available for loading query history")
            return 0
        ...
        try:
            ...
            for doc in cursor:
                try:
                    ...
                    self.logger.error(f"Error processing query {doc.get('_key', 'unknown')}: {e}")
    ...
    def _process_query_entities(self, query_id: str, query_data: Dict[str, Any]) -> None:
        """
        ...
        try:
            ...
            for entity_name in entities:
                if entity_name not in self.data.entity_usage:
                    ...
                    successful_queries = int(entity_usage.success_rate * (total_queries - 1))
    ...
    def analyze_query_chains(self) -> List[QueryChain]:
        ...
        for i in range(1, len(timeline)):
            ...
            refinement_type = self._determine_refinement_type(prev_text, curr_text, prev_entities, curr_entities)
        ...
        for chain in current_chains:
            if len(chain.queries) > 1:
                last_id, last_time = next((id_time for id_time in timeline if id_time[0] == chain.queries[-1]), (None, None))
                if last_time:
    ...
    def _determine_refinement_type(
        ...
        temporal_terms = ["today", "yesterday", "last week", "month", "year", "before", "after", "between"]
    ...
    def _detect_temporal_patterns(self) -> List[QueryPattern]:
        ...
        for hour, query_ids in hour_counts.items():
            ...
            if dominant_entities or dominant_intents:
                ...
                confidence = min(0.5 + (len(query_ids) / 20), 0.95)  # Scales up with more observations
    ...
    def _detect_refinement_patterns(self) -> List[QueryPattern]:
        ...
        for chain in refinement_chains:
            for transition in chain.transition_patterns:
                if "refinement_type" in transition and transition["refinement_type"]:
        ...
        for ref_type, count in refinement_type_counts.items():
            ...
            success_rates = [chain.success_rate for chain in refinement_chains 
                           if any(t.get("refinement_type") == ref_type for t in chain.transition_patterns)]
            
            if success_rates:
                avg_success = sum(success_rates) / len(success_rates)
                confidence = min(0.5 + (count / 10) * 0.4, 0.9)  # Scale confidence with observation count
                
                if avg_success > 0.7:
                    description += f", with high success rate ({avg_success:.1%})"
                    confidence = min(confidence + 0.1, 0.95)  # Boost confidence for successful patterns
            else:
    ...
    def calculate_metrics(self) -> UserQueryHistoryMetrics:
        ...
        metrics = UserQueryHistoryMetrics(
            ...
            start_date=self.data.query_timeline[0][1] if self.data.query_timeline else datetime.now(timezone.utc),
            end_date=self.data.query_timeline[-1][1] if self.data.query_timeline else datetime.now(timezone.utc)
        )
        ...
        # Content metrics
        metrics.top_entities = dict(sorted(entity_counts_agg.items(), key=lambda x: x[1], reverse=True)[:10])
        metrics.top_intents = dict(sorted(intent_counts.items(), key=lambda x: x[1], reverse=True))
        ...
        if self.data.query_chains:
            ...
            # Count refinements
            refinement_chains = [c for c in self.data.query_chains if c.chain_type == QueryChainType.REFINEMENT]
            metrics.refinement_rate = len(refinement_chains) / len(self.data.query_chains) if self.data.query_chains else 0.0
    ...
    def _extract_execution_time(self, query_data: Dict[str, Any]) -> Optional[float]:
    ...
    def generate_query_suggestions(self, context: Dict[str, Any] = None) -> List[ProactiveSuggestion]:
        """
                    ...
                    if related_entities:
                        ...
                        if pattern.confidence >= 0.6:
                            ...
                            suggestion = ProactiveSuggestion(
                                ...
                                priority=SuggestionPriority.MEDIUM,
                                confidence=pattern.confidence,
                                context={"pattern_id": pattern.pattern_id, "related_to": mentioned_entity}
                            )
        ...
        if refinement_patterns and len(refinement_patterns) >= 2:
            ...
            successful_patterns = [p for p in refinement_patterns if p.confidence >= 0.7 and p.attributes.get("avg_success_rate", 0) > 0.7]
            
            if successful_patterns:
                pattern = successful_patterns[0]
                
                # Create a suggestion based on successful refinement strategies
                refinement_type = pattern.attributes.get("refinement_type", "").replace("_", " ")
    ...
    def analyze_and_generate(self) -> Tuple[Dict[str, Any], List[ProactiveSuggestion]]:
        ...
        self.logger.info(f"Query pattern analysis: Generated {len(suggestions)} suggestions")
...
def main():
    ...
    parser.add_argument("--max-queries", type=int, default=1000, help="Maximum number of queries to analyze")
    ...
    parser.add_argument("--output", type=str, default="query_patterns.json", help="Output file name")
        ...
        try:
            ...
            if suggestions:
                ...
                for i, suggestion in enumerate(suggestions, 1):
                    print(f"{i}. {suggestion.title} ({suggestion.suggestion_type.value}, confidence: {suggestion.confidence:.2f})")
            ...
            if args.save:
                results = {
                    ...
                    "metrics": analyzer.data.user_metrics["current_user"].model_dump() if "current_user" in analyzer.data.user_metrics else None
                }
