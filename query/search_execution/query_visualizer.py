...
class ExecutionNode(IndalekoBaseModel):
    ...
    def set_operation_type(cls, v, values):
        ...
        elif node_type in [NodeType.INSERT, NodeType.UPDATE, NodeType.REPLACE, NodeType.REMOVE, NodeType.UPSERT]:
...
class ExecutionPlan(IndalekoBaseModel):
    ...
    cacheable: bool = Field(
        default=False, 
        description="Whether the query is cacheable"
    )
...
class PlanVisualizer:
    ...
    def parse_plan(self, explain_result: Dict[str, Any]) -> ExecutionPlan:
        ...
        if not plan_data:
            ...
            if "raw_result" in explain_result and isinstance(explain_result["raw_result"], dict):
        ...
        for node_data in nodes_data:
            ...
            if "indexes" in node_data and node_data["indexes"]:
        ...
        if index_nodes:
            ...
            for node in index_nodes:
                if node.collection and node.index:
                    index_info.append(f"{node.collection}.{node.index} ({node.index_type})")
                elif node.index:
    ...
    def _identify_bottlenecks(self, plan: ExecutionPlan) -> List[str]:
        ...
        if scan_nodes:
            ...
            if collections:
                bottlenecks.append(f"Full collection scan(s) on: {', '.join(collections)}")
    ...
    def _generate_recommendations(self, plan: ExecutionPlan) -> List[str]:
        ...
        if plan.total_cost > 5000 and not any(node.node_type == NodeType.LIMIT for node in plan.nodes):
            recommendations.append("Consider adding a LIMIT clause to reduce result set size")
    ...
    def visualize_text(
        self, plan: Union[ExecutionPlan, Dict[str, Any]], verbose: bool = False
    ) -> str:
        """
    ...
    def _visualize_node_tree(
        ...
        for child in children:
            child_lines = self._visualize_node_tree(child, all_nodes, child_prefix, verbose)
