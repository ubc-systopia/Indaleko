...
class IndalekoAssistantCLI:
    ...
    def _format_result_item(self, item: Dict[str, Any], indent: int = 2) -> str:
        ...
        if "Record" in item and "Attributes" in item["Record"]:
            ...
            name = attributes.get("Label", path.split("/")[-1] if "/" in path else path.split("\\")[-1] if "\\" in path else path)
            size = attributes.get("Size", "Unknown size")
            timestamp = attributes.get("Timestamp", attributes.get("CreateTime", "Unknown time"))
    ...
    def _display_results(self, results: Dict[str, Any]):
        ...
        if performance:
            print("\nPerformance:")
            print(f"  Execution time: {performance.get('execution_time_seconds', 'N/A')} seconds")
            
            # Display CPU usage
            cpu = performance.get("cpu", {})
            if cpu:
                print(f"  CPU: user={cpu.get('user_time', 'N/A')}s, system={cpu.get('system_time', 'N/A')}s")
