...
def run_session_2():
    ...
    try:
        ...
        print(
            "Error: continuation_id.txt not found. Please run test_session_1.py first.",
        )
        return
    ...
    print(
        f"- thesis_topic from previous session: {conversation.get_context_variable('thesis_topic')}",
    )
    print(
        f"- demo_vehicle from previous session: {conversation.get_context_variable('demo_vehicle')}",
    )
