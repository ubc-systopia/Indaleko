...
from query.assistants.assistant import IndalekoAssistant, HAS_RECOMMENDATIONS
...
def run_test_conversation(assistant: IndalekoAssistant, model: str = "gpt-4o") -> None:
    ...
    for i, query in enumerate(queries, 1):
        ...
        if recommendations:
            ...
            for j, rec in enumerate(recommendations, 1):
                ...
                print_colored(f"     Source: {rec['source']}, Confidence: {rec['confidence']:.2f}", Fore.CYAN)
        else:
    ...
    file_path = os.path.join(os.environ.get("INDALEKO_ROOT"), "conversation_with_recommendations.json")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test Recommendation Integration with Assistant API")
    ...
    if not HAS_RECOMMENDATIONS:
        print_colored("Recommendation Integration is not available. Please check your installation.", 
                   Fore.RED, bold=True)
        return
    
    print_colored("Testing Recommendation Integration with Assistant API", Fore.CYAN, bold=True)
    ...
    archivist_memory.add_long_term_goal("Knowledge Management", "Organize personal knowledge base", progress=0.4)
    archivist_memory.add_insight("document_management", "User frequently searches for recent documents", 0.8)
    archivist_memory.add_insight("file_types", "PDF and DOCX are the most common file types", 0.9)
