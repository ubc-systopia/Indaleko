...
class TopicSegment(BaseModel):
    ...
    segment_id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="Unique identifier for this segment")
    topic: str = Field(default="general", description="The topic of this segment")
    start_index: int = Field(..., description="Index of the first message in this segment")
    end_index: Optional[int] = Field(default=None, description="Index of the last message in this segment (None if ongoing)")
    entities: List[str] = Field(default_factory=list, description="Key entities relevant to this topic")
    summary: Optional[str] = Field(default=None, description="A summary of this conversation segment")
    importance: float = Field(default=0.5, description="Importance score for this segment (0.0-1.0)")
    created_at: datetime = Field(default_factory=datetime.utcnow, description="When this segment was created")
...
class ReferencedMemory(BaseModel):
    ...
    memory_type: str = Field(..., description="Type of memory (e.g., 'archival', 'pattern', 'insight')")
    relevance_score: float = Field(default=0.7, description="How relevant this memory is to the current context (0.0-1.0)")
    referenced_at: datetime = Field(default_factory=datetime.utcnow, description="When this memory was referenced")
    message_id: Optional[str] = Field(default=None, description="Message ID where this memory was referenced")
    summary: Optional[str] = Field(default=None, description="Brief summary of the memory content")
...
class ConversationState(BaseModel):
    ...
    topic_segments: List[TopicSegment] = Field(default_factory=list, description="Topic segments within this conversation")
    active_topic_segment: Optional[str] = Field(default=None, description="ID of the currently active topic segment")
    conversation_summary: Optional[str] = Field(default=None, description="Dynamic summary of the entire conversation")
    key_takeaways: List[str] = Field(default_factory=list, description="Key takeaways from the conversation")
    referenced_memories: List[ReferencedMemory] = Field(default_factory=list, description="Memories referenced during the conversation")
    context_variables: Dict[str, Any] = Field(default_factory=dict, description="Context variables maintained across the conversation")
    importance_score: float = Field(default=0.5, description="Overall importance of this conversation (0.0-1.0)")
    continuation_pointer: Optional[str] = Field(default=None, description="Pointer to continuation information if this conversation spans sessions")
    ...
    def start_topic_segment(self, topic: str, entities: List[str] = None) -> TopicSegment:
        """
    ...
    def end_topic_segment(self, summary: str = None) -> Optional[TopicSegment]:
        """
    ...
    def add_referenced_memory(self, memory_id: str, memory_type: str, 
                             relevance_score: float = 0.7,
                             summary: str = None, 
                             message_id: str = None) -> ReferencedMemory:
        """
