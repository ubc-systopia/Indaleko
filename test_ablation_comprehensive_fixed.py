#!/usr/bin/env python3
"""
Fixed comprehensive ablation test for Indaleko.
...
class ComprehensiveAblationTest:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Collections to test - using specific named collections that exist in the database
        ...
        try:
            ...
            for collection_name in [
                ...
                if not self.db.has_collection(collection_name):
                    self.logger.critical(f"Collection {collection_name} does not exist, terminating")
                    exit(0)
    ...
    def upload_data(self) -> bool:
        ...
        try:
            ...
            # Get collections - using direct access to collections that we know exist
            ...
            object_collection = self.db.collection(object_collection_name)
            music_collection = self.db.collection(music_collection_name)
            geo_collection = self.db.collection(geo_collection_name)

            if object_collection is None:
                self.logger.error(f"Could not find collection: {object_collection_name}")
                return False

            # Upload storage objects to Objects collection
            ...
            # Upload activity records - half to music, half to geo
            ...
            # Split activities between music and geo
            midpoint = len(serializable_activities) // 2
            music_activities = serializable_activities[:midpoint]
            geo_activities = serializable_activities[midpoint:]

            # Upload music activities
            if music_collection:
                self.logger.info(f"Uploading {len(music_activities)} to music collection...")
                for i in range(0, len(music_activities), activity_batch_size):
                    batch = music_activities[i:i+activity_batch_size]
                    try:
                        music_collection.import_bulk(batch, on_duplicate="update")
                    except Exception as e:
                        self.logger.error(f"Error uploading music batch: {e}")
            else:
                self.logger.warning(f"Skipping music upload - collection not found")

            # Upload geo activities
            if geo_collection:
                self.logger.info(f"Uploading {len(geo_activities)} to geo collection...")
                for i in range(0, len(geo_activities), activity_batch_size):
                    batch = geo_activities[i:i+activity_batch_size]
                    try:
                        geo_collection.import_bulk(batch, on_duplicate="update")
                    except Exception as e:
                        self.logger.error(f"Error uploading geo batch: {e}")
            else:
                self.logger.warning(f"Skipping geo upload - collection not found")
