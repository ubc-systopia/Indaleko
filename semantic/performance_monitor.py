...
class SemanticExtractorPerformance(metaclass=Singleton):
    ...
    def __init__(self, **kwargs):
        ...
        logging.info(
            f"Semantic Extractor Performance Monitor initialized on {platform.system()} host {socket.gethostname()}",
        )
    ...
    def _record_performance_data(self, metrics: dict[str, Any]) -> None:
        ...
        if self._record_to_db:
            try:
                ...
                logging.exception(f"Error recording performance data to database: {e}")
        ...
        if self._record_to_file and self._perf_file_name:
            try:
                ...
                logging.exception(f"Error recording performance data to file: {e}")
...
if __name__ == "__main__":
    ...
    def test_mime_detection(file_path: str):
        ...
        return {"mime_type": "text/plain", "confidence": 0.95, "encoding": "utf-8"}
