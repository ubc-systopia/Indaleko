    ...
    for file_path in path.glob(glob_pattern):
        if file_path.is_file():
            ...
            try:
                ...
                # Print progress
                print(f"Processing {total_files} files: {file_path.name} ({format_bytes(file_size)}) - {elapsed_time:.2f}s", end='\r')
...
def main():
    ...
    file_parser.add_argument('--verbose', '-v', action='store_true', help='Show all checksum types')
    ...
    dir_parser.add_argument('--extensions', '-e', nargs='+', help='Filter by file extensions (e.g., .jpg .png)')
    dir_parser.add_argument('--verbose', '-v', action='store_true', help='Show detailed information')
    ...
    if args.command == 'file':
        ...
        process_directory(args.directory_path, args.recursive, args.extensions, args.verbose)
