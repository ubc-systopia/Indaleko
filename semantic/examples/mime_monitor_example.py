...
class MonitoredMimeDetector:
    ...
    def process_directory(self, directory: str, recursive: bool = False) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.monitor.stop_monitoring(batch_context, success=True, additional_data={
...
def main():
    ...
    parser = argparse.ArgumentParser(description="MIME Type Detector with Performance Monitoring")
    ...
    if args.stats:
        ...
        for mime_type, type_stats in stats.get('file_type_stats', {}).items():
            if type_stats['count'] > 0:
                ...
                print(f"  {mime_type}: {type_stats['count']} files, {avg_time:.4f} seconds avg")
