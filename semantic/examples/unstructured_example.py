...
def compare_extractor_performance(directory_path, skip_db=True):
    ...
    print("-" * 80)
    print(f"{'File Type':<10} {'Count':<6} {'Total Time':<12} {'Avg Time/File':<14} {'Avg CPU %':<10} {'Avg Mem (MB)':<12}")
    print("-" * 80)
    
    for ext, stats in sorted(results.items()):
        print(f"{ext:<10} {stats['file_count']:<6} {stats['total_time']:.2f}s {stats['avg_time_per_file']:.2f}s {stats['avg_cpu']:.2f}% {stats['avg_memory']:.2f}MB")
...
def main():
    ...
    parser.add_argument("--experiment", action="store_true", help="Run a performance experiment")
    parser.add_argument("--use-db", action="store_true", help="Connect to the database for storage")
