...
from semantic.performance_monitor import SemanticExtractorPerformance, monitor_semantic_extraction, get_machine_id
...
class SemanticExtractorExperiment:
    ...
    def __init__(self, **kwargs):
        ...
        self.output_dir = kwargs.get("output_dir", os.path.join(
        ...
        self.logger.info(f"Experiment framework initialized with ID: {self.experiment_id} on machine {self.hostname} ({self.platform})")
    ...
    def generate_test_files(self, count: int = 10, size_range: Tuple[int, int] = (1024, 1024*1024),
                           types: List[str] = ["text", "image", "binary"]) -> List[str]:
        """
            ...
            if file_type == "text":
                ...
                with open(file_path, 'w') as f:
                    ...
                    for _ in range(lines):
                        f.write(''.join(random.choices('abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789', k=chars_per_line)))
                        f.write('\n')
    ...
    def run_throughput_experiment(self, extractor_type: str, sample_size: int = 100, 
                                 use_database: bool = False) -> Dict[str, Any]:
        """
        ...
        """
        self.logger.info(f"Starting throughput experiment for {extractor_type} extractor")
        ...
        for file_path in tqdm(file_paths, desc=f"Processing with {extractor_type}"):
            try:
                ...
                # Stop monitoring
                self.monitor.stop_monitoring(context, success=True, additional_data=result)
        ...
        with open(os.path.join(self.output_dir, f"{extractor_type}_throughput_stats.json"), 'w') as f:
    ...
    def run_file_type_comparison(self, extractor_type: str, file_types: List[str] = None,
                                count_per_type: int = 20) -> Dict[str, Any]:
        """
        ...
        self.logger.info(f"Starting file type comparison for {extractor_type} extractor")
        ...
        for file_type in file_types:
            ...
            file_paths = self.generate_test_files(count=count_per_type, types=[file_type])
            ...
            if type_stats["processed_files"] > 0:
                ...
                type_stats["avg_bytes_per_second"] = type_stats["processed_bytes"] / type_stats["total_time"] if type_stats["total_time"] > 0 else 0
        ...
        with open(os.path.join(self.output_dir, f"{extractor_type}_type_comparison.json"), 'w') as f:
    ...
    def run_size_scaling_experiment(self, extractor_type: str, 
                                   file_sizes: List[int] = None,
                                   files_per_size: int = 5) -> Dict[str, Any]:
        """
        ...
        if not file_sizes:
            ...
            file_sizes = [10*1024, 100*1024, 1024*1024, 10*1024*1024, 100*1024*1024]
        
        self.logger.info(f"Starting size scaling experiment for {extractor_type} extractor")
        ...
        for size in file_sizes:
            ...
            if size_stats["processed_files"] > 0:
                ...
                size_stats["bytes_per_second"] = (size * size_stats["processed_files"]) / size_stats["total_time"] if size_stats["total_time"] > 0 else 0
                size_stats["time_per_mb"] = size_stats["total_time"] / (size * size_stats["processed_files"] / (1024*1024)) if size > 0 else 0
        ...
        with open(os.path.join(self.output_dir, f"{extractor_type}_size_scaling.json"), 'w') as f:
    ...
    def run_coverage_experiment(self, days: int = 30, sample_interval: int = 1,
                               extractors: List[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for day in days_range:
                for extractor in extractors:
                    ...
                    coverage = min(1.0, current_coverage.get(extractor, 0) + (extracted_files / total_objects))
            ...
            # Save results
            with open(os.path.join(self.output_dir, "coverage_projection.json"), 'w') as f:
    ...
    def run_all_experiments(self, sample_size: int = 100) -> Dict[str, Any]:
        ...
        for extractor in extractors:
            self.run_file_type_comparison(extractor, count_per_type=max(5, sample_size // 10))
        
        # Run size scaling experiments
        for extractor in extractors:
            self.run_size_scaling_experiment(extractor, files_per_size=max(3, sample_size // 20))
    ...
    def _generate_type_comparison_visualization(self, results: Dict[str, Any]) -> None:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.output_dir, f"{extractor_type}_type_comparison.png"))
        plt.close()
    ...
    def _generate_coverage_visualization(self, results: Dict[str, Any]) -> None:
        ...
        for extractor in extractors:
            plt.plot(days_range, [c * 100 for c in coverage_data[extractor]], 
                     marker='o', linestyle='-', label=f"{extractor.capitalize()}")
...
""")
            ...
            throughput_experiments = [exp for exp in self.results["experiments"] if exp["experiment_type"] == "throughput"]
...
""")
            ...
            comparison_experiments = [exp for exp in self.results["experiments"] if exp["experiment_type"] == "file_type_comparison"]
...
""".format(extractor_type=exp["extractor_type"]))
...
""")
            ...
            scaling_experiments = [exp for exp in self.results["experiments"] if exp["experiment_type"] == "size_scaling"]
...
""")
            ...
            coverage_experiments = [exp for exp in self.results["experiments"] if exp["experiment_type"] == "coverage" and exp.get("success", False)]
...
""".format(total_objects=exp["total_objects"]))
...
""")
    ...
    def analyze_performance_data_by_machine(self, perf_file: str = None, machine_id: str = None) -> Dict[str, Any]:
        """
        ...
        """
        if perf_file is None and os.path.exists(os.path.join(self.output_dir, "perf_data.jsonl")):
        ...
        try:
            ...
            for entry in all_data:
                ...
                if "ActivityStats" in entry and "IO" in entry["ActivityStats"]:
                    record.update({
                        "io_read_count": entry["ActivityStats"]["IO"].get("read_count", 0),
                        "io_write_count": entry["ActivityStats"]["IO"].get("write_count", 0),
                        "io_read_bytes": entry["ActivityStats"]["IO"].get("read_bytes", 0),
                        "io_write_bytes": entry["ActivityStats"]["IO"].get("write_bytes", 0),
                    })
                    
                # Extract Memory stats if available
                if "ActivityStats" in entry and "Memory" in entry["ActivityStats"]:
                    record.update({
                        "memory_rss_delta": entry["ActivityStats"]["Memory"].get("rss_delta", 0),
                        "memory_vms_delta": entry["ActivityStats"]["Memory"].get("vms_delta", 0),
                        "memory_peak_rss": entry["ActivityStats"]["Memory"].get("peak_rss", 0),
                        "memory_peak_vms": entry["ActivityStats"]["Memory"].get("peak_vms", 0),
                    })
            ...
            for machine in df["machine_id"].unique():
                ...
                machine_stats = {
                    ...
                    "files_per_second": len(machine_df) / machine_df["elapsed_time"].sum() if machine_df["elapsed_time"].sum() > 0 else 0,
                    "mb_per_second": machine_df["file_size"].sum() / (1024 * 1024) / machine_df["elapsed_time"].sum() if machine_df["elapsed_time"].sum() > 0 else 0,
                ...
                for extractor in machine_df["extractor_name"].unique():
                    ...
                    extractor_stats = {
                        ...
                        "mb_per_second": extractor_df["file_size"].sum() / (1024 * 1024) / extractor_df["elapsed_time"].sum() if extractor_df["elapsed_time"].sum() > 0 else 0,
            ...
            if len(overall_stats["machines"]) > 1 and PLOTTING_AVAILABLE:
                self._generate_machine_comparison_visualization(df, os.path.join(self.output_dir, "machine_comparison.png"))
    ...
    def _generate_machine_comparison_visualization(self, df: pd.DataFrame, output_path: str) -> None:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Semantic Extractor Performance Experiments")
    
    # Experiment selection
    parser.add_argument("--all", action="store_true", help="Run all experiments")
    parser.add_argument("--throughput", action="store_true", help="Run throughput experiment")
    parser.add_argument("--file-types", action="store_true", help="Run file type comparison")
    parser.add_argument("--size-scaling", action="store_true", help="Run size scaling experiment")
    parser.add_argument("--coverage", action="store_true", help="Run coverage experiment")
    
    # Extractor selection
    parser.add_argument("--mime", action="store_true", help="Test MIME type detector")
    parser.add_argument("--checksum", action="store_true", help="Test checksum calculator")
    parser.add_argument("--exif", action="store_true", help="Test EXIF extractor")
    parser.add_argument("--all-extractors", action="store_true", help="Test all extractors")
    
    # Analysis options
    parser.add_argument("--analyze", action="store_true", help="Analyze existing performance data")
    parser.add_argument("--perf-file", type=str, help="Performance data file to analyze")
    parser.add_argument("--machine-id", type=str, help="Filter analysis by machine ID")
    
    # Experiment parameters
    parser.add_argument("--sample-size", type=int, default=100, help="Sample size for experiments")
    parser.add_argument("--output-dir", type=str, help="Output directory for results")
    parser.add_argument("--db", action="store_true", help="Use database files (when available)")
    parser.add_argument("--no-db-record", action="store_false", dest="record_to_db", help="Disable recording to database")
