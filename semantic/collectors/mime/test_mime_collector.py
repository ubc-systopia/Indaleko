...
class TestMimeTypeCollector(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        for file_path in [self.text_file, self.html_file, self.json_file, 
                        self.bin_file, self.py_file, self.pdf_file]:
    ...
    def test_mime_detection(self):
        ...
        self.assertTrue(
            html_mime["mime_type"].startswith("text/html") or 
            html_mime["mime_type"] == "text/plain"
        )
        ...
        self.assertTrue(
            json_mime["mime_type"].startswith("application/json") or
            json_mime["mime_type"].startswith("text/")
        )
        ...
        self.assertIn(bin_mime["mime_type_from_extension"], ["application/octet-stream", None])
    ...
    def test_create_mime_record(self):
        ...
        self.assertEqual(record.mime_type, self.mime_collector.detect_mime_type(self.text_file)["mime_type"])
        self.assertGreaterEqual(len(record.SemanticAttributes), 3)  # At least MIME type, confidence, and category
    ...
    def test_extension_matching(self):
        ...
        if html_result["mime_type"] == "text/html" and html_result["mime_type_from_extension"] == "text/html":
            self.assertGreater(html_result["confidence"], 0.9)  # High confidence when they match
