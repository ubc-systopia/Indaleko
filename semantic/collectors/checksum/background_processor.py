...
def process_file_checksums(file_obj: IndalekoObject, local_path: str) -> Optional[Dict[str, str]]:
    """
    ...
    try:
        ...
        logger.info(f"Calculated checksums for {local_path}: MD5={checksums['md5'][:8]}...")
...
def process_file_and_store(file_obj: IndalekoObject, local_path: str) -> Optional[Dict[str, str]]:
    """
...
def main():
    ...
    if args.foreground:
        ...
        logger.info(f"Checksum processor completed. Scheduled {scheduled} files for processing.")
