...
class ExifCollector(SemanticCollector):
    ...
    def extract_exif_data(self, file_path: str) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                logging.warning(
                    f"Error extracting additional EXIF data with Pillow: {e}",
                )
            ...
            logging.exception(f"Error extracting EXIF data from {file_path}: {e}")
            return {}
    ...
    def extract_gps_data(self, exif_data: dict[str, Any]) -> ExifGpsData | None:
        """
        ...
        try:
            ...
            if "GPS GPSTimeStamp" in exif_data and "GPS GPSDateStamp" in exif_data:
    ...
    def extract_capture_settings(
        ...
        if "EXIF DateTimeOriginal" in exif_data:
            datetime_original = self.parse_datetime(
                str(exif_data["EXIF DateTimeOriginal"]),
            )

        if "EXIF DateTimeDigitized" in exif_data:
            datetime_digitized = self.parse_datetime(
                str(exif_data["EXIF DateTimeDigitized"]),
            )
        ...
        if datetime_original or exposure_time or f_number or iso or focal_length:
    ...
    def extract_image_info(self, exif_data: dict[str, Any]) -> ExifImageInfo | None:
        """
        ...
        if "EXIF PlanarConfiguration" in exif_data:
            planar_codes = {"1": "Chunky format", "2": "Planar format"}
    ...
    def process_exif_data(self, exif_data: dict[str, Any]) -> ExifDataModel:
        ...
        return ExifDataModel(
            ...
            RelatedObjects=[
                uuid.uuid4(),
            ],  # This would be replaced with actual object ID
