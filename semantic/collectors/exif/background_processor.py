...
SUPPORTED_IMAGE_EXTENSIONS = [
    ...
    '.heic', '.heif', '.nef', '.cr2', '.dng'
]


def process_file_exif(file_obj: IndalekoObject, local_path: str) -> Optional[Dict[str, Any]]:
    """
...
def process_file_and_store(file_obj: IndalekoObject, local_path: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        logger.error(f"Error processing and storing EXIF metadata for {local_path}: {e}")
        return None
...
def schedule_exif_processing(
    ...
    min_last_processed_days: Optional[int] = 45,
...
def main():
    ...
    if args.foreground:
        ...
        logger.info(f"EXIF metadata processor completed. Scheduled {scheduled} files for processing.")
