...
class SemanticPerformanceCliIntegration:
    ...
    def __init__(self, cli_instance: IndalekoBaseCLI) -> None:
        ...
        # Add help text
        self.cli.append_help_text("  /perf              - Show performance monitoring commands")
        self.cli.append_help_text("  /experiments       - List and run performance experiments")
    ...
    def handle_perf_command(self, args: List[str]) -> None:
        ...
        if not args or args[0] == "help":
            ...
            self.cli.output("  /perf status              - Show current monitoring status")
            self.cli.output("  /perf enable              - Enable performance monitoring")
            self.cli.output("  /perf disable             - Disable performance monitoring")
            self.cli.output("  /perf stats               - Show current performance statistics")
            self.cli.output("  /perf reset               - Reset performance statistics")
            self.cli.output("  /perf analyze <file>      - Analyze performance data file")
            return
    ...
    def handle_experiments_command(self, args: List[str]) -> None:
        ...
        if not args or args[0] == "help":
            ...
            self.cli.output("  /experiments list                      - List available experiments")
            self.cli.output("  /experiments throughput <type> <count> - Run throughput experiment")
            self.cli.output("  /experiments filetypes <type>          - Run file type comparison")
            self.cli.output("  /experiments scaling <type>            - Run size scaling analysis")
            self.cli.output("  /experiments coverage <type>           - Run coverage experiment")
            return
            
        if args[0] == "list":
            self.cli.output("Available experiments:")
            self.cli.output("  throughput - Measures extraction throughput (files/sec, MB/sec)")
            self.cli.output("  filetypes  - Compares performance across different file types")
            self.cli.output("  scaling    - Analyzes how performance scales with file size")
            self.cli.output("  coverage   - Projects metadata growth for different extractors")
            return
        ...
        if args[0] == "throughput" and len(args) >= 3:
            try:
                ...
                self.cli.output(f"Running throughput experiment for {extractor_type} with {sample_size} files...")
        ...
        if args[0] == "scaling" and len(args) >= 2:
            try:
                extractor_type = args[1]
                self.cli.output(f"Running size scaling analysis for {extractor_type}...")
    ...
    def handle_report_command(self, args: List[str]) -> None:
        ...
        if not args or args[0] == "help":
            ...
            self.cli.output("  /report generate <type> <output>      - Generate performance report")
            self.cli.output("  /report compare <file1> <file2>       - Compare performance data files")
            self.cli.output("  /report summary <days>                - Summarize recent performance data")
            return
...
def register_semantic_performance_cli(cli_instance: IndalekoBaseCLI) -> SemanticPerformanceCliIntegration:
    """
