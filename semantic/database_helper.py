...
class SemanticDatabaseHelper:
    ...
    def get_files_for_processing(self, extractor_type: str, batch_size: int, 
        ...
        try:
            ...
            if extractor_type == "mime":
                ...
                semantic_attribute_id = uuid.UUID("8a7b9678-f2c5-4e3a-9a8b-cc8c2e626374")  # Mime type attribute
            elif extractor_type == "checksum":
                semantic_collection_name = IndalekoDBCollections.Indaleko_Semantic_Checksum
                semantic_attribute_id = uuid.UUID("c4e2d558-6a13-4734-9e19-5fe3c8a2c355")  # Checksum attribute (MD5)
            else:
            ...
            file_extensions = self.config["extractors"][extractor_type].get("file_extensions", ["*"])
            ...
            if not process_all_extensions:
                ...
                for ext in file_extensions:
                    if not ext.startswith("."):
                        ext = f".{ext}"
                    extension_conditions.append(f'LIKE(LOWER(obj.Record.Attributes.URI), "%{ext}")')
            ...
            logger.info(f"Found {len(filtered_results)} files for {extractor_type} processing")
    ...
    def store_checksum_data(self, object_id: str, checksum_data: Dict[str, Any]) -> bool:
        """
