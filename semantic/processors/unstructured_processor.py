...
from semantic.collectors.unstructured.unstructured_collector import UnstructuredCollector
from semantic.collectors.unstructured.data_models.input import UnstructuredInputDataModel
...
class UnstructuredProcessor:
    ...
    def __init__(self, **kwargs):
        ...
        if not self._logger.handlers:
            ...
            file_handler = logging.FileHandler(
                os.path.join(log_dir, f"unstructured_processor_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
            )
    ...
    @monitor_semantic_extraction(extractor_name="UnstructuredProcessor.process_directory")
    def process_directory(
        self, 
        directory_path: str, 
        recursive: bool = True,
        file_extensions: Optional[List[str]] = None,
        skip_larger_than_mb: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        ...
        """
        self._logger.info(f"Processing directory: {directory_path} (recursive={recursive})")
        ...
        files = self._find_files(directory_path, recursive, file_extensions, skip_larger_than_mb)
        ...
        self._logger.info(f"Directory processing complete: {result['status']} ({result['files_processed']}/{result['files_found']} files)")
    ...
    def process_files(self, file_paths: List[str]) -> Dict[str, Any]:
        ...
        self._logger.info(f"File processing complete: {result['status']} ({result['files_processed']}/{result['files_found']} files)")
    ...
    def process_pdf(self, pdf_path: str) -> Dict[str, Any]:
        ...
        if mime_type != "application/pdf":
            ...
            return {"error": "File is not a PDF", "path": pdf_path, "mime_type": mime_type}
        
        # Create input model
        input_model = UnstructuredInputDataModel(
            ObjectIdentifier=uuid.uuid4(),
            LocalPath=pdf_path,
            ModificationTimestamp=datetime.fromtimestamp(os.path.getmtime(pdf_path), timezone.utc),
        ...
        try:
            if recursive:
                ...
                for item in os.listdir(directory):
                    ...
                    if os.path.isfile(file_path) and self._is_valid_file(file_path, extensions, max_size_bytes):
                        files.append(file_path)
        except Exception as e:
            self._logger.error(f"Error finding files: {str(e)}")
        
        return files
    
    def _is_valid_file(self, file_path: str, extensions: Optional[List[str]], max_size_bytes: Optional[int]) -> bool:
        """
        ...
        try:
            ...
            self._logger.warning(f"Error checking file {file_path}: {str(e)}")
            return False
    ...
    def _extract_pdf_content(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self._logger.error(f"Error extracting PDF content: {str(e)}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Indaleko Unstructured Semantic Processor")
    ...
    dir_parser.add_argument("--recursive", "-r", action="store_true", help="Process subdirectories recursively")
    dir_parser.add_argument("--extensions", "-e", nargs="+", help="File extensions to process (e.g., pdf docx)")
    dir_parser.add_argument("--max-size", "-m", type=int, help="Skip files larger than this size in MB")
    dir_parser.add_argument("--skip-db", action="store_true", help="Skip database connection")
    ...
    files_parser.add_argument("--skip-db", action="store_true", help="Skip database connection")
    ...
    pdf_parser.add_argument("--skip-db", action="store_true", help="Skip database connection")
    pdf_parser.add_argument("--output", "-o", help="Output JSON file for extracted content")
