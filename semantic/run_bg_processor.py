    ...
    def start(self):
        ...
        try:
            ...
            if self.process.poll() is not None:
                logger.info(f"Background processor exited with code {self.process.returncode}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Indaleko Background Processor Service")
    parser.add_argument(
        "--config", 
        type=str, 
...
def install_windows_service(service_name, config_path, python_exe):
    ...
    try:
        ...
        if not nssm_path:
            logger.error("NSSM utility not found in PATH. Please install NSSM to install as a service.")
        ...
        subprocess.run([nssm_path, "set", service_name, "DisplayName", "Indaleko Background Processor"], check=True)
        subprocess.run([nssm_path, "set", service_name, "Description", "Performs background semantic data extraction for Indaleko"], check=True)
...
def uninstall_windows_service(service_name):
    ...
    try:
        ...
        nssm_path = shutil.which("nssm.exe")
        if not nssm_path:
            logger.error("NSSM utility not found in PATH. Please install NSSM to uninstall the service.")
            return
