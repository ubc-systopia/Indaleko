#!/usr/bin/env python
"""
Test script for the refactored OpenAI connector with prompt management integration.

This script runs a simple test to verify that the refactored OpenAI connector works
correctly with the prompt management system.

Usage:
    python test_refactored_openai.py
"""
...
def load_api_key() -> str | None:
    ...
    key_path = os.path.join(config_dir, "openai-key.ini")

    if not os.path.exists(key_path):
        # Try the unified keys file
...
def test_direct_connector():
    """Test the refactored OpenAI connector directly."""
    logger.info("Testing refactored OpenAI connector directly")
    ...
    connector_with_guardian = OpenAIConnector(
        api_key=api_key,
        model="gpt-4o",
        use_guardian=True,
    ...
    connector_without_guardian = OpenAIConnector(
        api_key=api_key,
        model="gpt-4o",
        use_guardian=False,
    )
...
def test_factory_integration():
    ...
    # Create LLM interface for OpenAI
    logger.info("Creating LLM interface for OpenAI with guardian")
    try:
    ...
    # Create LLM interface for OpenAI without guardian
    logger.info("Creating LLM interface for OpenAI without guardian")
    try:
...
def main() -> int:
    """Run the test script."""
    logger.info("Starting refactored OpenAI connector test")
