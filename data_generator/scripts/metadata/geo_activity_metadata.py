...
class GeoActivityData(ActivityMetadata):
    ...
    def generate_metadata(self, record_kwargs: IndalekoRecordDataModel, timestamps: Dict[str, datetime], is_truth_file: bool, 
                            truth_like: bool, truthlike_attributes: list[str]) -> Any:
        is_truth_file=self._define_truth_attribute("geo_location", is_truth_file, truth_like, truthlike_attributes)
        return self._generate_geo_metadata(record_kwargs, timestamps, is_truth_file)

    def _generate_geo_metadata(self, record_kwargs: IndalekoRecordDataModel, timestamps: Dict[str, datetime], 
                                is_truth_file: bool) -> IndalekoActivityDataModel:
        """
        ...
        if "geo_location" in self.selected_md:
            ...
            if geo_command == "at":
                if is_truth_file:
                    ...
                    max_lat = min(GeoActivityData.DEFAULT_MAX_LAT, truth_latitude + delta)
                    min_lat = max(GeoActivityData.DEFAULT_MIN_LAT, truth_latitude - delta)
                    max_long = min(GeoActivityData.DEFAULT_MAX_LONG, truth_longitude + delta)
                    min_long = max(GeoActivityData.DEFAULT_MIN_LONG, truth_longitude - delta)
                    min_alt = max(GeoActivityData.DEFAULT_MIN_ALT, truth_altitude - delta)
                    max_alt = min(GeoActivityData.DEFAULT_MAX_ALT, truth_altitude + delta)
                    ...
                    longitude = self._check_return_value_within_range(GeoActivityData.DEFAULT_MIN_LONG, 
                                                                        GeoActivityData.DEFAULT_MAX_LONG, 
                                                                        min_long, 
                                                                        max_long, 
                                                                        random.uniform)
                                                                        
                    altitude = self._check_return_value_within_range(GeoActivityData.DEFAULT_MIN_ALT, 
            ...
            latitude = random.uniform(GeoActivityData.DEFAULT_MIN_LAT, GeoActivityData.DEFAULT_MAX_LAT)
            longitude = random.uniform(GeoActivityData.DEFAULT_MIN_LONG, GeoActivityData.DEFAULT_MAX_LONG)
            altitude = random.uniform(GeoActivityData.DEFAULT_MIN_ALT, GeoActivityData.DEFAULT_MAX_ALT)
