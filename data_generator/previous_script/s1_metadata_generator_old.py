...
class Dataset_Generator():
    ...
    def _generate_metadata(self, current_filenum: int, max_num: int, key: str, is_truth_file: bool, truth_like: bool) -> Tuple[list[Dict[str,Any]], list[Dict[str,Any]], list[Dict[str,Any]], list[Dict[str,Any]], list[Dict[str,Any]], list[Dict[str,Any]]]:
        ...
        for n in range(1, max_num):
            ...
            timestamps = self._generate_timestamps(is_truth_file=self.define_truth_attribute("timestamps", is_truth_file, truth_like, truthlike_attributes))
    ...
    def _generate_activity_contexts(self, record_data: IndalekoRecordDataModel, is_truth_file: bool, truth_like: bool, truthlike_attributes:list[str], timestamps: Dict[str, datetime]) -> Tuple[IndalekoActivityDataModel, EcobeeAmbientData, Union[SpotifyAmbientData, AmbientMusicData]]:
        geo_activity_provider, geo_activity_context = self._generate_geo_semantics(record_data, timestamps, is_truth_file=self.define_truth_attribute("geo_location", is_truth_file, truth_like, truthlike_attributes))
        ambient_temperature_context = self._generate_smart_thermostat_data(record_data, timestamps, is_truth_file=self.define_truth_attribute("ecobee_temp", is_truth_file, truth_like, truthlike_attributes))
        ambient_music_context = self._generate_ambient_music_context(record_data, timestamps, is_truth_file=self.define_truth_attribute("ambient_music", is_truth_file, truth_like, truthlike_attributes))
    ...
    def _generate_semantic_content(self, extension: str, last_modified: str, is_truth_file: bool, truth_like: bool, truthlike_attributes: list[str], has_semantic: bool) -> Dict[str, Any]:
        ...
        if self.selected_semantic_md is not None and (has_semantic or is_truth_file):
            for content_type, content in self.selected_semantic_md.items():
                if self.define_truth_attribute(content_type, is_truth_file, truth_like, truthlike_attributes):
    ...
    def define_truth_attribute(self, attribute: str, truth_file: bool, truthlike_file: bool, truth_attributes: list[str]) -> bool:
    ...
    def _generate_file_info(self, current_filenum: int, n: int, is_truth_file: bool, truth_like: bool, truthlike_attributes: list[str], has_semantic: bool) -> Tuple[int, str, str, str, str]:
        file_size = self.generate_file_size(is_truth_file=self.define_truth_attribute("file.size", is_truth_file, truth_like, truthlike_attributes))
        file_name = self._generate_file_name(is_truth_file=self.define_truth_attribute("file.name", is_truth_file, truth_like, truthlike_attributes), has_semantic_attr=has_semantic)
        path, URI, updated_filename = self._generate_dir_location(file_name, is_truth_file=self.define_truth_attribute("file.directory", is_truth_file, truth_like, truthlike_attributes))
