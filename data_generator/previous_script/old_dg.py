...
class Dataset_Generator():
    ...
    def _convert_time_timestamp(self, timestamps: dict, to_timestamp: bool) -> tuple:
        ...
        if isinstance(timestamps["starttime"], list):
            converted_timestamps = []
            for time in timestamps["starttime"]:
                if to_timestamp:
                    timestamp=time.timestamp()
                else:
                    timestamp = self._convert_str_datetime(time)
                converted_timestamps.append(timestamp)
            starttime = converted_timestamps
            endtime = converted_timestamps
            
            if self.earliest_endtime is None:
                self.earliest_endtime = starttime[-1]
                self.earliest_starttime = starttime[0]
            else:
                self.earliest_endtime = min(self.earliest_endtime, starttime[-1])
                self.earliest_starttime = min(self.earliest_starttime, starttime[0])
    ...
    def _generate_dir_location(self, file_name: str, is_truth_file: bool=True) -> dict:
        """
        ...
        if is_truth_file and "file.directory" in self.selected_POSIX_md:
            truth_parent_loc = self.selected_POSIX_md["file.directory"]["location"]
    ...
    def _generate_metadata(self, current_filenum:int, max_num: int, key: str, is_truth_file: bool, truth_like: bool) -> dict:
        ...
        for n in range(1, max_num):
            ...
            activity_provider, activity_context = self._generate_geo_semantics(record_data, is_truth_file)
    ...
    def _generate_geo_semantics(self, record_kwargs: IndalekoRecordDataModel, is_truth_file: bool) -> list:
        """
        Creates the geographical semantic data
        """
        geo_timestamp = self._generate_ac_timestamp(is_truth_file)
    ...
    def _generate_ac_timestamp(self, is_truth_file) -> str:
        """
        Generate the activity context timestamp
        """
        if "timestamp" in self.selected_AC_md:
            time_query = self.selected_AC_md["timestamp"]
            geo_timestamp = self._generate_queried_timestamp(time_query["starttime"], time_query["endtime"], time_query["command"], self.default_lower_timestamp, is_truth_file).strftime("%Y-%m-%dT%H:%M:%SZ")
        else:
            geo_timestamp = self._generate_random_timestamp(self.default_lower_timestamp, datetime.now()).strftime("%Y-%m-%dT%H:%M:%SZ") 
    ...
    def _generate_file_info(self, current_filenum, n, is_truth_file, truth_like, truthlike_attributes):
        ...
        path, URI = self._generate_dir_location(file_name, is_truth_file=self._define_truth_attribute("file.directory", is_truth_file, truth_like, truthlike_attributes))
        IO_UUID = self._create_metadata_UUID(current_filenum + n, is_truth_file=is_truth_file)
        return file_size, file_name, path, URI, IO_UUID
    ...
    def _generate_timestamps(self, is_truth_file: bool=True) -> dict[datetime]:
        ...
        if "timestamps" in self.selected_POSIX_md:
            ...
            if "birthtime" in query: # specifically checks for birthtime since other timestamps shouldn't be earlier than the birthtime
                ...
                birthtime = self._generate_queried_timestamp(birthtime_query["starttime"], birthtime_query["endtime"], birthtime_query["command"], default_startdate=self.default_lower_timestamp, is_truth_file = is_truth_file)
                ...
                for timestamp in stamp_labels: # for each of the other timestamps, set the timestamp based on whether it has been selected in the query or not
                    if timestamp in selected_timestamps:
                        ...
                        # randomtime = self._generate_random_timestamp(lower_bound = birthtime, upper_bound=self.default_upper_timestamp)
                        # existing_timestamp = random.choice(list(timestamps.values()))
                ...
                for timestamp in non_selected_timestamps:
                    # randomtime = self._generate_random_timestamp(lower_bound = self.default_lower_timestamp, upper_bound = self.default_upper_timestamp)
                    # existing_timestamp = random.choice(list(timestamps.values()))
                    # timestamps[timestamp] = random.choice([existing_timestamp, randomtime])
    ...
    def _generate_timestamps_v1(self, is_truth_file: bool=True) -> dict:
        """
        Generates timestamps for the specified file:
        self.selected_POSIX_md{
            timestamps:{
                CONSTRAINT: the modified, accessed, changed must have a timestamp at least that of the created timestamp date
        ...
        RETURNS: {birthtime: datetime, modifiedtime: datetime, accessedtime: datetime, changedtime: datetime}
        """
        ...
        if "timestamps" in self.selected_POSIX_md:
            ...
            selected_timestamps = set(query.keys())
            non_selected_timestamps = stamp_labels - selected_timestamps
            if "birthtime" in query:
                birthtime_query = query["birthtime"]
                birthtime = self._generate_queried_timestamp(birthtime_query["starttime"], birthtime_query["endtime"], birthtime_query["command"], default_startdate=self.default_lower_timestamp, is_truth_file = is_truth_file) # creates filler timestamp, if marked false
                timestamps["birthtime"] = birthtime
                ...
                for timestamp in selected_timestamps: # for each of the other timestamps, set the timestamp based on whether it has been selected in the query or not
                    timestamps[timestamp] = self._generate_queried_timestamp(query[timestamp]["starttime"], query[timestamp]["endtime"], query[timestamp]["command"], default_startdate = self.default_lower_timestamp, is_truth_file=is_truth_file)
                for timestamp in non_selected_timestamps:
                    randomtime = self._generate_random_timestamp(self.default_lower_timestamp, upper_bound = self.default_upper_timestamp)
                    existing_timestamp = random.choice(list(timestamps.values()))
                    timestamps[timestamp] = random.choice([existing_timestamp, randomtime])
    ...
    def _generate_random_timestamp(self, lower_bound, upper_bound) -> datetime:
        """
    ...
    def _generate_queried_timestamp(self, starttime, endtime, command, default_startdate, is_truth_file = True) -> datetime:
...
def main():
    selected_md_attributes = {"Posix": {"file.name": {"pattern": "essay", "command": "exactly", "extension": [".pdf"]}, "timestamps": {"birthtime": {"starttime": "2025-01-08T00:00:00", "endtime": "2025-01-08T23:59:59", "command": "range"}}, "file.directory": {"location": "local", "local_dir_name": "local_directory"}}, "Semantic": {}, "Activity": {}}
