    ...
    def _generate_queried_timestamp(self, starttime:datetime, endtime:datetime, command:str, default_truth_startdate:datetime, is_truth_file:bool = True, is_birthtime: bool = False) -> datetime:
        ...
        if starttime > datetime.now() or endtime > datetime.now():
            ...
            if not is_truth_file:
                ...
                if is_birthtime and starttime == self.default_lower_timestamp:
                    timestamp = fake.date_time_between(start_date=self.default_lower_timestamp + timedelta(hours=filler_delta), end_date=self.earliest_starttime-timedelta(hours=filler_delta)).replace(microsecond=0)
                elif is_birthtime and starttime > self.default_lower_timestamp and self.default_lower_timestamp <= self.earliest_starttime-timedelta(hours=filler_delta) <= starttime-timedelta(hours=filler_delta):
