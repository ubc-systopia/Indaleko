...
class QueryExtractor():
    ...
    def create_extraction_prompt(self, query: str) -> Dict:
        ...
        system_prompt = """
            ...
            “Activity”: “geo_location”: {"location": str, "command": ["at", "within"], "km": int (only when command is "within" convert to km if necessary), "timestamp": str one of "birthtime" or "modified" or "changed" or "accessed"}}}}. Get rid of keys with null values. “Semantic” can have any number of contents. "command", "Type", and "EmphasizedTextTags" should be a single string not a list. Keep key capitalization as is. 
            ...
            Posix timestamps are for when the file is modified, created, accessed or changed. For timestamps for 'Posix' use command "equal" when the starttime and endtime are not the same, otherwise, use command "range" if the starttime and endtime are different ex.) query: What are the files I created 5 days ago? The command in the dictionary should be "range" and not "equal": {"Posix": {"timestamps": {"birthtime: {"starttime": "2025-01-02T00:00:00", "endtime": "2025-01-02T23:59:59", "command": "range"}}}}
            The timestamp within the geo_location is when the activity data was collected at that location, so there should be a timestamp for any geo_location populated.
    ...
    def create_extraction_prompt_v1(self, query: str) -> Dict:
        """
        Create a prompt for the LLM to generate an  query.
        Args:
            query: str the query 
        ...
        system_prompt = """
            ...
            "timestamps": {"birthtime": {"starttime": "str or list (lists must be in order latest to most recent)", "endtime": "str or list", "command": "equal, range"}, "modified": "same as birthtime", "accessed": "same as birthtime", "changed": "same as birthtime"}, 
            "file.size": {"target_min": "int (in bytes so if in GB, multiply by 1e+9 to get bytes, if necessary)", "target_max": "int", "command": "equal, range, greater_than, greater_than_equal, less_than, less_than_equal"}, 
            "file.directory": {"location": "str (google_drive, dropbox, icloud, local; must be stated local if local_dir_name specified)", "local_dir_name": "str (provide name for local directories only; create a directory name if none provided)"}}, 
            "Semantic": {"Content_1": {"Languages": "str (based on language of Text)", "PageNumber": "int", "Text": "str", "Type": "Title, Subtitle, Header, Footer, Paragraph, BulletPoint, NumberedList, Caption, Quote, Metadata, UncategorizedText, SectionHeader, Footnote, Abstract, FigureDescription, Annotation", "EmphasizedTextTags": "bold, italic, underline, strikethrough, highlight", "EmphasizedTextContents": "str"}}, 
            "Activity": {"geo_location": {"location": "str", "command": "at, within", "km": "int (only when command is 'within' convert to km if necessary)", "timestamp": "str (one of 'birthtime', 'modified', 'changed', or 'accessed')"}, 
            "ecobee_temp": {"temp": {"start": "float within [-50.0, 100.0]", "end": "float within [-50.0, 100.0]", "command": "range, equal"}, "humidity": {"start": "float within [0.0, 100.0]", "end": "float within [0.0, 100.0]", "command": "range, equal"}, "target_temp": {"start": "float within [-50.0, 100.0]", "end": "float", "command": "range, equal"}, "hvac_mode": "str (heat, cool, auto, off)", "hvac_state": "str (heating, cooling, fan, idle)", "timestamp": "str one of ['birthtime', 'modified', 'changed', 'accessed']"}, 
            "ambient_music": {"track_name": "str", "artist_name": "str", "playback_position_ms": "int (ms [0, track_duration_ms])", "track_duration_ms": "int (in milliseconds bound by [10000, 300000])", "source": "str (spotify or other; if spotify can specify)", "device_type": "str (device the music was streamed)", "timestamp": "str one of ['birthtime', 'modified', 'changed', 'accessed']"}}}
            ...
            If there are no time queries listed, there should be no timestamps ex.) what are the pdf files I created?: Dictionary = {"Posix": {"file.name": {"extension": [".txt"], "command": "contains"}}, "Semantic":{}, "Activity": {}}
            Posix timestamps are for when the file is modified, created, accessed or changed. For timestamps for 'Posix' use command "equal" when the starttime and endtime are not the same, otherwise, use command "range" if the starttime and endtime are different ex.) query: What are the files I created 5 days ago? The command in the dictionary should be "range" and not "equal": {"Posix": {"timestamps": {"birthtime: {"starttime": "2025-01-02T00:00:00", "endtime": "2025-01-02T23:59:59", "command": "range"}}}}
            The timestamp within the geo_location is when the activity data was collected at that location, so there should be a timestamp for any geo_location populated, this goes for the ecobee_temp and ambient_music. ex.) what is the photo I took yesterday when I was in Vancouver? => "Activity": {"geo_location": {"location": "Vancouver", "command": "at", "timestamp": "birthtime"}
            Order any list of timestamps from earliest timestamp to most recent. Timestamps in the form of lists can only use the command 'equal'. Any timestamps should be relevant to today's date. For command "range" starttime and endtime should be different where starttime < endtime.
            Timestamps should use 'YYYY-MM-DDTHH:MM:SS' format and must be relative to the time right now: {curr_date}. 
            Constraints: File size is within 1B-10GB inclusive. The start date for the timestamps is bounded by 2000, 10 25 and the current date inclusive. If any type of timestamp from modified, changed and accessed take on boundaries that are not within the birthtime, raise an error. If any of these constraints are broken, return an error message specifiying the specific error.
            """
