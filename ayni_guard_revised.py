...
class AyniGuard:
    ...
    def __init__(self, db_instance=None, llm_factory=None):
        """
    ...
    def ensure_collections(self):
        ...
        collections = [
            IndalekoDBCollections.Prompt_Cache_Recent,
            IndalekoDBCollections.Prompt_Cache_Archive,
            IndalekoDBCollections.Prompt_Stability_Metrics,
        ]

        for collection_name in collections:
            if not self.db.has_collection(collection_name):
                raise NotImplementedError("We don't have this implemented")

        # Create indexes for performance
        recent_cache = self.db.collection(IndalekoDBCollections.Prompt_Cache_Recent)
        recent_cache.ensure_hash_index(["prompt_hash"], unique=True)
        recent_cache.ensure_skiplist_index(["created_at"])

        archive_cache = self.db.collection(IndalekoDBCollections.Prompt_Cache_Archive)
    ...
    def check_cache(self, prompt_hash: str) -> AyniResult | None:
        ...
        cursor = self.db.aql.execute(
            aql,
            bind_vars={
                "@collection": IndalekoDBCollections.Prompt_Cache_Recent,
                "hash": prompt_hash,
                "now": now.isoformat(),
            },
        )
    ...
    def store_cache(self, prompt_hash: str, result: AyniResult, prompt: dict, user_id: str | None = None):
        ...
        """
        # Store in recent cache (hot tier)
        ...
        metrics = self.db.collection(IndalekoDBCollections.Prompt_Stability_Metrics)
        metrics.insert(
            {
                "prompt_hash": prompt_hash,
    ...
    def archive_old_prompts(self, age_days: int = 30):
        """
        ...
        self.db.aql.execute(
            aql,
            bind_vars={
                "@recent_collection": IndalekoDBCollections.Prompt_Cache_Recent,
                "@archive_collection": IndalekoDBCollections.Prompt_Cache_Archive,
    ...
    def detect_injection_patterns(self, limit: int = 100) -> list[dict]:
        ...
        cursor = self.db.aql.execute(
            aql,
            bind_vars={"@collection": IndalekoDBCollections.Prompt_Cache_Recent, "limit": limit},
        )
