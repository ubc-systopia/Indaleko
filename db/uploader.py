...
class IndalekoDBUploader:
    ...
    @staticmethod
    def build_load_string(file_name: str, collection_name: str, db: Union[IndalekoDBConfig, None] = None) -> str:
        '''
    ...
    @staticmethod
    def bulk_upload(file_name: str, db: Union[IndalekoDBConfig, None] = None, chunk_size: int = 1000) -> bool:
    ...
    @staticmethod
    def external_upload(file_name: str, db: Union[IndalekoDBConfig, None] = None, chunk_size: int = 1000) -> bool:
        '''
        This will upload the data to the database using arangoimport.
        '''
    @staticmethod
    def external_upload(file_name: str, db: Union[IndalekoDBConfig, None] = None) -> bool:
        '''
        This will upload the data to the database using arangoimport.
        '''
        if db is None:
        ...
        success = False
        try:
            load_string = IndalekoDBUploader.build_load_string(file_name, file_keys['collection'], db)
            process = subprocess.Popen(load_string, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    def copy_to_docker(**kwargs) -> None:
        ...
        for filename, dest_filename in [
            ...
            __run_docker_cmd(f'docker cp {filename} {
