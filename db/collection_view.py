...
class IndalekoCollectionView:
    ...
    def _get_existing_views(self, use_cache=True) -> dict[str, dict[str, Any]]:
        ...
        try:
            ...
            logging.exception(f"Failed to retrieve existing views: {e}")
        ...
        try:
            ...
            try:
                ...
                if "duplicate" in error_str and "name" in error_str:
                    ...
                    logging.warning(
                        f"View '{view_name}' already exists but wasn't detected",
                    )
                    self._existing_views = self._get_existing_views()
                    ...
                    try:
                        ...
                        logging.warning(
                            f"Error checking existing view: {view_check_error}",
                        )
                        return {
                            "status": "exists",
            ...
            logging.exception(f"Failed to create view '{view_name}': {e}")
            return {
                "status": "error",
                "message": f"Error creating view: {e!s}",
    ...
    def update_view(self, view_definition: IndalekoViewDefinition) -> dict[str, Any]:
        ...
        try:
            # Update the view
            properties = view_definition.get_creation_properties()
            self.db_config._arangodb.update_view(name=view_name, properties=properties)
            ...
            logging.exception(f"Failed to update view '{view_name}': {e}")
            return {
                "status": "error",
                "message": f"Error updating view: {e!s}",
                "view_definition": view_definition.model_dump(),
            }
    ...
    def delete_view(self, view_name: str) -> dict[str, Any]:
        ...
        try:
            # Delete the view
            self.db_config._arangodb.delete_view(view_name)

            # Update view cache
            self._existing_views = self._get_existing_views()

            return {"status": "success", "message": f"Deleted view '{view_name}'"}
        except Exception as e:
            logging.exception(f"Failed to delete view '{view_name}': {e}")
            return {"status": "error", "message": f"Error deleting view: {e!s}"}
