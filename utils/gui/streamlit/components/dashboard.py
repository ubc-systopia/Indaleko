...
def render_dashboard():
    ...
    st.markdown("<div class='sub-header'>Overview of your Unified Personal Index</div>", unsafe_allow_html=True)
    ...
    if storage_data:
        ...
        fig = px.pie(storage_data, values="count", names="storage", title="Files by Storage Volume")
    ...
    with col2:
        quick_debug = st.checkbox("Debug", help="Show detailed diagnostic information during search")

    if quick_query:
        with st.spinner("Searching..."):
            results = execute_query(quick_query, st.session_state.db_service, debug=quick_debug)
            if results:
                ...
                if isinstance(results, (list, tuple)):
                    ...
                    if len(results) > 5:
                        st.info(f"Showing 5 of {len(results)} results. Go to Search for more.")
