...
class IndalekoFilePicker:
    ...
    def _uri_to_local_path(self, uri: str, volume_guid: str) -> Optional[str]:
        """
    ...
    def pick_random_files(
        ...
        for _ in random_offsets:
            file_obj = IndalekoObject(**self.object_collection.collection.random())
    ...
    def pick_files_for_semantic_processing(
        self, 
        semantic_attribute_id: Union[uuid.UUID, str],
        process_func: Callable[['IndalekoObject', str], None] = None,
        count: int = 10,
        max_age_days: Optional[int] = None,
        min_last_processed_days: Optional[int] = None
    ) -> List[IndalekoObject]:
        """
        ...
        if min_last_processed_days is not None:
            cutoff_date = datetime.now(timezone.utc) - timedelta(days=min_last_processed_days)
            query += f"""
                LET attr = (
        ...
        # Execute the query
        try:
            results = self.db_config.db.aql.execute(query, bind_vars=bind_vars)
    ...
    def pick_all_files(
        ...
        try:
            ...
            cursor = self.db_config.db.aql.execute(query, batch_size=batch_size)
    ...
    def queue_for_background_processing(
        ...
        priority: int = 1,
        semantic_attribute_id: Optional[Union[uuid.UUID, str]] = None
    ) -> int:
        """
