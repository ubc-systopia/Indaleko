...
from ..protocol import EntityRole, EntityInterface, ENTITY_PROFILES
from ..core import (
    FireCircleEntity,
    create_storyteller,
    create_analyst,
...
class CircleSession:
    ...
    def __init__(
        ...
        entities: Optional[List[FireCircleEntity]] = None,
        coordinator: Optional[FireCircleEntity] = None
    ):
        \"\"\"
...
class FireCircleOrchestrator:
    ...
    def create_session(
        self,
        entities: Optional[List[EntityRole]] = None,
        session_id: Optional[str] = None
    ) -> CircleSession:
        \"\"\"
    ...
    def get_session(self, session_id: str) -> Optional[CircleSession]:
        \"\"\"
        Get a session by ID.
    ...
    def process_message(
        ...
        for entity in responding_entities:
            try:
                ...
                # Log error
                error_message = f\"Error processing message with {entity.role.value}: {str(e)}\"
                print(error_message)
                session.add_message(
                    \"system\",
                    error_message,
        ...
        synthesizer = next((e for e in session.entities if e.role == EntityRole.SYNTHESIZER), None)
