...
class MockWriter(IWriter):
    ...
    def write(self, arr) -> None:
        assert (
            type(arr) == list
        ), f"given arr is not a list; got arr of type {
            type(arr)}"
    ...
    def test_multiple_events(self):
        ...
        def ops(x):
            return [
                ...
                (
                    [
                        TestCompactRecord.generate_random_string(),
                        TestCompactRecord.generate_random_string(),
                        TestCompactRecord.generate_random_string(),
                    ],
                    x.add_event,
...
class TestLogCompactor(unittest.TestCase):
    ...
    def run_helper(self, test_num: int):
        ...
        match test_num:
            ...
            case 7:
                # (0, ['18:36:21.633351', 'close', '30', 'ampdaemon', '8310']),
                expects = [["8310", "ampdaemon", "", ["close|today_18:36:21.633351"]]]
        ...
        for log_record in state:
            assert (
                len(log_record) == expected_log_length
            ), f"expected to have fields in the log record; got={
                len(log_record)}"
        ...
        if test_num in (4,):
            ...
            assert (
                state
            ), f"the returned state is not valid; got {
        ...
        state_hash = {
            self.get_sha256(state_record): i for i, state_record in enumerate(state)
        }
        ...
        for expected_record in expects:
            ...
            assert (
                er_hash in state_hash
            ), f"the record does not exist in our state; \nexpected_record={
