...
class CompactRecord:
    ...
    def add_procname(self, procname: str) -> 'CompactRecord':
        ...
        """
        assert isinstance(procname, str), f'procname is not string; got {
            type(procname)}'
        if self.proc_name == '':
    ...
    def add_event(self, event_name: str, event_date: str, event_ts: str, procname: str = None, pid: str = None, path: str = None) -> 'CompactRecord':
        """
        ...
        assert procname is None or isinstance(
            procname, str), f'procname has to be None or of type str; got {type(procname)}'
        assert pid is None or isinstance(
            pid, str), f'pid has to be None or of type str; got {type(pid)}'
        assert path is None or isinstance(
            path, str), f'path has to be None or of type str; got {type(path)}'
...
class LogCompactor(IOperator):
    def __init__(self, writer: 'IWriter', datefunc=None) -> None:
        ...
        self.state: typing.Dict[typing.Tuple[str, str], CompactRecord] = collections.defaultdict(
            lambda: CompactRecord())

        self.datefunc = lambda: datetime.now().date().strftime(
            "%Y-%m-%d") if not datefunc else datefunc()
        self.writer = writer
