...
TEST_TEMPLATES = {
    ...
    "entity_extraction": {
        ...
        "variables": ["text"],
        "examples": [{"text": "Apple CEO Tim Cook announced new products at their Cupertino headquarters on Tuesday."}],
    },
}
...
class PromptManagementBenchmark:
        ...
        for prompt_name, prompt in TEST_PROMPTS.items():
            ...
            for strategies in strategies_to_test:
                ...
                prompt_results[strategy_key] = {
                    ...
                    "savings_percent": (
                        (statistics.mean(token_savings) / result.original_token_count) * 100
    ...
    def benchmark_caching_performance(self):
        ...
        for provider in self.providers:
            try:
                ...
                guardian = LLMGuardian(use_cache=True, cache_ttl=3600, archive_ttl=86400)
                ...
                for prompt_name, prompt in TEST_PROMPTS.items():
                    ...
                    provider_results[prompt_name] = {
                        ...
                        "time_savings_percent": (
                            ((statistics.mean(miss_times) - statistics.mean(hit_times)) / statistics.mean(miss_times))
                            * 100
                            if statistics.mean(miss_times) > 0
                            else 0
                        ),
                    }
    ...
    def benchmark_response_time(self):
        ...
        for provider in self.providers:
            try:
                ...
                connector_with_guardian = self.factory.create_connector(connector_type=provider, use_guardian=True)

                connector_without_guardian = self.factory.create_connector(connector_type=provider, use_guardian=False)
                ...
                for prompt_name, prompt in TEST_PROMPTS.items():
                    ...
                    provider_results[prompt_name] = {
                        ...
                        "overhead_percent": (
                            (
                                (statistics.mean(with_guardian_times) - statistics.mean(without_guardian_times))
                                / statistics.mean(without_guardian_times)
                            )
                            * 100
                            if statistics.mean(without_guardian_times) > 0
                            else 0
                        ),
                    }
    ...
    def benchmark_template_performance(self):
        ...
        for template_id, variable_sets in TEST_TEMPLATE_VARIABLES.items():
            ...
            template_results["optimized"] = {
                ...
                "savings_percent": (
                    (statistics.mean(optimized_token_savings) / statistics.mean(regular_token_counts)) * 100
                    if statistics.mean(regular_token_counts) > 0
                    else 0
                ),
            }
            ...
            template_results["optimization_overhead_percent"] = (
                ...
                * 100
                if template_results["regular"]["avg_creation_time_ms"] > 0
                else 0
            )
    ...
    def generate_visualizations(self):
        ...
        if self.results["token_optimization"]:
            self._create_token_optimization_chart(os.path.join(self.output_dir, f"token_optimization_{timestamp}.png"))
        ...
        if self.results["response_time"]:
            self._create_response_time_chart(os.path.join(self.output_dir, f"response_time_{timestamp}.png"))

        # 5. Memory Usage Chart
        if self.results["memory_usage"]:
            self._create_memory_usage_chart(os.path.join(self.output_dir, f"memory_usage_{timestamp}.png"))
        ...
        # 7. Summary Chart
        self._create_summary_chart(os.path.join(self.output_dir, f"summary_{timestamp}.png"))
    ...
    def _create_caching_performance_chart(self, filename):
        ...
        if not any("error" not in provider_data for provider_data in self.results["caching_performance"].values()):
            plt.text(
                0.5,
                0.5,
                "No valid caching performance data available",
        ...
        providers = [
            p
            for p in self.results["caching_performance"].keys()
    ...
    def _create_response_time_chart(self, filename):
        ...
        if not any("error" not in provider_data for provider_data in self.results["response_time"].values()):
            plt.text(
                0.5,
                0.5,
                "No valid response time data available",
                horizontalalignment="center",
                verticalalignment="center",
            )
            plt.savefig(filename)
        ...
        providers = [p for p in self.results["response_time"].keys() if "error" not in self.results["response_time"][p]]
    ...
    def _create_template_performance_chart(self, filename):
        ...
        regular_times = [
            self.results["template_performance"][template]["regular"]["avg_creation_time_ms"] for template in templates
        ]
        optimized_times = [
            self.results["template_performance"][template]["optimized"]["avg_creation_time_ms"]
        ...
        savings_pct = [
            self.results["template_performance"][template]["optimized"]["savings_percent"] for template in templates
        ]
    ...
    def _create_summary_chart(self, filename):
        ...
        if self.results["token_optimization"]:
            ...
            plt.text(
                0.5,
                0.5,
                "No token optimization data available",
                horizontalalignment="center",
                verticalalignment="center",
                transform=plt.gca().transAxes,
            )
        ...
        if self.results["verification_performance"]:
            ...
            plt.text(
                0.5,
                0.5,
                "No verification performance data available",
                horizontalalignment="center",
                verticalalignment="center",
                transform=plt.gca().transAxes,
            )
...
def main():
    ...
    parser.add_argument("--use-db", action="store_true", help="Use database for templates and caching")
    parser.add_argument("--skip-token-optimization", action="store_true", help="Skip token optimization benchmarks")
    parser.add_argument("--skip-verification", action="store_true", help="Skip verification benchmarks")
    parser.add_argument("--skip-caching", action="store_true", help="Skip caching benchmarks")
    parser.add_argument("--skip-response-time", action="store_true", help="Skip response time benchmarks")
    parser.add_argument("--skip-memory", action="store_true", help="Skip memory benchmarks")
    parser.add_argument("--skip-templates", action="store_true", help="Skip template benchmarks")
    ...
    if (
        args.skip_token_optimization
        and args.skip_verification
        and args.skip_caching
        and args.skip_response_time
        and args.skip_memory
        and args.skip_templates
    ):
