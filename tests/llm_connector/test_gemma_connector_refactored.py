"""
Tests for the refactored Gemma connector with prompt management integration.
...
class TestGemmaConnector(unittest.TestCase):
    """
    Tests for the GemmaConnector class with prompt management integration.
    """

    def setUp(self):
        """Set up test fixtures."""
        # Mock openai.OpenAI
        self.mock_openai_patcher = patch("query.utils.llm_connector.gemma_connector_refactored.openai")
        self.mock_openai = self.mock_openai_patcher.start()
        self.mock_client = MagicMock()
        self.mock_openai.OpenAI.return_value = self.mock_client

        # Mock LLMGuardian
        self.mock_guardian_patcher = patch("query.utils.llm_connector.gemma_connector_refactored.LLMGuardian")
        ...
        self.gemma_with_guardian = GemmaConnector(
            model="gemma-2b-instruct",
            use_guardian=True,
        ...
        self.mock_chat_message.content = "test response"
        self.mock_chat_response.choices = [MagicMock(message=MagicMock(content='{"test": "response"}'))]
        self.mock_client.chat.completions.create.return_value = self.mock_chat_response

        # Mock completions fallback
        self.mock_completions_response = MagicMock()
        self.mock_completions_response.choices = [MagicMock(text='{"test": "response"}')]
        self.mock_client.completions.create.return_value = self.mock_completions_response
    ...
    def test_init_without_guardian(self):
        ...
        self.assertEqual(self.gemma.model, "gemma-2b-instruct")
        self.assertEqual(self.gemma.base_url, "http://localhost:1234/v1")
        self.assertFalse(self.gemma.use_guardian)
        self.assertIsNone(self.gemma.guardian)

    def test_init_with_guardian(self):
        """Test initialization with LLMGuardian."""
        self.assertEqual(self.gemma_with_guardian.model, "gemma-2b-instruct")
        self.assertTrue(self.gemma_with_guardian.use_guardian)
        self.assertEqual(self.gemma_with_guardian.guardian, self.mock_guardian)
        self.mock_guardian_class.assert_called_once()

    def test_get_llm_name(self):
        """Test get_llm_name method."""
        self.assertEqual(self.gemma.get_llm_name(), "Gemma")

    def test_count_tokens(self):
        """Test token counting method."""
        # Since we may not have tiktoken, test the fallback method
        self.gemma.tokenizer = None
        count = self.gemma._count_tokens("This is a test sentence with more than 4 characters.")
    ...
    def test_extract_json_from_content(self):
        ...
        result = self.gemma._extract_json_from_content(valid_json)
        self.assertEqual(result, {"key": "value", "number": 42})

        # Test invalid JSON
        invalid_json = "This is not JSON"
        result = self.gemma._extract_json_from_content(invalid_json)
    ...
    def test_generate_query_without_guardian(self):
        ...
        self.mock_chat_response.choices[0].message.content = (
            '{"query": "find my documents", "translated_query": "FOR doc IN documents RETURN doc", "explanation": "Test", "error": false}'
        )

        # Call method
        result = self.gemma.generate_query(prompt)
    ...
    def test_generate_query_with_guardian(self):
        ...
        # Call method
        result = self.gemma_with_guardian.generate_query(prompt)
    ...
    def test_get_completion_without_guardian(self):
        ...
        self.mock_chat_response.choices[0].message.content = "This is a test response."

        # Call method
        completion, metadata = self.gemma.get_completion(
        ...
        self.assertEqual(metadata["provider"], "gemma")
        self.assertEqual(metadata["model"], "gemma-2b-instruct")
    ...
    def test_get_completion_with_guardian(self):
        ...
        # Call method
        completion, metadata = self.gemma_with_guardian.get_completion(
            system_prompt="You are a helpful assistant.",
            user_prompt="Tell me about Indaleko.",
        )
    ...
    def test_summarize_text_without_guardian(self):
        ...
        # Set up mock response
        self.mock_chat_response.choices[0].message.content = "This is a summary."

        # Call method
        result = self.gemma.summarize_text("This is a long text that needs to be summarized.")
    ...
    def test_summarize_text_with_guardian(self):
        ...
        # Call method
        result = self.gemma_with_guardian.summarize_text("This is a long text that needs to be summarized.")
    ...
    def test_extract_keywords_without_guardian(self):
        ...
        # Set up mock response
        self.mock_chat_response.choices[0].message.content = "keyword1, keyword2, keyword3"

        # Call method
        result = self.gemma.extract_keywords("This is text containing keywords.")
    ...
    def test_extract_keywords_with_guardian(self):
        ...
        # Call method
        result = self.gemma_with_guardian.extract_keywords("This is text containing keywords.")
    ...
    def test_classify_text_without_guardian(self):
        ...
        # Set up mock response
        self.mock_chat_response.choices[0].message.content = "Technology"

        # Call method
        result = self.gemma.classify_text("This is about AI.", ["Technology", "Science", "Art"])
    ...
    def test_classify_text_with_guardian(self):
        ...
        # Call method
        result = self.gemma_with_guardian.classify_text("This is about AI.", ["Technology", "Science", "Art"])
    ...
    def test_answer_question_without_guardian(self):
        ...
        # Set up mock response
        self.mock_chat_response.choices[0].message.content = '{"answer": "Test answer", "confidence": 0.9}'

        # Call method
        result = self.gemma.answer_question(
    ...
    def test_answer_question_with_guardian(self):
        ...
        # Call method
        result = self.gemma_with_guardian.answer_question(
            context="This is context.",
            question="What is this about?",
            schema={"type": "object", "properties": {"answer": {"type": "string"}, "confidence": {"type": "number"}}},
        )
    ...
    def test_chat_completion_fallback(self):
        """Test that the connector falls back to completions API when chat fails."""
        # Setup to make chat API fail
        self.mock_client.chat.completions.create.side_effect = Exception("Chat API not available")

        # Call method
        result = self.gemma.generate_text("Generate some text.")

        # Verify fallback was used
        self.mock_client.completions.create.assert_called_once()
    ...
    def test_extract_semantic_attributes_without_guardian(self):
        ...
        # Set up mock response
        self.mock_chat_response.choices[0].message.content = (
            '{"entities": ["Indaleko", "AI"], "keywords": ["system", "database"]}'
        )

        # Call method
        result = self.gemma.extract_semantic_attributes("This is text about Indaleko.")

        # Verify result
        self.assertEqual(result["entities"], ["Indaleko", "AI"])
    ...
    def test_extract_semantic_attributes_with_guardian(self):
        ...
        # Mock guardian response
        self.mock_guardian.get_completion_from_prompt.return_value = (
            '{"entities": ["Guardian", "Indaleko"], "keywords": ["security", "verification"]}',
            {"verification": {"allowed": True}},
        )

        # Call method
        result = self.gemma_with_guardian.extract_semantic_attributes("This is text about Indaleko.")

        # Verify result
        self.assertEqual(result["entities"], ["Guardian", "Indaleko"])
