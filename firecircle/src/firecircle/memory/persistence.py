...
from firecircle.protocol.message import Message
...
class CircleMemory(BaseModel):
    ...
    entity_id: str = Field(
        ...,
        description="ID of the entity that created this memory"
    )
...
class CommuniqueMemory(CircleMemory):
    ...
    title: str = Field(
        ...,
        description="Title of the communiqué"
    )
    
    content: str = Field(
        ...,
        description="Content of the communiqué"
    )
...
class MemoryStore(Generic[T], abc.ABC):
    ...
    def __init__(
        ...
        self.logger = logger or logging.getLogger(f"{__name__}.{memory_class.__name__}Store")
    ...
    @abc.abstractmethod
    def retrieve(self, memory_id: str) -> Optional[T]:
        """
            ...
            for key, value in query.items():
                if hasattr(memory, key):
                    ...
                    if isinstance(value, dict) and "__op" in value:
                        ...
                        if op == "eq" and memory_value != compare_value:
                            match = False
                        elif op == "ne" and memory_value == compare_value:
                            match = False
                        elif op == "gt" and memory_value <= compare_value:
                            match = False
                        elif op == "lt" and memory_value >= compare_value:
                            match = False
                        elif op == "gte" and memory_value < compare_value:
                            match = False
                        elif op == "lte" and memory_value > compare_value:
                            match = False
                        elif op == "in" and memory_value not in compare_value:
                            match = False
                        elif op == "contains" and compare_value not in memory_value:
                            match = False
