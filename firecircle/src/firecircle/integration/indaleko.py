...
from firecircle.memory.persistence import CircleMemory, ConversationMemory, InsightMemory
...
class IndalekoIntegration:
    ...
    def _detect_indaleko_root(self) -> Optional[str]:
        """
    ...
    def execute_query(self, query: str) -> Dict[str, Any]:
        ...
        if not self.query_system_available:
            self.logger.warning("Indaleko query system not available, cannot execute query")
    ...
    def explain_query(self, query: str) -> Dict[str, Any]:
        ...
        """
        if not self.query_system_available:
            self.logger.warning("Indaleko query system not available, cannot explain query")
            return {"error": "Query system not available"}
    ...
    def get_relevant_archivist_memories(self, query: str, max_results: int = 5) -> List[Dict[str, Any]]:
        """
    ...
    def initialize_circle_from_continuation(
        self, continuation_id: str, entity_registry: EntityRegistry, 
        orchestrator: CircleOrchestrator, context: CircleContext
    ) -> bool:
        """
    ...
    def run_indaleko_command(self, command: List[str]) -> Tuple[int, str, str]:
        ...
        try:
            ...
            if return_code == 0:
                ...
                self.logger.warning(f"Command failed with code {return_code}: {cmd_str}")
