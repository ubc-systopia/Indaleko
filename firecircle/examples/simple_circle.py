...
from firecircle.protocol.orchestrator import CircleOrchestrator, TurnTakingPolicy, ConversationPhase
from firecircle.entities.base import Entity, EntityCapability
from firecircle.entities.registry import EntityRegistry
from firecircle.memory.context import CircleContext, AccessLevel
...
class SimpleEntity(Entity):
    ...
    def _respond_to_question(self, message: Message) -> List[Message]:
        ...
        if not templates:
            ...
            return [
                self.create_message(
                    f"I don't have a specific response to that question.",
...
def run_simple_circle():
    ...
    engineer = SimpleEntity(
        ...
        knowledge_areas=["implementation", "technical", "architecture", "system", "database"],
        response_templates={
            "relevant_question": [
    ...
    user_researcher = SimpleEntity(
        ...
        knowledge_areas=["users", "humans", "experience", "interface", "needs", "design"],
        response_templates={
            "relevant_question": [
