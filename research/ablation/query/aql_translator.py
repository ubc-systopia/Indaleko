...
class AQLQueryTranslator:
    ...
    def translate_to_aql(
        ...
        query_text: str,
        collection: str,
        activity_types: list[Any] | None = None,
        ...
        if activity_types and len(activity_types) > 1 and relationship_type:
            ...
            return self._translate_cross_collection_query(query_text, collection, activity_types, relationship_type)
    ...
    def _translate_single_collection_query(self, query_text: str, collection: str) -> tuple[str, dict[str, Any]]:
        """
    ...
    def _translate_cross_collection_query(
        ...
        # Determine specific relationship types based on collections
        relationship_field = relationship_type

        # Map collection pairs to their relationship fields
        relationship_map = {
        ...
        if "task" in secondary_collection:
            secondary_type = "task"
        elif "collaboration" in secondary_collection:
            secondary_type = "collaboration"
        elif "location" in secondary_collection:
            secondary_type = "location"
        elif "music" in secondary_collection:
            secondary_type = "music"

        # Get possible relationship fields for these collections
        relationship_fields = relationship_map.get((primary_type, secondary_type), [])

        # If the provided relationship type is not in our map, use the default
        if relationship_fields and relationship_type not in relationship_fields:
        ...
        FOR primary_doc IN {primary_collection}
          ...
          FILTER primary_doc.references.{relationship_field} != null
          FOR related_doc IN {secondary_collection}
            FILTER related_doc._id IN primary_doc.references.{relationship_field}
        """

        # Add specific filters based on collection types and query content
        collection_specific_filters = []

        # Task-specific filters
        if "task" in query_text.lower():
            if primary_type == "task":
                collection_specific_filters.append("primary_doc.active == true")
            elif secondary_type == "task":
                collection_specific_filters.append("related_doc.active == true")

        # Music-specific filters
        if "music" in query_text.lower() or "artist" in query_text.lower() or "song" in query_text.lower():
            for artist in ["taylor swift", "ed sheeran", "drake", "adele", "billie eilish"]:
                if artist in query_text.lower():
                    if primary_type == "music":
                        collection_specific_filters.append(f"LOWER(primary_doc.artist) == '{artist}'")
                    elif secondary_type == "music":
                        collection_specific_filters.append(f"LOWER(related_doc.artist) == '{artist}'")

        # Location-specific filters
        if "office" in query_text.lower() or "headquarters" in query_text.lower():
            if primary_type == "location":
                collection_specific_filters.append("primary_doc.location_type == 'office'")
            elif secondary_type == "location":
                collection_specific_filters.append("related_doc.location_type == 'office'")

        # Add the collection-specific filters if any
        if collection_specific_filters:
            aql += "    FILTER " + " AND ".join(collection_specific_filters) + "\n"

        # Add entity filters based on extracted entities
        if entities:
            entity_conditions = []
            for i, entity in enumerate(entities):
                ...
                # Add specific field conditions based on primary collection type
                if primary_type == "task":
                    entity_conditions.append(f"LOWER(primary_doc.task_name) LIKE @entity_{i}")
                    entity_conditions.append(f"LOWER(primary_doc.application) LIKE @entity_{i}")
                elif primary_type == "collaboration":
                    entity_conditions.append(f"LOWER(primary_doc.event_type) LIKE @entity_{i}")
                    entity_conditions.append(f"LOWER(primary_doc.platform) LIKE @entity_{i}")
                elif primary_type == "location":
                    entity_conditions.append(f"LOWER(primary_doc.location_name) LIKE @entity_{i}")
                    entity_conditions.append(f"LOWER(primary_doc.location_type) LIKE @entity_{i}")
                    ...
                    entity_conditions.append(f"LOWER(primary_doc.album) LIKE @entity_{i}")
                    entity_conditions.append(f"LOWER(primary_doc.genre) LIKE @entity_{i}")

                # Add specific field conditions based on secondary collection type
                if secondary_type == "task":
                    entity_conditions.append(f"LOWER(related_doc.task_name) LIKE @entity_{i}")
                    entity_conditions.append(f"LOWER(related_doc.application) LIKE @entity_{i}")
                elif secondary_type == "collaboration":
                    entity_conditions.append(f"LOWER(related_doc.event_type) LIKE @entity_{i}")
                    entity_conditions.append(f"LOWER(related_doc.platform) LIKE @entity_{i}")
                elif secondary_type == "location":
                    entity_conditions.append(f"LOWER(related_doc.location_name) LIKE @entity_{i}")
                    entity_conditions.append(f"LOWER(related_doc.location_type) LIKE @entity_{i}")
                elif secondary_type == "music":
                    entity_conditions.append(f"LOWER(related_doc.artist) LIKE @entity_{i}")
                    entity_conditions.append(f"LOWER(related_doc.track) LIKE @entity_{i}")
                    entity_conditions.append(f"LOWER(related_doc.album) LIKE @entity_{i}")
                    entity_conditions.append(f"LOWER(related_doc.genre) LIKE @entity_{i}")
