"""Unit tests for the task activity collector and recorder."""
...
class TestTaskActivityCollector(unittest.TestCase):
    """Test cases for the task activity collector."""
    ...
    def test_collect_method(self):
        ...
        self.assertIn("task_type", activity_dict)
        self.assertIn("status", activity_dict)
        self.assertIn("priority", activity_dict)
        self.assertIn("created_at", activity_dict)
        self.assertIn("source", activity_dict)

        # Verify the task name is one of the expected values
        self.assertIn(activity_dict["task_name"], self.collector.tasks)
        ...
        self.assertIn(activity_dict["status"], self.collector.task_statuses)

        # Verify the priority is one of the expected values
        self.assertIn(activity_dict["priority"], self.collector.priorities)
    ...
    def test_generate_truth_data_method(self):
        ...
        # Generate truth data for a query that contains a task name
        ...
        # Verify truth data contains entries for the task
        self.assertGreater(len(truth_data), 0)

        # Generate truth data for a query that contains an application
        query = "Find tasks in Microsoft Word"
        truth_data = self.collector.generate_truth_data(query)
    ...
    def test_generate_matching_data_method(self):
        ...
        # Generate matching data for a query that contains a task name
        query = "Find files related to Budget Planning"
        ...
        # Verify each matching activity contains the task name
        for activity_dict in matching_data:
            self.assertEqual(activity_dict["task_name"], "Budget Planning")

    def test_generate_non_matching_data_method(self):
        """Test the generate_non_matching_data method of the TaskActivityCollector."""
        # Generate non-matching data for a query that contains a task name
        query = "Find files related to Marketing Campaign"
        ...
        # Verify each non-matching activity does not contain the task name
        for activity_dict in non_matching_data:
            self.assertNotEqual(activity_dict["task_name"], "Marketing Campaign")

        # Verify created_at dates are further in the past
        for activity_dict in non_matching_data:
            # Older tasks should have been created longer ago
            days_ago = (activity_dict["created_at"].now() - activity_dict["created_at"]).days
            self.assertGreaterEqual(days_ago, 10)

    def test_seed_method(self):
        """Test the seed method of the TaskActivityCollector."""
        ...
        if activity1["application"] != activity2["application"]:
            different_attributes += 1
        if activity1["status"] != activity2["status"]:
            different_attributes += 1
...
class TestTaskActivityRecorder(unittest.TestCase):
    """Test cases for the task activity recorder."""
    ...
    def setUp(self):
        ...
        self.patcher = patch('research.ablation.recorders.task_recorder.IndalekoDBConfig')
        self.mock_db_config = self.patcher.start()
        self.mock_db_config.return_value.get_arangodb.return_value = self.db_mock
        self.db_mock.collection.side_effect = lambda name: self.collection_mock if name == IndalekoDBCollections.Indaleko_Ablation_Task_Activity_Collection else self.truth_collection_mock
    ...
    def test_record_method(self):
        """Test the record method of the TaskActivityRecorder."""
        # Create test data
        test_data = {
    ...
    def test_record_batch_method(self):
        """Test the record_batch method of the TaskActivityRecorder."""
    ...
    def test_record_truth_data_method(self):
        """Test the record_truth_data method of the TaskActivityRecorder."""
    ...
    def test_get_collection_name_method(self):
        """Test the get_collection_name method of the TaskActivityRecorder."""
        # Get the collection name
        collection_name = self.recorder.get_collection_name()

        # Verify the collection name is correct
        self.assertEqual(collection_name, IndalekoDBCollections.Indaleko_Ablation_Task_Activity_Collection)

    def test_count_records_method(self):
        """Test the count_records method of the TaskActivityRecorder."""
        ...
        self.assertIn(f"LENGTH({IndalekoDBCollections.Indaleko_Ablation_Task_Activity_Collection})", call_args)

    def test_delete_all_method(self):
        """Test the delete_all method of the TaskActivityRecorder."""
        # Set up mock to return success
        ...
        self.db_mock.aql.execute.assert_called_once()
        call_args = self.db_mock.aql.execute.call_args[0][0]
        self.assertIn(f"REMOVE doc IN {IndalekoDBCollections.Indaleko_Ablation_Task_Activity_Collection}", call_args)
