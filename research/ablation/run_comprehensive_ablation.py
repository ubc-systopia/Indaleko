...
def generate_test_data(entity_manager: NamedEntityManager, count: int = 100):
    ...
    try:
        ...
        if location_success:
            collections_loaded.append("Location")
            test_data["location"] = location_data
            logging.info(f"Successfully loaded {len(location_data)} location activity records")
        else:
            logging.warning("Failed to record location data")
    except Exception as e:
        logging.exception(f"Error generating location data: {e}")
    ...
    try:
        ...
        if task_success:
            collections_loaded.append("Task")
            test_data["task"] = task_data
            logging.info(f"Successfully loaded {len(task_data)} task activity records")
        else:
            logging.warning("Failed to record task data")
    except Exception as e:
        logging.exception(f"Error generating task data: {e}")

    # Generate and record music data
    try:
        music_collector = MusicActivityCollector(entity_manager=entity_manager, seed_value=42)
        music_recorder = MusicActivityRecorder()

        music_data = music_collector.generate_batch(count)
        
        # Ensure all music data has an ID field
        for item in music_data:
            if "id" not in item:
                content_hash = f"music:{item.get('track_name', 'unknown')}:{item.get('artist', 'unknown')}"
                item["id"] = generate_deterministic_uuid(content_hash)

        music_success = music_recorder.record_batch(music_data)
        
        if music_success:
            collections_loaded.append("Music")
            test_data["music"] = music_data
            logging.info(f"Successfully loaded {len(music_data)} music activity records")
        else:
            logging.warning("Failed to record music data")
    except Exception as e:
        logging.exception(f"Error generating music data: {e}")

    logging.info(f"Successfully loaded data for {len(collections_loaded)} collections: {', '.join(collections_loaded)}")
    return test_data


def generate_cross_collection_queries(entity_manager: NamedEntityManager, ablation_tester: AblationTester, count: int = 5):
...
def test_ablation_impact(ablation_tester: AblationTester, queries: List[Dict[str, Any]]):
...
def visualize_results(impact_metrics: Dict[str, Any], output_dir: str):
...
def generate_summary_report(impact_metrics: Dict[str, Any], output_dir: str):
...
def clear_existing_data():
    ...
    try:
        ...
        logging.exception(f"Failed to clear data: {e}")
...
def main():
    ...
    try:
        ...
        # Generate test data
        test_data = generate_test_data(entity_manager, count=args.count)
        if not test_data:
        ...
        if not sanity_check_passed:
            logger.warning("Data sanity check failed, but continuing with test")
        ...
        logger.exception(f"Error running ablation tests: {e}")
        sys.exit(1)
    finally:
        # Clean up
        if 'ablation_tester' in locals():
