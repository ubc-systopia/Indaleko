...
class LocationActivityRecorder(ISyntheticRecorder):
    ...
    def record(self, data: dict) -> bool:
        """Record location activity data to the database.

        Args:
            data: The location activity data to record.
        ...
        try:
            # Convert to LocationActivity model for validation
            activity = LocationActivity(**data)
            ...
            self.logger.info(f"Recorded location activity with _key: {result['_key']}")
            return True
        except Exception as e:
            self.logger.error(f"Failed to record location activity: {e}")
            return False

    def record_batch(self, data_batch: list[dict[str, Any]]) -> bool:
        """Record a batch of location activity data to the database.

        Args:
            data_batch: List of location activity data to record.
        ...
        try:
            ...
            self.logger.info(f"Recorded {len(results)} location activities in batch")
            return True
        except Exception as e:
            self.logger.error(f"Failed to record location activity batch: {e}")
            return False
    ...
    def get_record_by_id(self, record_id: UUID) -> dict[str, Any] | None:
        """Get a location activity record by its ID.
